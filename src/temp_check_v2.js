  ...
  (function () {
    ...
    var QuillManager = {
      ...
      init: function (containerId, initialContent, placeholder, onChange) {
        ...
        // Add double-click handler for editing formulas
        // Note: Quill renders formula as a specialized embed. We can detect clicks on it.
        // However, standard Quill doesn't easily expose click on embed. 
        // We will try detecting selection change to a formula blot.
        quill.on('selection-change', function (range, oldRange, source) {
          if (range && range.length === 0) { // Cursor selection
            var [leaf, offset] = quill.getLeaf(range.index);
            if (leaf.statics.blotName === 'formula') {
              // It's a formula! Open editor logic could go here if we want auto-open on click
              // Or we can rely on a specific interaction. 
              // For now, let's just allow the toolbar button to insert.
              // To Edit: We actually need to know the 'value' of the blot.
              // A common pattern is to select the formula and hit the toolbar button again?
              // Or custom UI overlay. Let's keep it simple for MVP: Toolbar button inserts new.
              // But user asked for "Edit".
              // Let's enable Clicking the formula to edit.
...
<body>
    ...
    function renderPreviewQuestion() {
      ...
      html += '</div>';
      html += '<h3 class="text-2xl font-bold text-brand-dark-gray dark:text-brand-white">' + (q.questionText || 'No question text') + '</h3>';
      html += '</div>';

      // Question image
      if (q.questionImageURL) {
        html += '<div class="mb-6">';
        html += '<img src="' + (q.questionImageURL) + '" alt="Question image" class="max-w-full max-h-96 rounded-lg border border-brand-light-gray dark:border-brand-dark-gray/40" referrerpolicy="no-referrer" onerror="this.style.display=\'none\';">';
        html += '</div>';
      }
      ...
      if (q.options && q.options.length > 0) {
        ...
        q.options.forEach(function (opt, optIndex) {
          ...
          if (opt.imageURL) {
            html += '<img src="' + (opt.imageURL) + '" alt="Option image" class="mt-2 max-w-[200px] max-h-[150px] rounded border border-brand-light-gray dark:border-brand-dark-gray/40" referrerpolicy="no-referrer" onerror="this.style.display=\'none\';">';
          }
          html += '</div>';
          if (isCorrect) {
    ...
    function previewEditQuestion() {
      if (!currentPreviewPoll) return;
      closePollPreview();
      startEditPoll(currentPreviewPoll.pollId, currentPreviewQuestionIndex);
    }
    ...
    function renderWizardQuestions() {
      ...
      wizardState.questions.forEach(function (q, qIndex) {
        ...
        // METADATA: Custom Metacognition Prompt (Conditional)
        var metaPromptHtml = '';
        if (q.metacognitionEnabled) {
          metaPromptHtml = '<div class="px-4 pt-3 pb-0">' +
            '<p class="text-xs font-semibold text-slate-500 mb-1">Metacognition Prompt (Optional)</p>' +
            '<div id="q-meta-' + qIndex + '" class="bg-white" style="height: 80px;"></div>' +
            '</div>';
        }

        // Compact Question Body
        var bodyHtml = metaPromptHtml + '<div class="question-card-body p-4 space-y-3">' +
          // Question Stem - Rich Text only
          '<div class="flex-1 flex flex-col">' +
          '<label class="text-xs font-semibold text-slate-500 mb-1">Question Stem</label>' +
          '<div id="' + questionInputId + '" class="bg-white" style="min-height: 120px; height: auto;"></div>' +
          '</div></div>';
        ...
        q.options.forEach(function (opt, optIndex) {
          ...
          var correctBorder = isCorrect ? 'border-green-400 bg-green-50/10' : 'border-brand-light-gray/80';
          var optImagePreview = getImagePreviewUrl(opt.imageURL, opt.imageFileId);
          var optIsUploading = opt.imageURL === 'UPLOADING';

          bodyHtml += '<div class="answer-option flex items-start gap-2 p-2 rounded-lg border ' + correctBorder + ' transition-all">' +
            ...
            // Option text (Quill)
            '<div class="flex-1">' +
            '<div id="answer-input-' + questionNumber + '-' + optionLabels[optIndex].toLowerCase() + '" class="bg-white" style="min-height: 80px; height: auto;"></div>' +
            '</div>';

          // Remove option button only (no image upload)
          bodyHtml += '<div class="flex flex-col gap-1 mt-1">' +
            '<button type="button" class="p-1 rounded text-slate-300 hover:text-red-500 transition-colors" onclick="removeWizardOption(' + qIndex + ',' + optIndex + ')">' +
            '<span class="material-symbols-outlined text-base">close</span></button>' +
            '</div></div>'; // Close flex col + answer option
        });
        bodyHtml += '</div>';
        ...
        questionDiv.addEventListener('drop', function (e) {
          ...
          if (wizardDraggedIndex !== null && wizardDraggedIndex !== dropIndex) {
            // Sync content before reordering to prevent data loss
            syncWizardQuillContent();
