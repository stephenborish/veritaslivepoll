  (function () {
    ...
    var CONFIDENCE_LEVEL_STYLES = {
      guessing: {
        ...
        badgeClass:
          "bg-red-500/10 text-red-700 dark:text-red-200 border border-red-500/30",
        barClass: "bg-red-500",
      ...
      "very-sure": {
        ...
        badgeClass:
          "bg-blue-500/10 text-blue-700 dark:text-blue-200 border border-blue-500/30",
      ...
      certain: {
        ...
        badgeClass:
          "bg-green-500/10 text-green-700 dark:text-green-200 border border-green-500/30",
    ...
    function updateProgressRing(ringId, percentage) {
      ...
      const circumference = 138.23; // 2 * π * 22 (radius)
    ...
    function renderSparkline(containerId, data, color) {
      ...
      data.forEach((value, index) => {
        ...
        const y =
          padding + (height - 2 * padding) * (1 - (value - min) / range);
    ...
    function triggerCelebration(element, type = "default") {
      ...
      if (type === "all-responded") {
        showToast(
          ...
          "Great work! Everyone has submitted their answer.",
        );
        ...
        showToast(
          "success",
          "💯 Perfect Accuracy!",
          "100% of students got the answer correct!",
        );
        playSoundEffect("celebration");
      }
    }
    ...
    function trackQuestionAdvance(questionNum) {
      addActivityItem(
        "info",
        "arrow_forward",
        `Advanced to Question ${questionNum}`,
      );
    }
    ...
    function VeritasCombobox(config) {
      ...
      this.renderItem =
        config.renderItem ||
        function (item, selected, active) {
          var checkIcon = selected
            ? '<span class="absolute inset-y-0 right-0 flex items-center pr-4 text-veritas-navy"><span class="material-symbols-outlined text-sm">check</span></span>'
            : "";
          var activeClass = active
            ? "bg-veritas-navy text-white"
            : "text-gray-900";
      ...
      this.container =
        ...
        (this.input ? this.input.parentElement : null);

      if (!this.input || !this.container) {
        console.warn(
          "Combobox init failed: missing input or container",
          this.inputId,
        );
        return;
      }
      ...
      this.button = this.container.querySelector(
        "button[data-combobox-toggle]",
      );
      this.optionsList = document.createElement("ul");
      this.optionsList.className =
        "absolute z-[100] mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm hidden";
      ...
      this.input.addEventListener("keydown", function (e) {
        ...
        switch (e.key) {
          case "ArrowDown":
            e.preventDefault();
            self.activeIndex =
              (self.activeIndex + 1) % self.filteredData.length;
    ...
    VeritasCombobox.prototype.renderOptions = function () {
      ...
      if (this.filteredData.length === 0) {
        this.optionsList.innerHTML =
          '<div class="relative cursor-default select-none py-2 px-4 text-gray-700">Nothing found.</div>';
      } else {
        this.optionsList.innerHTML = this.filteredData
    ...
    window.expandImage = function (imgElement) {
      ...
      var modalHtml =
        '<div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.9);z-index:9999;display:flex;align-items:center;justify-content:center;cursor:pointer;" onclick="this.remove()">';
      modalHtml +=
    ...
    var PREVIEW_PAYLOAD = {
      ...
      polls: [
        ...
        },
        {
          ...
          questions: [
            {
              questionText: "How many moles are in 36 grams of water (H₂O)?",
              options: [
                { text: "1.0 mol" },
                { text: "2.0 mol" },
                { text: "0.5 mol" },
            ...
            {
              questionText:
                "What is the limiting reactant when 2 mol of H₂ reacts with 1 mol of O₂?",
              options: [
                { text: "Hydrogen" },
                { text: "Oxygen" },
                { text: "Water" },
      ...
      dashboardSummary: {
        recentSessions: [
          {
            sessionName: "Cell Energy Exit Ticket",
          ...
          {
            sessionName: "Stoichiometry Practice",
          ...
          {
            sessionName: "Industrial Revolution Check",
            date: new Date(Date.now() - 3600 * 1000 * 65).toISOString(),
            className: "World History – Period 7",
            participationPct: 83,
            masteryPct: 69,
            flags: 0,
          },
          ...
          questions: [
            {
              ...
              options: [
                ...
                { text: "Protein factory" },
                { text: "Genetic blueprint" },
                { text: "Cell wall" },
              ],
        ...
        kpis: [
          {
            ...
            value: "88%",
            delta: 5,
            tooltip: "Average participation over the last 7 days.",
          },
          {
            label: "Mastery Trend",
            value: "74%",
            delta: 3,
            tooltip: "Average mastery compared to last week.",
          },
          {
            label: "Integrity Events",
            value: "2",
            delta: -1,
            tooltip: "Academic integrity events this week.",
          },
        ],
        topics: [
    ...
    function bootstrapAppsScriptRuntime() {
      // WEB DEPLOYMENT OVERRIDE
      // If we are served via Firebase Hosting (or localhost), assume Firebase Mode immediately.
      var hostname = window.location.hostname;
      if (
        hostname.includes("web.app") ||
        hostname.includes("veritas.courses") ||
        hostname === "localhost" ||
        hostname.includes("127.0.0.1")
      ) {
        console.log(
          "[Runtime] Web environment detected (" +
            hostname +
            "). Forcing Firebase Mode.",
        );
        PREVIEW_MODE = false;
      ...
      runtimeWatchTimer = setInterval(function () {
        ...
        if (previewBootstrapped) {
          ...
          try {
            ...
            console.warn(
              "Unable to clear cached dashboard payload after reconnect:",
              err,
            );
          }
          showToast(
            "info",
            "Connected",
            "Live data connection restored. Loading polls…",
            3500,
          );
        }
        loadInitialData();
      ...
      runtimeFallbackTimer = setTimeout(function () {
        ...
        console.warn(
          "Apps Script runtime not detected after " +
            RUNTIME_MAX_WAIT_MS +
            "ms – showing cached preview data.",
        );
        PREVIEW_MODE = true;
        ...
        showToast(
          "warning",
          "Offline preview",
          "Live backend unavailable. Displaying cached sample data.",
          5000,
        );
      }, RUNTIME_MAX_WAIT_MS);
    }
    ...
    function previewOnlyNotice(actionLabel) {
      ...
      return veritasAlert(
        "This feature is disabled in preview mode. Open the deployed Veritas dashboard to " +
          actionText +
          ".",
        {
          title: "Preview mode",
    ...
    function createVeritasModal() {
      ...
      var confirmLabel = confirmBtn.querySelector(
        "span:not(.veritas-modal-spinner)",
      );
      var active = false;
      ...
      root.addEventListener("click", function (event) {
        if (
          event.target === root ||
          event.target.classList.contains("veritas-modal-backdrop")
        ) {
      ...
      return {
        alert: function (options) {
          return openModal("alert", options).then(function () {
            return;
          });
        },
        confirm: function (options) {
          return openModal("confirm", options).then(function (result) {
            return result !== false;
          });
        },
        prompt: function (options) {
          return openModal("prompt", options).then(function (result) {
            return result;
          });
        },
      };
    ...
    function updateDashboardSidebarToggleState() {
      ...
      var ariaLabel = dashboardSidebarVisible
        ? "Hide navigation"
        : "Show navigation";
      dashboardSidebarToggleButtons.forEach(function (btn) {
        ...
        btn.setAttribute(
          "aria-expanded",
          dashboardSidebarVisible ? "true" : "false",
        );
      });
    }
    ...
    function buttonSpinnerMarkup(text) {
      return (
        '<span class="flex w-full items-center justify-center gap-2"><span class="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent"></span><span>' +
    ...
    function onGoogleChartsReady() {
      ...
      if (
        typeof renderAnalyticsByTab === "function" &&
    ...
    var headerSessionControls = document.getElementById(
    ...
    var sessionToggleButtons = document.querySelectorAll(
      ".builder-session-toggle",
    );
    var secureSettingsPanel = document.getElementById(
      "secure-assessment-settings",
    );
    var secureSessionMeta = document.getElementById("secure-session-meta");
    var secureTimeLimitInput = document.getElementById(
      "secure-time-limit-input",
    );
    var secureAccessCodeInput = document.getElementById(
      "secure-access-code-input",
    );
    var secureAvailableInput = document.getElementById(
      "secure-available-input",
    );
    ...
    var studentManagerStatus = document.getElementById(
      "student-manager-status",
    );
    var studentManagerProgress = document.getElementById(
      "student-manager-progress",
    );
    ...
    var studentManagerConnection = document.getElementById(
      "student-manager-connection",
    );
    var studentManagerConnectionDot = document.getElementById(
      "student-manager-connection-dot",
    );
    var studentManagerMessage = document.getElementById(
      "student-manager-message",
    );
    var studentManagerLoading = document.getElementById(
      "student-manager-loading",
    );
    ...
    var studentManagerUnlock = document.getElementById(
      "student-manager-unlock",
    );
    var studentManagerForceSubmit = document.getElementById(
      "student-manager-force-submit",
    );
    ...
    var questionSortResetBtn = document.getElementById(
      "question-sort-reset-btn",
    );
    var questionSortHelp = document.getElementById("question-sort-help");
    var questionNavigatorContainer =
      document.getElementById("question-navigator");
    var archivedClassFilterSelect = document.getElementById(
      "archived-class-filter",
    );
    ...
    var archivedQuestionCount = document.getElementById(
      "archived-question-count",
    );
    var archivedResponseCount = document.getElementById(
      "archived-response-count",
    );
    var archivedQuestionsContainer = document.getElementById(
      "archived-questions-container",
    );
    ...
    var emulatorStudentCount = document.getElementById(
      "emulator-student-count",
    );
    ...
    if (
      ...
      console.warn(
        "Google Charts unavailable – continuing initialization without charts.",
      );
    ...
    bindClick("nav-create-poll", function () {
      console.log(
        "Create New Poll button clicked - opening wizard via Wizard.init()",
      );
      Wizard.init();
    });
    ...
    if (questionNavigatorContainer) {
      questionNavigatorContainer.addEventListener(
        "dragover",
        handleNavigatorDragOver,
      );
    ...
    function initEmulatorControls() {
      ...
      if (!emulatorControls || !emulatorEnabled) {
        console.warn(
          "[Emulator] DOM elements not found - emulator controls will not be available",
        );
        return;
      }
      ...
      emulatorAvailable = !!(
        ...
        FIREBASE_CONFIG.projectId.includes("emulator")
      );
    ...
    function triggerLivePollEmulation(pollId) {
      ...
      var studentCount = emulatorStudentCount
        ? parseInt(emulatorStudentCount.value, 10) || 5
        : 5;
      var concurrent = emulatorConcurrent ? emulatorConcurrent.checked : false;

      console.log("[Emulator] Triggering live poll emulation", {
        pollId: pollId,
      ...
      showToast(
        "info",
        "Emulator",
        "Starting emulation with " + studentCount + " students...",
      );

      // FIREBASE MIGRATION: Call Cloud Function for emulation
      firebase
        .functions()
        .httpsCallable("manageProctoring")({
          action: "EMULATE_LIVE",
          pollId: pollId,
          studentCount: studentCount,
          concurrent: concurrent,
        ...
        .then(function (result) {
          ...
          if (res && res.success) {
            showToast(
              ...
              "Emulator",
              "Emulation complete: " +
                res.successfulSubmissions +
                "/" +
                res.totalSubmissions,
            );
          } else {
    ...
    function triggerSecureAssessmentEmulation(pollId, sessionId) {
      ...
      console.log("[Emulator] Triggering secure assessment emulation", {
        pollId: pollId,
        sessionId: sessionId,
        studentCount: studentCount,
      });
    ...
    var statusFilterSelect = document.getElementById(
    ...
    var correctnessFilterSelect = document.getElementById(
      "student-correctness-filter-select",
    );
    ...
    if (studentSortSelectEl) {
      studentSortSelectEl.addEventListener("change", function () {
        ...
        if (this.value === "recent" && studentTableSort.direction === "asc") {
          ...
          var dirIcon = document.querySelector(
            "#student-sort-direction-btn span",
          );
          if (dirIcon) {
          ...
          if (icon) {
            icon.textContent =
              studentTableSort.direction === "asc" ? "south" : "north";
          }
        }
        renderStudentStatus();
      });
    ...
    var sortDirectionBtn = document.getElementById(
      "student-sort-direction-btn",
    );
    if (sortDirectionBtn) {
      sortDirectionBtn.addEventListener("click", function () {
        studentTableSort.direction =
          studentTableSort.direction === "asc" ? "desc" : "asc";
    ...
    if (pollSortSelect) {
      pollSortSelect.addEventListener("change", function () {
        pollSortMode = this.value;
        refreshPollViews();
      });
    } else {
    ...
    if (pollFilterSelect) {
      pollFilterSelect.addEventListener("change", function () {
        pollClassFilter = this.value;
        refreshPollViews();
      });
    } else {
    ...
    if (archivedClassFilterSelect) {
      archivedClassFilterSelect.addEventListener("change", function () {
        archivedClassFilter = this.value;
        renderArchivedList();
      });
    } else {
    ...
    function setMainSection(section) {
      function updateUI() {
        ...
        var individualTimedView = document.getElementById(
          "individual-timed-view",
        );
        if (individualTimedView) {
          individualTimedView.style.display =
            section === "individual-timed" ? "block" : "none";
        }
        var createPollView = document.getElementById("create-poll-view");
        if (createPollView) {
          createPollView.style.display =
            section === "create-poll" ? "flex" : "none";
        }

        var navButtons = document.querySelectorAll(
          ".sidebar-nav-btn[data-section-target]",
        );
    ...
    function updateMobileLayoutVisibility() {
      ...
      if (liveMobileNav) {
        liveMobileNav.style.display =
          isMobileViewport && currentMainSection === "live" ? "flex" : "none";
      }

      if (liveMobileActions) {
        liveMobileActions.style.display =
          isMobileViewport && currentMainSection === "live" ? "flex" : "none";
      }
    ...
    function setActiveMobilePanelForViewport(panel) {
      ...
      panels.forEach(function (panelEl) {
        var matches =
          panelEl.getAttribute("data-mobile-panel") === activeMobilePanel ||
          !isMobileViewport;
    ...
    function renderDashboardSummary() {
      ...
      var polls =
        dashboardSummary && dashboardSummary.polls
          ? dashboardSummary.polls
          : [];
    ...
    function renderRecentSessions() {
      ...
      if (
        ...
        container.innerHTML =
          '<div class="text-center py-6 text-gray-500 text-sm">No recent sessions found. Create your first poll to get started!</div>';
        return;
      }
      ...
      try {
        ...
        dashboardSummary.recentSessions.forEach(function (session) {
          ...
          item.innerHTML =
            ...
            '<p class="font-semibold text-sm text-veritas-navy">' +
            escapeHtml(session.sessionName || "Untitled") +
            "</p>" +
            ...
            "</span>" +
            "</div>" +
            '<span class="text-xs text-gray-500">Avg ' +
            Math.round(session.masteryPct || 0) +
            "%</span>" +
            "</div>";
        ...
        container.innerHTML =
          '<div class="text-center py-6 text-red-500 text-sm">Error loading recent sessions</div>';
      }
    }
    ...
    function renderActivityPulse() {
      ...
      if (
        ...
        dashboardSummary.dailyActivity.length === 0
      ) {
        chartContainer.innerHTML =
          '<div class="text-center py-6 text-gray-500 text-sm">No activity data available</div>';
      ...
      try {
        ...
        var maxCount = Math.max.apply(
          Math,
          dashboardSummary.dailyActivity.map(function (d) {
            return d.count || 0;
          }),
        );
        ...
        dashboardSummary.dailyActivity.forEach(function (day) {
          ...
          var heightPct =
            maxCount > 0 ? ((day.count || 0) / maxCount) * 100 : 0;
          bar.style.height = heightPct + "%";
          bar.title =
            (day.dayName || "Day") + ": " + (day.count || 0) + " interactions";
        ...
        summaryContainer.innerHTML =
          ...
          totalActivity +
          " total interactions</span>" +
          "</div>";
      } catch (e) {
        console.error("Error in renderActivityPulse:", e);
        chartContainer.innerHTML =
          '<div class="text-center py-6 text-red-500 text-sm">Error loading activity data</div>';
        summaryContainer.innerHTML = "";
      }
    }
    ...
    function loadInitialData() {
      ...
      if (PREVIEW_MODE && PREVIEW_PAYLOAD) {
        previewBootstrapped = true;
        console.info(
          "Apps Script runtime not detected – rendering preview data.",
        );
        setTimeout(function () {
      ...
      console.log(
        "[Firebase] loadInitialData: Relying on RTDB listeners for polls.",
      );

      // We still need to load Classes for the dropdowns
      var db = firebase.database();
      db.ref("rosters/classes")
        .once("value")
    ...
    function updatePollDropdown(polls) {
      ...
      polls.forEach(function (poll) {
        ...
        var questionTotal = Array.isArray(poll.questions)
          ? poll.questions.length
          : 0;
        var label =
          poll.pollName +
          " (" +
          (poll.className || "Unassigned") +
          ") - " +
          questionTotal +
          " questions";
    ...
    function refreshClassOptions(classList) {
      ...
      BASE_CLASSES.forEach(function (name) {
        classSet.add(name);
      });
      Object.keys(rosterData).forEach(function (name) {
    ...
    function updateClassFilters() {
      ...
      if (
        previousPollFilter &&
        ALL_CLASSES.indexOf(previousPollFilter) !== -1
      ) {
      ...
      archivedClassFilterSelect.innerHTML =
      ...
      if (
        previousArchivedFilter &&
        ALL_CLASSES.indexOf(previousArchivedFilter) !== -1
      ) {
    ...
    function pollMatchesSearch(poll, normalizedQuery) {
      ...
      var className = (
        poll && poll.className ? poll.className : ""
      ).toLowerCase();
      ...
      if (Array.isArray(poll.questions)) {
        for (var i = 0; i < poll.questions.length; i++) {
          ...
          if (correctText && correctText.indexOf(normalizedQuery) !== -1)
            return true;
      ...
      if (poll && Array.isArray(poll.liveSessions))
        sessionCollections.push(poll.liveSessions);
      if (poll && Array.isArray(poll.recentSessions))
        sessionCollections.push(poll.recentSessions);
      if (poll && Array.isArray(poll.sessions))
        sessionCollections.push(poll.sessions);
        ...
        for (var s = 0; s < collection.length; s++) {
          ...
          var sessionName = (
            typeof sessionNameRaw === "string"
              ? sessionNameRaw
              : String(sessionNameRaw || "")
          ).toLowerCase();
          if (sessionName.indexOf(normalizedQuery) !== -1) return true;
          var sessionNotesRaw = session.notes || session.summary || "";
          var sessionNotes = (
            typeof sessionNotesRaw === "string"
              ? sessionNotesRaw
              : String(sessionNotesRaw || "")
          ).toLowerCase();
          if (sessionNotes.indexOf(normalizedQuery) !== -1) return true;
          var sessionQuestions = Array.isArray(session.questions)
            ? session.questions
            : [];
          for (var q = 0; q < sessionQuestions.length; q++) {
            var sessionQuestion = sessionQuestions[q] || {};
            var sessionTextRaw =
              sessionQuestion.questionText || sessionQuestion.text || "";
            var sessionText = (
              typeof sessionTextRaw === "string"
                ? sessionTextRaw
                : String(sessionTextRaw || "")
            ).toLowerCase();
            if (sessionText.indexOf(normalizedQuery) !== -1) return true;
            var sessionOptions = Array.isArray(sessionQuestion.options)
              ? sessionQuestion.options
              : [];
            for (var so = 0; so < sessionOptions.length; so++) {
              var sessionOption = sessionOptions[so] || {};
              var sessionOptionRaw =
                sessionOption.text || sessionOption.label || "";
              var sessionOptionText = (
                typeof sessionOptionRaw === "string"
                  ? sessionOptionRaw
                  : String(sessionOptionRaw || "")
              ).toLowerCase();
              if (sessionOptionText.indexOf(normalizedQuery) !== -1)
                return true;
            }
          }
        }
    ...
    function getFilteredSortedPolls() {
      ...
      list.sort(function (a, b) {
        switch (pollSortMode) {
          ...
          case "class_asc":
            var classCompare = (a.className || "").localeCompare(
              b.className || "",
            );
    ...
    function getPollTimestamp(poll) {
      var value =
        poll && (poll.updatedAt || poll.lastRunAt || poll.createdAt || "");
      ...
      try {
        ...
        if (pollsCardView) {
          pollsCardView.innerHTML =
            '<div class="col-span-full text-center p-8 text-red-500 bg-red-50 rounded-lg border border-red-200">System Error: Unable to render polls. Please refresh or contact support.</div>';
        }
      }
      try {
        renderPollTable(polls);
        ...
        if (polls.length === 0) {
          ...
          if (pollSearchQuery) {
            ...
            message =
              "No polls match “" +
              safeQuery +
              "”. Try a different title or question keyword.";
          }
          ...
          if (pollSearchQuery) {
            ...
            if (emptyStateParagraph)
              emptyStateParagraph.textContent =
                'Your search for "' +
                safeQuery +
                '" did not return any results. Try different keywords.';
          } else {
            if (emptyStateTitle)
              emptyStateTitle.textContent = "Welcome to Your Poll Library";
            if (emptyStateParagraph)
              emptyStateParagraph.textContent =
                "This is where your polls will live. Create your first interactive poll to engage your students and gather real-time feedback.";
          }
        } else {
    ...
    function renderPollCards(polls) {
      ...
      polls.forEach(function (poll) {
        try {
          ...
          var safeName = escapeHtml(
            poll && poll.pollName ? poll.pollName : "Untitled Poll",
          );
          card.setAttribute("aria-label", "Poll: " + safeName);

          var className = escapeHtml(
            poll && poll.className ? poll.className : "Unassigned",
          );
          var questionCount = Array.isArray(poll && poll.questions)
            ? poll.questions.length
            : 0;
          var lastEdited = formatDateTime(
            poll.updatedAt || poll.lastRunAt || poll.createdAt,
          );
          ...
          htmlParts.push(
            '<h3 class="text-lg font-bold text-veritas-navy dark:text-white">' +
              safeName +
              "</h3>",
          );
          htmlParts.push("</div>");
          ...
          if (sessionType === "SECURE_ASSESSMENT") {
            ...
            if (secureTimeLabel) {
              htmlParts.push(
                '<div class="flex items-center gap-2 mb-2"><span class="material-symbols-outlined text-base">timer</span><span>' +
                  escapeHtml(secureTimeLabel) +
                  "</span></div>",
              );
            }
            htmlParts.push(
              '<div class="flex items-center gap-2 mb-2"><span class="material-symbols-outlined text-base">key</span><span>Access Code: ',
            );
            if (secureAccessCode) {
              htmlParts.push(
                '<span class="font-semibold tracking-widest">' +
                  escapeHtml(secureAccessCode) +
                  "</span>",
              );
            } else {
            ...
            if (secureWindowLabel) {
              htmlParts.push(
                '<div class="flex items-center gap-2"><span class="material-symbols-outlined text-base">calendar_month</span><span>' +
                  escapeHtml(secureWindowLabel) +
                  "</span></div>",
              );
            }
            htmlParts.push("</div>");
          ...
          // Stats
          htmlParts.push(
            '<div class="text-xs text-gray-500 dark:text-gray-400 mb-4">Last updated: ' +
              lastEdited +
              "</div>",
          );
          ...
          htmlParts.push(
            '<button type="button" class="flex-1 poll-card-btn primary poll-preview-btn" data-poll-id="' +
              (poll && poll.pollId ? poll.pollId : "") +
              '" aria-label="Preview ' +
              safeName +
              '"><span class="material-symbols-outlined text-base">visibility</span>Preview</button>',
          );
          htmlParts.push(
            '<button type="button" class="poll-card-btn secondary poll-edit-btn" data-poll-id="' +
              (poll && poll.pollId ? poll.pollId : "") +
              '" aria-label="Edit ' +
              safeName +
          ...
          htmlParts.push(
            '<button type="button" class="poll-card-btn info poll-copy-btn" data-poll-id="' +
              (poll && poll.pollId ? poll.pollId : "") +
              '" aria-label="Duplicate ' +
              safeName +
              '"><span class="material-symbols-outlined text-base">content_copy</span></button>',
          );
          htmlParts.push(
            '<button type="button" class="poll-card-btn danger poll-delete-btn" data-poll-id="' +
              (poll && poll.pollId ? poll.pollId : "") +
              '" aria-label="Delete ' +
              safeName +
          ...
          console.error(
            "Error rendering individual poll card:",
            cardError,
            poll,
          );
          renderErrors++;
      ...
      if (renderErrors > 0 && pollsCardView.children.length === 0) {
        ...
        pollsCardView.innerHTML =
          '<div class="col-span-full text-center p-8 text-red-500 bg-red-50 rounded-lg border border-red-200">Unable to load polls due to a data error. Please contact support.</div>';
      }
    ...
    function describeSecureAvailability(poll) {
      ...
      if (available && due) {
        return (
          "Window: " + formatDateTime(available) + " → " + formatDateTime(due)
        );
      }
      if (available) {
    ...
    function buildSessionModeCellHtml(poll) {
      ...
      if (sessionType === "SECURE_ASSESSMENT") {
        ...
        if (code) {
          html +=
            '<span class="text-xs text-rose-700 dark:text-rose-200">Code: <span class="font-semibold tracking-widest">' +
            escapeHtml(code) +
            "</span></span>";
        } else {
          html +=
            '<span class="text-xs text-rose-700/80 dark:text-rose-200/80">No access code</span>';
        }
      }
      html += "</div>";
      return html;
    ...
    function togglePollDropdown(btn, pollId) {
      ...
      menu.className =
        "fixed z-[9999] w-52 rounded-xl border border-white/5 bg-gray-800 p-1 text-sm text-white shadow-lg transition duration-100 ease-out origin-top-right";
      ...
      items.forEach(function (item) {
        ...
        btnItem.className =
          "group flex w-full items-center gap-2 rounded-lg px-3 py-1.5 hover:bg-white/10 text-left transition-colors " +
          (item.danger ? "text-red-400 hover:text-red-300" : "text-white");

        var iconColorClass = item.danger
    ...
    function renderPollTable(polls) {
      ...
      polls.forEach(function (poll) {
        try {
          ...
          row.className =
            "hover:bg-brand-light-gray/40 dark:hover:bg-brand-dark-gray/40 transition-colors";
          var lastEdited = formatDateTime(
            poll.updatedAt || poll.lastRunAt || poll.createdAt,
          );
          var createdLabel = poll.createdAt
            ? formatDateTime(poll.createdAt)
            : "—";
          var htmlParts = [];
          htmlParts.push('<td class="px-4 py-3 align-top">');
          htmlParts.push(
            '<div class="font-semibold text-brand-dark-gray dark:text-brand-white"><span class="poll-name-link cursor-pointer hover:text-primary dark:hover:text-primary transition-colors" data-poll-id="' +
              poll.pollId +
              '">' +
          ...
          htmlParts.push(
            '<div class="text-xs text-brand-dark-gray/60 dark:text-brand-white/50">Created ' +
              createdLabel +
              "</div>",
          );
          htmlParts.push("</td>");
          htmlParts.push(
            '<td class="px-4 py-3 align-top text-brand-dark-gray/80 dark:text-brand-white/70">' +
              escapeHtml(poll.className || "—") +
              "</td>",
          );
          htmlParts.push(
            '<td class="px-4 py-3 align-top text-brand-dark-gray/80 dark:text-brand-white/70">' +
          ...
          htmlParts.push(
            '<button class="poll-select-btn h-8 px-3 rounded-md bg-primary/10 text-primary dark:text-brand-white dark:bg-brand-white/10 text-xs font-semibold hover:bg-primary/20 dark:hover:bg-brand-white/20 transition-colors" data-poll-id="' +
              poll.pollId +
              '">Select</button>',
          );
          ...
          htmlParts.push(
            '<button type="button" class="poll-options-btn inline-flex items-center gap-2 rounded-md bg-gray-800 px-3 py-1.5 text-xs font-semibold text-white shadow-inner shadow-white/10 hover:bg-gray-700 focus:outline-none transition-colors" data-poll-id="' +
              poll.pollId +
    ...
    async function handleCopyPoll(pollId) {
      var poll = ALL_POLLS.find(function (p) {
        return p.pollId === pollId;
      });
      if (!poll) return;
      ...
      if (PREVIEW_MODE) {
        setTimeout(async function () {
          ...
          refreshPollViews();
          await veritasAlert("Poll copied (preview data only).", {
            title: "Poll copied",
          });
        }, 140);
        return;
    ...
    async function handleDeletePoll(pollId) {
      var confirmed = await veritasConfirm(
        "Delete this poll permanently? This will also remove all responses.",
        {
          title: "Delete poll",
          confirmText: "Delete",
          destructive: true,
      ...
      if (PREVIEW_MODE) {
        setTimeout(function () {
          ALL_POLLS = ALL_POLLS.filter(function (p) {
            return p.pollId !== pollId;
          });
          refreshPollViews();
          veritasAlert("Poll deleted (preview data only).", {
            title: "Delete poll",
          });
        }, 120);
        return;
    ...
    function onPollSelectChange() {
      ...
      var selectedPoll = pollId
        ? ALL_POLLS.find(function (p) {
            return p.pollId === pollId;
          })
        : null;
      var className =
        selectedPoll && selectedPoll.className ? selectedPoll.className : "";
    ...
    function updateSessionLaunchControls(selectedPoll) {
      ...
      var isSecure = selectedPoll
        ? isSecureSessionTypeClient(selectedPoll.sessionType)
        : false;
      ...
      startPollBtn.classList.remove(
        "bg-veritas-gold",
        "hover:bg-veritas-gold/90",
        "bg-rose-600",
        "hover:bg-rose-700",
      );
      if (isSecure) {
    ...
    function updateSecureSessionMeta(selectedPoll) {
      ...
      if (timeLabel) {
        metaItems.push(
          '<span class="flex items-center gap-1.5"><span class="material-symbols-outlined text-sm opacity-80">timer</span><span>' +
            escapeHtml(timeLabel) +
            "</span></span>",
        );
      }
      ...
      if (accessCode) {
        metaItems.push(
          '<span class="flex items-center gap-1.5"><span class="material-symbols-outlined text-sm opacity-80">key</span><span>Code: <span class="font-semibold tracking-widest">' +
            escapeHtml(accessCode) +
            "</span></span></span>",
        );
      } else {
        metaItems.push(
          '<span class="flex items-center gap-1.5"><span class="material-symbols-outlined text-sm opacity-80">key</span><span class="opacity-70">No access code</span></span>',
        );
      }

      var calculatorEnabled =
        selectedPoll.calculatorEnabled === true ||
        (selectedPoll.secureSettings &&
          selectedPoll.secureSettings.calculatorEnabled === true);
      metaItems.push(
        '<span class="flex items-center gap-1.5"><span class="material-symbols-outlined text-sm opacity-80">calculate</span><span>' +
          (calculatorEnabled ? "Calculator enabled" : "Calculator off") +
          "</span></span>",
      );
      ...
      if (windowLabel) {
        metaItems.push(
          '<span class="flex items-center gap-1.5"><span class="material-symbols-outlined text-sm opacity-80">calendar_month</span><span>' +
      ...
      secureSessionMeta.innerHTML = metaItems.join(
    ...
    function renderRosterClassList() {
      ...
      if (ALL_CLASSES.length === 0) {
        rosterClassList.innerHTML =
          '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">No classes yet. Click the + button to add one.</p>';
        return;
      }
      ...
      ALL_CLASSES.forEach(function (className) {
        ...
        button.className =
          ...
          (isActive
            ? "bg-primary text-brand-white shadow-sm"
            : "bg-brand-light-gray/40 dark:bg-brand-dark-gray/30 text-brand-dark-gray dark:text-brand-white hover:bg-brand-light-gray/60 dark:hover:bg-brand-dark-gray/50");
    ...
    function renderRosterTable() {
      ...
      entries.forEach(function (entry, index) {
        ...
        row.innerHTML =
          ...
          '<input type="text" class="roster-name-input w-full rounded-lg border border-brand-light-gray dark:border-brand-dark-gray/50 bg-background-light dark:bg-brand-dark-gray/30 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" data-index="' +
          index +
          ...
          "</td>" +
          '<td class="px-4 py-2 align-top">' +
          '<input type="email" class="roster-email-input w-full rounded-lg border border-brand-light-gray dark:border-brand-dark-gray/50 bg-background-light dark:bg-brand-dark-gray/30 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" data-index="' +
          index +
          ...
          '<button class="remove-roster-row-btn h-8 w-8 rounded-full bg-red-600/10 text-red-600 dark:text-red-300 dark:bg-red-500/20 hover:bg-red-600/20 dark:hover:bg-red-500/30 transition-colors" data-index="' +
          index +
          '" title="Remove">' +
      ...
      rosterTableBody
        .querySelectorAll(".roster-name-input")
      ...
      rosterTableBody
        .querySelectorAll(".roster-email-input")
        .forEach(function (input) {
          input.oninput = function () {
            var idx = parseInt(this.dataset.index, 10);
      ...
      rosterTableBody
        .querySelectorAll(".remove-roster-row-btn")
          ...
          onRosterDataLoaded(
            {
              ...
              classes:
                ...
                window.currentClasses.includes(selectedRosterClass)
                  ? window.currentClasses
    ...
    async function confirmBulkAddStudents() {
      ...
      if (entries.length === 0) {
        await veritasAlert(
          "No valid student entries found. Use format: Name, Email",
      ...
      if (PREVIEW_MODE) {
        setTimeout(async function () {
          ...
          rosterDirty = false;
          renderRosterTable();
          await veritasAlert("Students added (preview data only).", {
            title: "Bulk add students",
          });
        }, 160);
        return;
      ...
      rostersRef
        ...
        .then(function () {
          ...
          showToast(
            "success",
            "Students added",
            "Successfully added " + entries.length + " students.",
          );
        })
        .catch(function (error) {
          confirmBtn.disabled = false;
    ...
    async function onAddClass() {
      var className = await veritasPrompt("Class name", {
        title: "Add class",
        confirmText: "Create",
      });
      if (!className) return;
    ...
    async function onRenameClass() {
      ...
      var newName = await veritasPrompt("Rename class", {
        title: "Rename class",
        defaultValue: selectedRosterClass,
        confirmText: "Rename",
      });
      if (!newName) return;
      newName = newName.trim();
        ...
        .then(function (result) {
          if (result.data && result.data.success) {
            ...
            if (result.data.pollsUpdated > 0) {
              rosterStatus.textContent +=
                " (" +
                result.data.pollsUpdated +
                " poll" +
                (result.data.pollsUpdated === 1 ? "" : "s") +
                " updated)";
            }
            selectedRosterClass = newName;
            loadRosterData();
    ...
    async function onDeleteClass() {
      ...
      var confirmed = await veritasConfirm(
        'Delete class "' + selectedRosterClass + '" and remove all students?',
        {
          title: "Delete class",
          confirmText: "Delete",
          destructive: true,
    ...
    function loadArchivedPolls(force) {
      ...
      archivedList.innerHTML =
        '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">Loading archived polls...</p>';
      ...
      historyRef
        ...
        .then(function (snapshot) {
          ...
          if (archivedPolls.length > 0) {
            if (
              !archivedPolls.some(function (p) {
                return p.pollId === selectedArchivedPollId;
              })
            ) {
              selectedArchivedPollId = archivedPolls[0].pollId;
            }
    ...
    function renderArchivedList() {
      ...
      var filtered =
        archivedClassFilter === "all"
          ? archivedPolls
          : archivedPolls.filter(function (poll) {
              return poll.className === archivedClassFilter;
            });

      if (filtered.length === 0) {
        archivedList.innerHTML =
          '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">No archived polls for this filter.</p>';
        archivedEmptyState.classList.remove("hidden");
        archivedDetail.classList.add("hidden");
        return;
      }
      ...
      filtered.forEach(function (poll) {
        ...
        var metaText =
          (poll.className || "Unassigned") +
          " • " +
          formatDateTime(poll.lastRunAt || poll.updatedAt || poll.createdAt);
        button.innerHTML =
          '<div class="font-semibold">' +
          escapeHtml(poll.pollName || "Untitled Poll") +
          '</div><div class="text-xs opacity-80">' +
          escapeHtml(metaText) +
          "</div>";
        button.onclick = function () {
    ...
    function renderArchivedDetail() {
      var poll = archivedPolls.find(function (p) {
        return p.pollId === selectedArchivedPollId;
      });
      if (!poll) {
        archivedEmptyState.classList.remove("hidden");
      ...
      archivedPollMeta.textContent =
        (poll.className || "Unassigned") +
        " • Last run " +
        formatDateTime(poll.lastRunAt || poll.updatedAt || poll.createdAt);
      archivedQuestionCount.textContent = poll.questionCount || 0;
      archivedResponseCount.textContent =
        (poll.totalResponses || 0) + " of " + (poll.totalStudents || 0);

      archivedQuestionsContainer.innerHTML = "";
      poll.questions.forEach(function (question, index) {
        var card = document.createElement("div");
        card.className =
          "border border-brand-light-gray dark:border-brand-dark-gray/40 rounded-lg p-4 bg-brand-white dark:bg-brand-dark-gray/40";
        var summary = question.summary || {
          correct: 0,
          incorrect: 0,
          noResponse: 0,
          violations: 0,
        };
        var responses = Array.isArray(question.responses)
          ? question.responses
          : [];
        var nonResponders = Array.isArray(question.nonResponders)
        ...
        if (responses.length === 0) {
          responsesHtml =
            '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">No responses recorded.</p>';
        } else {
          responsesHtml =
            '<table class="min-w-full text-sm divide-y divide-brand-light-gray dark:divide-brand-dark-gray/40"><thead class="text-xs uppercase tracking-wide text-brand-dark-gray/60 dark:text-brand-white/50"><tr><th class="py-2 text-left">Student</th><th class="py-2 text-left">Answer</th><th class="py-2 text-left">Status</th></tr></thead><tbody class="divide-y divide-brand-light-gray/70 dark:divide-brand-dark-gray/40">';
          responses.forEach(function (response) {
            var status = response.violation
              ? "Violation"
              : response.isCorrect
                ? "Correct"
                : "Incorrect";
            var statusClass = response.violation
              ? "text-orange-600 dark:text-orange-300"
              : response.isCorrect
                ? "text-green-600 dark:text-green-300"
                : "text-red-600 dark:text-red-300";
            responsesHtml +=
              '<tr><td class="py-2 pr-3">' +
              escapeHtml(response.name || response.email) +
              '</td><td class="py-2 pr-3">' +
        ...
        if (nonResponders.length > 0) {
          nonResponderHtml =
            '<div class="mt-3"><h4 class="text-xs font-semibold uppercase text-brand-dark-gray/60 dark:text-brand-white/60">No Response</h4><div class="flex flex-wrap gap-2 mt-1">';
          nonResponders.forEach(function (student) {
            var chipClass = student.violation
              ? "bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-200"
              : "bg-brand-light-gray text-brand-dark-gray dark:bg-brand-dark-gray/50 dark:text-brand-white/80";
        ...
        var imageHtml = question.questionImageURL
          ? '<div class="mt-3"><img src="' +
            escapeHtml(question.questionImageURL) +
            '" class="max-h-48 rounded-lg object-contain" alt="Question image" /></div>'
          : "";
        var questionHtml =
          '<div><h3 class="font-serif text-lg text-brand-dark-gray dark:text-brand-white">Question ' +
          (index + 1) +
          '</h3><p class="text-sm text-brand-dark-gray/70 dark:text-brand-white/70 mt-1">' +
          escapeHtml(question.questionText || "") +
          "</p></div>";
        var summaryHtml =
          '<div class="text-sm text-brand-dark-gray/70 dark:text-brand-white/70 text-right">' +
      ...
      getAnalytics({ pollId: pollId })
        .then(function (result) {
          ...
          if (data.success) {
            ...
            loadingEl.innerHTML =
              '<div class="text-center py-12"><p class="text-red-600">Error: ' +
    ...
    function renderPostPollAnalytics(data) {
      ...
      // Update header
      document.getElementById("analytics-poll-name").textContent =
        data.pollName || "Poll Analysis";
      document.getElementById("analytics-poll-meta").textContent =
        data.className + " • " + data.questionCount + " Questions";
      ...
      if (data.metacognition && data.metacognition.enabled) {
        ...
        document
          .getElementById("metacognition-section")
    ...
    function renderTeacherActionItems(items) {
      ...
      var html =
        '<h4 class="text-lg font-bold text-brand-dark-gray dark:text-white mb-3 flex items-center gap-2">' +
    ...
    function renderAnalyticsClassOverview(overview, distribution) {
      ...
      html +=
        '<div class="text-sm text-blue-700 dark:text-blue-300 font-semibold mb-1">Mean Score</div>';
      html +=
      ...
      if (interp.meanScore) {
        html +=
          '<div class="mt-2 text-xs font-medium text-blue-800 dark:text-blue-200">' +
          escapeHtml(interp.meanScore.message) +
          "</div>";
      }
      html += "</div>";
      ...
      // Median
      html +=
        '<div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700/30 rounded-lg p-4">';
      html +=
        '<div class="text-sm text-green-700 dark:text-green-300 font-semibold mb-1">Median Score</div>';
      html +=
      ...
      // Std Dev
      html +=
        '<div class="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700/30 rounded-lg p-4">';
      html +=
        '<div class="text-sm text-purple-700 dark:text-purple-300 font-semibold mb-1">Std. Deviation</div>';
      html +=
      ...
      if (interp.stdDev) {
        html +=
          '<div class="mt-2 text-xs font-medium text-purple-800 dark:text-purple-200">' +
          escapeHtml(interp.stdDev.message) +
          "</div>";
      }
      html += "</div>";
      ...
      // Participation
      html +=
        '<div class="bg-gray-50 dark:bg-gray-900/20 border border-gray-200 dark:border-gray-700/30 rounded-lg p-4">';
      html +=
        '<div class="text-sm text-gray-700 dark:text-gray-300 font-semibold mb-1">Participants</div>';
      html +=
      ...
      if (interp.participation) {
        html +=
          '<div class="mt-2 text-xs font-medium text-gray-800 dark:text-gray-200">' +
          escapeHtml(interp.participation.message) +
          "</div>";
      }
      html += "</div>";
      html += "</div>";
      ...
      if (
        ...
        html +=
          '<div class="bg-white dark:bg-brand-dark-gray/40 border border-brand-light-gray dark:border-brand-dark-gray/40 rounded-lg p-4">';
        html +=
          '<h4 class="text-sm font-semibold text-brand-dark-gray dark:text-brand-white mb-3">Score Distribution</h4>';
        html += '<div class="flex items-end gap-2 h-48">';

        var maxCount = Math.max.apply(
          null,
          distribution.histogram.map(function (d) {
            return d.count;
          }),
        );
        distribution.histogram.forEach(function (item) {
          ...
          html +=
            ...
            "</div>";
          html +=
            '<div class="w-full bg-primary rounded-t" style="height: ' +
            heightPct +
            '%;"></div>';
          html +=
            '<div class="text-xs font-semibold text-brand-dark-gray dark:text-brand-white">' +
            item.score +
            "</div>";
          html += "</div>";
        });
    ...
    function renderMetacognitionAnalysis(metacognition) {
      ...
      if (!metacognition.overall) {
        container.innerHTML =
          '<p class="text-brand-dark-gray/60 dark:text-brand-white/60">No metacognition data available yet.</p>';
        return;
      }

      var html =
        '<div class="bg-white dark:bg-brand-dark-gray/40 border border-brand-light-gray dark:border-brand-dark-gray/40 rounded-lg p-6 mb-4">';
      html +=
        '<h4 class="text-lg font-semibold text-brand-dark-gray dark:text-brand-white mb-4">Confidence vs. Correctness Matrix</h4>';
      ...
      html +=
        '<div class="text-sm font-semibold text-green-700 dark:text-green-300">Conscious Competence</div>';
      html += "</div>";
      html +=
        '<div class="text-3xl font-bold text-green-900 dark:text-green-100">' +
        metacognition.overall.confidentCorrect +
        "%</div>";
      html +=
        '<div class="text-xs text-green-700 dark:text-green-300 mt-1">Confident & Correct (Mastery!)</div>';
      html += "</div>";
      ...
      html +=
        '<div class="text-sm font-semibold text-red-700 dark:text-red-300">Confidently Incorrect</div>';
      html += "</div>";
      html +=
        '<div class="text-3xl font-bold text-red-900 dark:text-red-100">' +
        metacognition.overall.confidentIncorrect +
        "%</div>";
      html +=
        '<div class="text-xs text-red-700 dark:text-red-300 mt-1">RED ALERT: Deep misconception!</div>';
      html += "</div>";

      // Uncertain & Correct (Lucky guess)
      html +=
        '<div class="bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-500 rounded-lg p-4">';
      html += '<div class="flex items-center gap-2 mb-2">';
      html += '<span class="text-2xl">🤔</span>';
      html +=
        '<div class="text-sm font-semibold text-yellow-700 dark:text-yellow-300">Imposter Syndrome</div>';
      html += "</div>";
      html +=
        '<div class="text-3xl font-bold text-yellow-900 dark:text-yellow-100">' +
        metacognition.overall.uncertainCorrect +
        "%</div>";
      html +=
        '<div class="text-xs text-yellow-700 dark:text-yellow-300 mt-1">Uncertain but Correct (Lucky?)</div>';
      html += "</div>";
      ...
      html += '<span class="text-2xl">🤷</span>';
      html +=
        '<div class="text-sm font-semibold text-blue-700 dark:text-blue-300">Conscious Incompetence</div>';
      html += "</div>";
      html +=
        '<div class="text-3xl font-bold text-blue-900 dark:text-blue-100">' +
        metacognition.overall.uncertainIncorrect +
        "%</div>";
      html +=
        '<div class="text-xs text-blue-700 dark:text-blue-300 mt-1">Know they don\'t know (Good!)</div>';
      html += "</div>";
    ...
    function renderItemAnalysis(items) {
      ...
      items.forEach(function (item, index) {
        var difficultyColor =
          item.difficultyPct > 90
            ? "green"
            : item.difficultyPct < 30
              ? "red"
              : "blue";
        var discColor =
        ...
        html +=
          '<div class="bg-white dark:bg-brand-dark-gray/40 border border-brand-light-gray dark:border-brand-dark-gray/40 rounded-lg p-5 mb-4">';
        html += '<div class="flex items-start justify-between gap-4 mb-4">';
        html += '<div class="flex-1">';
        html += '<div class="flex items-center gap-2">';
        html +=
          '<h4 class="font-semibold text-brand-dark-gray dark:text-brand-white">Question ' +
          (index + 1) +
          "</h4>";
        ...
        if (interp.overall) {
          var qBadgeColor =
            interp.overall.quality === "excellent"
              ? "bg-green-100 text-green-800"
              : interp.overall.quality === "flawed"
                ? "bg-red-100 text-red-800"
                : interp.overall.quality === "needs-adjustment"
          ...
          html +=
            ...
            qBadgeColor +
            '">' +
            escapeHtml(interp.overall.quality) +
            "</span>";
        }
        html += "</div>";
        ...
        html +=
          '<div class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60 mb-1">Difficulty</div>';
        html +=
        ...
        if (interp.difficulty) {
          html +=
            ...
            escapeHtml(interp.difficulty.level) +
            "</div>";
        }
        html += "</div>";
        html += '<div class="text-center">';
        html +=
          '<div class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60 mb-1">Discrimination</div>';
        html +=
          '<div class="text-2xl font-bold text-' +
          discColor +
          "-600 dark:text-" +
        ...
        if (interp.discrimination) {
          html +=
            '<div class="text-[10px] text-gray-500">' +
            escapeHtml(interp.discrimination.level) +
            "</div>";
        }
        html += "</div>";
        html += "</div>";
        ...
        if (interp.actionable && interp.actionable.length > 0) {
          html +=
            '<div class="mt-3 p-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/30 rounded text-sm text-amber-800 dark:text-amber-200">';
          html +=
            '<p class="font-semibold text-xs uppercase tracking-wide mb-1">Recommendation:</p>';
        ...
        if (item.distractorAnalysis && item.distractorAnalysis.length > 0) {
          html +=
            '<div class="bg-gray-50 dark:bg-brand-dark-gray/20 rounded-lg p-3">';
          html +=
            '<h5 class="text-xs font-semibold uppercase text-brand-dark-gray/60 dark:text-brand-white/60 mb-2">Distractor Analysis</h5>';
          html += '<div class="space-y-2">';

          item.distractorAnalysis.forEach(function (dist) {
            var qualityColor =
              dist.quality === "excellent" || dist.quality === "good"
                ? "green"
                : dist.quality === "problematic"
                  ? "red"
                  : "gray";
            ...
            html += '<div class="flex items-center gap-2">';
            html +=
              '<span class="font-semibold text-brand-dark-gray dark:text-brand-white">' +
              dist.optionLetter +
              "</span>";
            html +=
              '<span class="text-brand-dark-gray/70 dark:text-brand-white/70">' +
              escapeHtml(dist.option.substring(0, 50)) +
              (dist.option.length > 50 ? "..." : "") +
              "</span>";
            if (dist.isCorrect) {
              html +=
                '<span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800 dark:bg-green-500/20 dark:text-green-300">Correct</span>';
            }
            html += "</div>";
            html += '<div class="flex items-center gap-3">';
            html +=
              '<span class="text-brand-dark-gray/60 dark:text-brand-white/60">' +
              dist.totalPct +
              "%</span>";
            html +=
    ...
    function showAnalyticsView() {
      ...
      var analyticsClassFilter = document.getElementById(
        "analytics-class-filter",
      );
      analyticsClassFilter.innerHTML =
        '<option value="all">All Classes</option>';
      var classes = Array.from(
        new Set(
          ALL_POLLS.map(function (p) {
            return p.className;
          }),
        ),
      );
    ...
    function loadAnalytics(filters) {
      ...
      // Show loading states
      document
        .getElementById("analytics-overview-loading")
        .classList.remove("hidden");
      document
        .getElementById("analytics-overview-content")
        .classList.add("hidden");
      document
        .getElementById("analytics-items-loading")
        .classList.remove("hidden");
      document
        .getElementById("analytics-items-content")
        .classList.add("hidden");
      document
        .getElementById("analytics-students-loading")
        .classList.remove("hidden");
      document
        .getElementById("analytics-students-content")
        .classList.add("hidden");
      ...
      getAnalytics(analyticsFilters)
        ...
        .catch(function (error) {
          ...
          // Show error states
          document.getElementById("analytics-overview-loading").innerHTML =
            '<p class="text-red-600">Error loading analytics: ' +
            escapeHtml(error.message || "Unknown error") +
            "</p>";
          document.getElementById("analytics-items-loading").innerHTML =
            '<p class="text-red-600">Error loading analytics: ' +
            escapeHtml(error.message || "Unknown error") +
            "</p>";
          document.getElementById("analytics-students-loading").innerHTML =
            '<p class="text-red-600">Error loading analytics: ' +
            escapeHtml(error.message || "Unknown error") +
            "</p>";
        });
    ...
    function renderAnalyticsOverview() {
      ...
      (analyticsData.kpis || []).forEach(function (kpi) {
        var card = document.createElement("div");
        card.className =
          "bg-brand-white dark:bg-brand-dark-gray/40 rounded-xl p-5 border border-brand-light-gray dark:border-brand-dark-gray/40 shadow-sm hover:shadow-md transition-shadow cursor-pointer";
        ...
        card.innerHTML =
          '<div class="text-xs font-semibold uppercase tracking-wide text-brand-dark-gray/60 dark:text-brand-white/60 mb-2">' +
      ...
      if (analyticsData.topics && analyticsData.topics.length > 0) {
        ...
        analyticsData.topics.forEach(function (topic) {
          ...
          topicHtml +=
            '<div class="text-sm font-semibold text-brand-dark-gray dark:text-brand-white min-w-[120px]">' +
            escapeHtml(topic.topic) +
            "</div>";
          topicHtml +=
            '<div class="flex-1 bg-brand-light-gray dark:bg-brand-dark-gray/30 rounded-full h-6 overflow-hidden">';
          topicHtml +=
            '<div class="' +
            color +
            ' h-full flex items-center justify-end px-2 text-xs text-white font-semibold" style="width: ' +
            topic.masteryPct +
            '%">';
          if (topic.masteryPct > 15) topicHtml += topic.masteryPct + "%";
          topicHtml += "</div></div>";
          topicHtml +=
            '<div class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60 min-w-[60px] text-right">' +
            topic.questionCount +
            " items</div>";
          topicHtml += "</div>";
        });
        ...
        topicHeat.innerHTML =
          '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">No topic data available. Add topic tags to your questions.</p>';
      }
      ...
      if (!analyticsData.items || analyticsData.items.length === 0) {
        ...
        typeof google.visualization.ScatterChart === "function"
      ) {
        var scatterData = [
          ["Difficulty (% Correct)", "Discrimination", "Question"],
        ];
        ...
        var options = {
          ...
          hAxis: {
            title: "Difficulty (% Correct)",
            minValue: 0,
            maxValue: 100,
          },
          vAxis: {
            title: "Discrimination (r_bis)",
            minValue: -0.5,
            maxValue: 1.0,
          },
          legend: "none",
        ...
        scatterHost.innerHTML =
          '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">Preparing analytics visuals…</p>';
      }
    }
    ...
    function renderAnalyticsItems() {
      ...
      items.forEach(function (item) {
        var row = document.createElement("tr");
        row.className =
          "hover:bg-brand-light-gray/20 dark:hover:bg-brand-dark-gray/30 cursor-pointer";
        ...
        correctCell.innerHTML =
          '<span class="' + correctColor + '">' + item.correctPct + "%</span>";
        ...
        var discColor =
          item.rbis >= 0.3
            ...
            : item.rbis >= 0.15
              ...
              : "text-red-600 dark:text-red-400";
        discCell.innerHTML =
          '<span class="' + discColor + '">' + item.rbis.toFixed(2) + "</span>";
    ...
    function renderChoiceBar(item) {
      ...
      choices.forEach(function (choice, idx) {
        ...
        var borderStyle = isCorrect
          ? "border-2 border-brand-dark-gray dark:border-brand-white"
          : "";
    ...
    function showStudentInsightsError(message) {
      ...
      if (container) {
        container.innerHTML =
          '<div class="flex items-center justify-center p-12 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/30 rounded-lg">' +
    ...
    function loadStudentInsights() {
      ...
      var className =
        analyticsFilters && analyticsFilters.className
          ? analyticsFilters.className
          : "all";
      var dateFrom =
        analyticsFilters && analyticsFilters.dateFrom
          ? analyticsFilters.dateFrom
          : null;
      var dateTo =
        analyticsFilters && analyticsFilters.dateTo
      ...
      getAnalytics({ className: className }) // Simplified for now
        .then(function (result) {
          ...
          if (data && data.success) {
            ...
            var errorMsg =
              data && data.error ? data.error : "Failed to load insights";
    ...
    function renderStudentInsights() {
      ...
      document.getElementById("struggling-count").textContent =
        stats.strugglingCount || 0;
      document.getElementById("non-responder-count").textContent =
        stats.nonResponderCount || 0;
      document.getElementById("rule-violator-count").textContent =
        stats.ruleViolatorCount || 0;
      document.getElementById("high-performer-count").textContent =
        stats.highPerformerCount || 0;
    ...
    function renderStudentInsightsTable() {
      ...
      displayStudents.forEach(function (student) {
        ...
        var participationColor =
          student.participationRate >= 90
            ? "text-green-600 dark:text-green-400"
            : student.participationRate >= 75
              ? "text-blue-600 dark:text-blue-400"
        ...
        student.flags.forEach(function (flag) {
          ...
          flagsHtml +=
            '<span class="' +
            flagClass +
            ' px-2 py-0.5 rounded text-xs font-semibold mr-1">' +
            flagText +
            "</span>";
        });

        html +=
          '<tr class="border-t border-brand-light-gray dark:border-brand-dark-gray/40 hover:bg-brand-light-gray/20 dark:hover:bg-brand-dark-gray/30 transition-colors">';
        html +=
          '<td class="px-4 py-3 text-sm font-medium text-brand-dark-gray dark:text-brand-white">' +
          escapeHtml(formatStudentName(student)) +
          "</td>";
        html +=
          '<td class="px-4 py-3 text-center"><span class="text-sm font-semibold ' +
          accuracyColor +
          '">' +
          ...
          "/" +
          (student.correctAnswers + student.incorrectAnswers) +
          "</span></td>";
        html +=
          '<td class="px-4 py-3 text-center"><span class="text-sm font-semibold ' +
          participationColor +
          '">' +
          Math.round(student.participationRate) +
          '%</span><br><span class="text-xs text-brand-dark-gray/50 dark:text-brand-white/50">' +
      ...
      tbody.innerHTML =
        html ||
        '<tr><td colspan="5" class="px-4 py-8 text-center text-brand-dark-gray/60 dark:text-brand-white/60">No students found</td></tr>';
    }
    ...
    function filterStudentsByFlag(flag) {
      ...
      filteredStudentInsights = currentStudentInsights.students.filter(
        function (s) {
    ...
    function clearStudentFilter() {
      ...
      document
        .getElementById("student-insights-filter-badge")
        .classList.add("hidden");
      document
        .getElementById("clear-student-filter-btn")
        .classList.add("hidden");
    ...
    function selectStudent(student) {
      ...
      document
        .getElementById("analytics-student-empty")
        .classList.add("hidden");
      document
        .getElementById("analytics-student-detail")
        .classList.remove("hidden");
      ...
      summary.innerHTML =
        ...
        '<div><h3 class="text-xl font-bold text-brand-dark-gray dark:text-brand-white">' +
        escapeHtml(formatStudentName(student)) +
        "</h3>" +
      ...
      if (sessions.length > 0) {
        ...
        sessions
          ...
          .forEach(function (session) {
            var color =
              session.scorePct >= 70
                ...
                : session.scorePct >= 50
                  ...
                  : "bg-red-500";
            sparkHtml +=
              '<div class="flex-1 flex flex-col items-center gap-1" title="' +
            ...
            sparkHtml +=
              ...
              color +
              '" style="height: ' +
              session.scorePct * 0.8 +
              'px; min-height: 20px;"></div>';
            sparkHtml +=
              '<div class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60">' +
              session.scorePct.toFixed(0) +
              "%</div>";
            sparkHtml += "</div>";
          });
        sparkHtml += "</div>";
        ...
        sparkline.innerHTML =
          '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">No session data</p>';
      }
      ...
      if (topics.length > 0) {
        ...
        topics.forEach(function (topic) {
          ...
          var pct =
            perf.total > 0 ? ((perf.correct / perf.total) * 100).toFixed(0) : 0;
          var color =
            pct >= 70
              ? "bg-green-500"
              : pct >= 50
          ...
          topicsHtml +=
            '<div class="text-sm font-semibold text-brand-dark-gray dark:text-brand-white min-w-[100px]">' +
            escapeHtml(topic) +
            "</div>";
          topicsHtml +=
            '<div class="flex-1 bg-brand-light-gray dark:bg-brand-dark-gray/30 rounded-full h-5 overflow-hidden">';
          topicsHtml +=
            '<div class="' +
            color +
            ' h-full flex items-center justify-end px-2 text-xs text-white font-semibold" style="width: ' +
        ...
        topicsDiv.innerHTML =
          '<p class="text-sm text-brand-dark-gray/60 dark:text-brand-white/60">No topic data</p>';
      }
    }

    // --- Analytics Event Listeners ---
    document.getElementById("analytics-apply-filters-btn").onclick =
      function () {
    ...
    document.getElementById("analytics-clear-filters-btn").onclick =
      function () {
    ...
    document.getElementById("analytics-export-btn").onclick = function () {
      ...
      var csv =
        "Session Name,Class,Date,Questions,Participants,Total Students,Mastery %,Participation %,Median Time (s),Integrity Rate\n";
      ...
      link.setAttribute(
        "download",
        "veritas-analytics-" + new Date().toISOString().split("T")[0] + ".csv",
      );
      ...
      tab.onclick = function () {
        ...
        document.querySelectorAll(".analytics-tab").forEach(function (t) {
          ...
          t.classList.add(
            ...
            "dark:text-brand-white/70",
            "hover:bg-brand-light-gray/20",
            "dark:hover:bg-brand-dark-gray/30",
          );
        });
        ...
        // Update content
        document
          .querySelectorAll(".analytics-tab-content")
          .forEach(function (content) {
            content.classList.add("hidden");
          });
        document
          .getElementById("analytics-content-" + tabId)
          .classList.remove("hidden");
    ...
    // Item table sorting
    document
      .querySelectorAll("#analytics-items-table th[data-sort]")
      .forEach(function (th) {
        th.onclick = function () {
          var field = th.getAttribute("data-sort");
          if (analyticsItemsSort.field === field) {
            analyticsItemsSort.direction =
              analyticsItemsSort.direction === "asc" ? "desc" : "asc";
          } else {
    ...
    async function onStartPoll() {
      var pollId = pollSelect.value;
      if (!pollId) {
        await veritasAlert("Please select a poll first.", {
          title: "Live poll",
        });
        return;
      }

      var selectedPoll = ALL_POLLS.find(function (p) {
        return p.pollId === pollId;
      });
      if (!selectedPoll) {
        await veritasAlert("Could not find the selected poll data.", {
          title: "Error",
        });
        setButtonLoading(startPollBtn, false);
        return;
          ...
          .then(function (result) {
            ...
            if (data && data.success) {
              ...
              if (
                emulatorAvailable &&
                emulatorEnabled &&
                emulatorEnabled.checked
              ) {
                triggerSecureAssessmentEmulation(pollId, data.sessionId);
              }
            } else {
              throw new Error(
                (data && data.error) || "Failed to start secure session",
              );
            }
          })
          .catch(function (error) {
    ...
    function onResumePoll() {
      ...
      var qIndex =
        typeof CURRENT_POLL_DATA.questionIndex === "number"
          ? CURRENT_POLL_DATA.questionIndex
          : 0;
    ...
    function onEndQuestionAndShowAnswer() {
      ...
      if (endShowBtn) {
        endShowBtn.disabled = true;
      }
      if (PREVIEW_MODE) {
        if (endShowBtn) {
          endShowBtn.disabled = false;
        }
    ...
    function onPreviousQuestion() {
      ...
      var qDef =
        CURRENT_POLL_DATA.questions && CURRENT_POLL_DATA.questions[nextIndex]
          ? CURRENT_POLL_DATA.questions[nextIndex]
          : null;
    ...
    function onNextQuestion() {
      if (navigationRequestInFlight) {
        console.warn(
          "Navigation already in progress; ignoring extra Next click",
        );
        return;
      }
      ...
      if (CURRENT_POLL_DATA && CURRENT_POLL_DATA.questions) {
        ...
        if (nextIndex < CURRENT_POLL_DATA.questions.length) {
          console.log(
            "[Optimistic] Advancing UI to question index:",
            nextIndex,
          );
    ...
    async function onResetQuestion() {
      ...
      if (!pollId || typeof questionIndex === "undefined") {
        await veritasAlert("No active question to reset.", {
          title: "Reset question",
        });
        return;
      }
      var proceed = await veritasConfirm(
        "Reset the current question for all students?",
        {
          title: "Reset question",
      ...
      var clearResponses = await veritasConfirm(
        "Select OK to reset and clear responses, or Cancel to reset while keeping submissions.",
        {
          title: "Responses",
      ...
      setSessionFn({
        ...
        questionText:
          ...
          CURRENT_POLL_DATA.questions[questionIndex]
            ? CURRENT_POLL_DATA.questions[questionIndex].questionText
            : "",
        options:
          CURRENT_POLL_DATA.questions &&
          CURRENT_POLL_DATA.questions[questionIndex]
            ? CURRENT_POLL_DATA.questions[questionIndex].options
            : [],
      })
        .then(function (result) {
          isPaused = false;
          showToast(
            ...
            "Question Reset",
            clearResponses
              ? "Timer restarted and responses cleared."
              : "Timer restarted (responses kept).",
          );
        })
        .catch(handleError);
    }
    ...
    function setLiveCalculatorToggleState(isEnabled, isSecureSession) {
      ...
      liveCalculatorToggle.setAttribute(
        "aria-disabled",
        disabled ? "true" : "false",
      );
      liveCalculatorToggle.setAttribute(
        "aria-pressed",
        liveCalculatorEnabled ? "true" : "false",
      );
      liveCalculatorToggle.dataset.state = liveCalculatorEnabled ? "on" : "off";

      liveCalculatorToggle.classList.toggle(
        "bg-amber-500",
        liveCalculatorEnabled && !disabled,
      );
      liveCalculatorToggle.classList.toggle(
        "text-veritas-navy",
        liveCalculatorEnabled && !disabled,
      ...
      if (liveCalculatorStateText) {
        liveCalculatorStateText.textContent = liveCalculatorEnabled
          ? "On"
          : "Off";
        liveCalculatorStateText.classList.toggle(
          "text-veritas-gold",
          liveCalculatorEnabled,
        );
        liveCalculatorStateText.classList.toggle(
          "opacity-80",
          !liveCalculatorEnabled,
    ...
    async function onToggleLiveCalculator() {
      if (!liveCalculatorToggle || PREVIEW_MODE) {
        if (PREVIEW_MODE) {
          previewOnlyNotice("toggle the calculator");
        }
        return;
      }
      ...
      try {
        var setLiveSessionState = firebase
          .functions()
          .httpsCallable("setLiveSessionState");
        ...
        showToast(
          "success",
          targetState ? "Calculator enabled" : "Calculator disabled",
          targetState
            ? "Students can now access the calculator."
            : "Calculator has been disabled for students.",
        );
      } catch (error) {
        liveCalculatorToggle.removeAttribute("aria-busy");
        liveCalculatorToggle.disabled = false;
    ...
    function onRevealResults() {
      ...
      const setLiveSessionState = firebase
        .functions()
        .httpsCallable("setLiveSessionState");
      setLiveSessionState({
        pollId: CURRENT_POLL_DATA.pollId,
    ...
    async function onEndPoll() {
      var confirmed = await veritasConfirm(
        "Are you sure you want to end this poll completely?",
        {
          title: "End poll",
      ...
      const finalizeSession = firebase
        .functions()
        .httpsCallable("finalizeSession");
      finalizeSession({
    ...
    function updateLiveOverviewFromData(data) {
      var totalResponses =
        data && typeof data.totalResponses === "number"
          ? data.totalResponses
          : 0;
      var totalStudents =
        data && typeof data.totalStudents === "number" ? data.totalStudents : 0;
      var responsesEl = document.getElementById("live-overview-responses");
      var progressEl = document.getElementById(
        "live-overview-response-progress",
      );
      var responseCaption = document.getElementById(
        "live-overview-response-caption",
      );
      ...
      if (hudResponseCount) {
        hudResponseCount.textContent =
          (totalStudents > 0 ? totalResponses : "0") +
          " / " +
          (totalStudents || 0);
      }
      if (mobileResponseChip) {
        mobileResponseChip.textContent =
          (totalStudents > 0 ? totalResponses : 0) +
          " / " +
          (totalStudents || 0);
      }
      if (hudResponseCaption) {
        hudResponseCaption.textContent =
          totalStudents > 0
            ? completionPct + "% complete"
            : "Awaiting roster data";
      }
      if (hudResponseTrend) {
        hudResponseTrend.classList.remove(
          "hud-trend-up",
          "hud-trend-down",
          "hud-trend-steady",
        );
      ...
      sessionHudState.lastCompletionPct =
        totalStudents > 0 ? completionPct : null;
      ...
      if (allResponded) {
        if (!sessionHudState.allRespondedCelebrated) {
          ...
          showToast(
            "success",
            "All Students Responded!",
            "Everyone has submitted their answer.",
            null,
            {
              label: "Reveal Answer",
              onClick: function () {
      ...
      resultsArray.forEach(function (result) {
        ...
        var answerText =
          result.answerText || result.text || result.optionText || "";
        var isCorrect =
          result.isCorrect === true ||
          (!!correctAnswer && answerText === correctAnswer);
        if (isCorrect) {
          correctCount += count;
        }
      });
      ...
      var accuracyCaption = document.getElementById(
        "live-overview-accuracy-caption",
      );
      ...
      var accuracyPct =
        countedResponses > 0
          ? Math.round((correctCount / countedResponses) * 100)
          : 0;
      if (accuracyEl) {
      ...
      if (accuracyCaption) {
        if (countedResponses === 0) {
          ...
          accuracyCaption.textContent =
            correctCount + " of " + countedResponses + " correct";
        }
      }

      if (hudAccuracyValue) {
        hudAccuracyValue.textContent =
          countedResponses > 0 ? accuracyPct + "%" : "--%";
      }
      if (mobileAccuracyChip) {
        mobileAccuracyChip.textContent =
          countedResponses > 0 ? accuracyPct + "%" : "--%";
      }
      ...
      if (countedResponses > 0) {
        ...
        var hudAccuracyPercent = document.getElementById(
          "hud-accuracy-percent",
        );
      ...
      if (hudAccuracyCaption) {
        if (countedResponses === 0) {
          ...
          hudAccuracyCaption.textContent =
            correctCount + " of " + countedResponses + " correct";
        }
      }
      if (hudAccuracyTrend) {
        hudAccuracyTrend.classList.remove(
          "hud-trend-up",
          "hud-trend-down",
          "hud-trend-steady",
        );
      ...
      sessionHudState.lastAccuracyPct =
        countedResponses > 0 ? accuracyPct : null;
    }
    ...
    function updateLiveOverviewCounts(lockedCount, blockedCount, waitingCount) {
      ...
      var lockoutsCaption = document.getElementById(
        "live-overview-lockouts-caption",
      );
      ...
      if (mobileLocksLabel) {
        mobileLocksLabel.textContent =
          pendingTotalForHud === 1
            ? "1 Locked"
            : pendingTotalForHud + " Locked";
      }
      ...
      var waitingCaption = document.getElementById(
        "live-overview-waiting-caption",
      );
      if (waitingCaption) {
        if (!waitingCount) {
    ...
    async function onEndIndividualSession() {
      ...
      var pollId =
        ...
        (CURRENT_POLL_DATA && CURRENT_POLL_DATA.pollId);
      if (!pollId) {
        return;
      }
    ...
    function updateLiveView(data) {
      ...
      var isCollecting =
        typeof metadata.isCollecting === "boolean"
      ...
      var calculatorEnabled =
        metadata.calculatorEnabled === true ||
      ...
      if (!CURRENT_POLL_DATA.questions && ALL_POLLS) {
        var sourcePoll = ALL_POLLS.find(function (p) {
          return p.pollId === (data.pollId || CURRENT_POLL_DATA.pollId);
        });
      ...
      var isSecureSession = isSecureSessionTypeClient(
        data.sessionType || pollSessionType,
      );
      setLiveCalculatorToggleState(calculatorEnabled, isSecureSession);

      var questionChanged =
        previousQuestionIndex === null ||
        data.questionIndex !== previousQuestionIndex;
      ...
      if (questionChanged) {
        ...
        var defaultSeconds = data.timerSeconds
          ? Math.min(600, Math.max(15, parseInt(data.timerSeconds, 10)))
          : 90;
      ...
      document.getElementById("live-question-info").textContent =
        "Q" + (data.questionIndex + 1) + "/" + data.totalQuestions;
      document.getElementById("live-question-text").textContent =
        data.questionText;
      document.getElementById("live-question-text-compact").textContent =
        data.questionText;

      // Handle question image with cache-busting
      var questionImageContainer = document.getElementById(
        "question-image-container",
      );
      var questionImageEl = document.getElementById("live-question-image");
      var questionLayoutContainer = document.getElementById(
      ...
      document.getElementById("response-count").textContent =
        data.totalResponses + " / " + data.totalStudents + " answered";
      var responseCountHeader = document.getElementById(
        "response-count-header",
      );
      if (responseCountHeader) {
        responseCountHeader.textContent =
          data.totalResponses + " / " + data.totalStudents + " answered";
      }
      ...
      if (resultsStrip) {
        ...
        if (isCollecting || !hasQuestion || !isResultsRevealed) {
          ...
          if (stripStatus) {
            var suffix =
              data.totalResponses === 0 ? " No responses were recorded." : "";
            stripStatus.textContent =
              "Students can now see the correct answer and results." + suffix;
          }
        }
      }
    ...
    function updateQuestionDots(current, total) {
      ...
      for (var i = 0; i < total; i++) {
        ...
        dot.className =
          ...
          (i === current ? "bg-primary" : "bg-primary/50");
    ...
    function renderResultsBars(results, correctAnswer, studentsList) {
      ...
      var optionDefs = Array.isArray(CURRENT_POLL_DATA.options)
        ? CURRENT_POLL_DATA.options.slice()
        : [];
      var knownKeys = {};
      ...
      if (optionDefs.length === 0) {
        container.innerHTML =
          '<p class="text-center text-gray-500 py-4">Waiting for responses...</p>';
        return;
      }
      ...
      var total = optionDefs.reduce(function (sum, opt) {
        ...
        var value =
          results && typeof results[key] === "number" ? results[key] : 0;
        return sum + value;
      }, 0);
      ...
      optionDefs.forEach(function (option, index) {
        ...
        var count =
          results && typeof results[answerText] === "number"
            ? results[answerText]
            : 0;
        ...
        var borderColor = isCorrect
          ? "border-green-500 dark:border-green-400"
          : "border-gray-200 dark:border-gray-700";
        var bgColor = isCorrect
        ...
        cardHtml +=
          '<span class="text-white text-base font-bold">' + letter + "</span>";
        cardHtml += "</div>";
        ...
        if (answerText) {
          cardHtml +=
            '<p class="text-[1.125rem] font-medium text-brand-dark-gray dark:text-brand-white break-words leading-relaxed">' +
            escapeHtml(answerText) +
            "</p>";
        } else if (!imageURL) {
          cardHtml +=
            '<p class="text-sm font-medium text-gray-400 dark:text-gray-500">(No content)</p>';
        }

        if (imageURL) {
          cardHtml +=
            '<div class="mt-1.5"><img src="' +
            escapeHtml(imageURL) +
            '" class="max-w-full max-h-24 rounded-lg border border-gray-200 dark:border-gray-700" alt="Answer ' +
            letter +
            ' image" referrerpolicy="no-referrer"/></div>';
        }

        // Student names with confidence (compact, below answer text)
        var studentsWithAnswer = Array.isArray(studentsList)
          ? studentsList.filter(function (s) {
              return s.answer === answerText;
            })
          : [];
        ...
        var percentageBg = isCorrect
          ? "bg-green-500 dark:bg-green-600"
          : "bg-gray-700 dark:bg-gray-600";
        cardHtml +=
          '<div class="flex-shrink-0 ' +
          percentageBg +
          ' text-white rounded-lg px-2 py-1 flex flex-col items-center">';
        cardHtml +=
          '<span class="text-lg font-bold leading-none">' +
          percentage +
          "%</span>";
        cardHtml +=
          '<span class="text-[10px] opacity-90 mt-0.5">' + count + "</span>";
        cardHtml += "</div>";
    ...
    function renderLiveMetacognition(metacognition) {
      ...
      card.className =
        "bg-white dark:bg-brand-dark-gray/30 p-3 rounded-xl border border-purple-200 dark:border-purple-700 shadow-sm";

      var totalResponses = metacognition.totalResponses || 0;
      var participation = metacognition.responseRate || 0;
      var flagged = Array.isArray(metacognition.flaggedStudents)
        ? metacognition.flaggedStudents
        : [];
      ...
      html +=
        '<span class="material-symbols-outlined text-purple-600 dark:text-purple-300 text-xl">psychology</span>';
      html +=
        '<div><h4 class="text-brand-dark-gray dark:text-brand-white font-semibold text-sm leading-tight">Confidence Pulse</h4>';
      html +=
        '<p class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60">Certainty vs. accuracy snapshot</p></div>';
      html += "</div>";
      html +=
        '<div class="text-xs px-2 py-1 rounded-md bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-200 font-semibold">' +
      ...
      html +=
        '<span class="px-2 py-1 rounded-md bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200">✓ Correct: ' +
        (metacognition.correctCount || 0) +
        "</span>";
      html +=
        '<span class="px-2 py-1 rounded-md bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-200">✗ Incorrect: ' +
        (metacognition.incorrectCount || 0) +
        "</span>";
      html +=
        '<span class="px-2 py-1 rounded-md bg-slate-100 text-slate-700 dark:bg-slate-800/50 dark:text-slate-200">⊘ Unanswered: ' +
        (metacognition.unansweredCount || 0) +
        "</span>";
      html += "</div>";
      ...
      matrixDefs.forEach(function (def) {
        var pct =
          metacognition.matrixPercentages &&
          typeof metacognition.matrixPercentages[def.key] === "number"
            ? metacognition.matrixPercentages[def.key]
            : 0;
        var count =
          metacognition.matrixCounts &&
          typeof metacognition.matrixCounts[def.key] === "number"
            ? metacognition.matrixCounts[def.key]
            : 0;
        ...
        html +=
          '<div class="flex items-center gap-1.5"><span class="text-sm">' +
          def.emoji +
        ...
        html += "</div>";
        html +=
          '<div class="confidence-bar-track my-1"><div class="confidence-bar-fill ' +
          def.barClass +
          '" style="width:' +
          pct +
          '%"></div></div>';
        ...
        if (categoryStudents.length > 0) {
          ...
          categoryStudents.forEach(function (s) {
            html +=
              '<span class="px-1.5 py-0.5 rounded bg-white/50 dark:bg-black/20 text-[10px] font-medium border border-black/5 truncate max-w-full">' +
        ...
        html +=
          '<div class="flex items-center justify-between text-[11px] text-brand-dark-gray/60 dark:text-brand-white/60 mt-auto">';
      ...
      if (flagged.length > 0) {
        html +=
          '<div class="rounded-lg border border-rose-500/40 bg-rose-500/10 p-2 mt-2">';
        html += '<div class="flex items-center gap-1 mb-1">';
        html +=
          '<span class="material-symbols-outlined text-sm text-rose-700 dark:text-rose-200">report</span>';
        html +=
          '<span class="text-xs font-semibold text-rose-700 dark:text-rose-200">' +
          flagged.length +
          " confidently wrong</span>";
        html += "</div>";
        html += '<div class="flex flex-wrap gap-1">';
        flagged.forEach(function (student) {
          var display = escapeHtml(student.name || student.email || "Student");
          html +=
            '<span class="px-1.5 py-0.5 rounded bg-white/70 text-xs font-medium text-rose-700 dark:bg-rose-900/40 dark:text-rose-100">' +
            display +
            "</span>";
        });
        html += "</div>";
    ...
    function normalizeProctorStatus(status) {
      ...
      if (collapsed.indexOf("blocked") === 0) {
        return "BLOCKED";
      }
      if (
        collapsed.indexOf("awaiting_fullscreen") === 0 ||
        collapsed.indexOf("awaitingfullscreen") === 0 ||
        collapsed.indexOf("awaiting_full_screen") === 0
      ) {
        return "AWAITING_FULLSCREEN";
      }
      if (
        collapsed.indexOf("locked") === 0 ||
        (collapsed.indexOf("locked") > 0 &&
          collapsed.indexOf("unlocked") === -1) ||
        (collapsed.indexOf("approval") !== -1 &&
          collapsed.indexOf("unlocked") === -1)
      ) {
        return "LOCKED";
      }
    ...
    function deriveProctorStatus(student) {
      ...
      return normalizeProctorStatus(
        student.status || student.proctorStatus || null,
      );
    }
    ...
    function updateViolationsAlert(flaggedStudents) {
      ...
      if (!flaggedStudents || flaggedStudents.length === 0) {
        popoverList.innerHTML =
          '<div class="text-xs text-center text-gray-500 py-4 italic">No students need attention</div>';
        return;
      }
      ...
      flaggedStudents.forEach(function (student) {
        ...
        html +=
          '<div class="flex items-center justify-between gap-3 p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-100 dark:border-gray-700">';
        html += '<div class="flex flex-col min-w-0">';
        html +=
          '<span class="text-xs font-bold truncate text-gray-900 dark:text-white">' +
          escapeHtml(formatStudentName(student)) +
          "</span>";
        html +=
          '<span class="text-[10px] text-gray-500 dark:text-gray-400">' +
          (isLocked ? "Locked" : "Blocked") +
          "</span>";
        html += "</div>";

        if (isLocked) {
          html +=
            '<button class="quick-approve-btn flex items-center justify-center h-8 w-8 rounded-full bg-amber-100 hover:bg-amber-200 text-amber-900 transition-colors" data-email="' +
            student.email +
            '" data-poll-id="' +
            CURRENT_POLL_DATA.pollId +
            '" data-lock-version="' +
          ...
          html +=
            '<button class="quick-unblock-btn flex items-center justify-center h-8 w-8 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-900 transition-colors" data-email="' +
            student.email +
      ...
      // Event Listeners
      popoverList
        .querySelectorAll(".quick-approve-btn")
        .forEach(function (btn) {
      ...
      popoverList
        .querySelectorAll(".quick-unblock-btn")
        .forEach(function (btn) {
          btn.onclick = async function (e) {
            e.stopPropagation();
            var email = this.dataset.email;
    ...
    function updateLockedStudentsAlert(lockedStudents) {
      ...
      lockedStudents.forEach(function (student) {
        ...
        html +=
          '<button class="locked-student-unlock-btn flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-lg shadow-sm transition-colors" ';
      ...
      buttonsContainer
        .querySelectorAll(".locked-student-unlock-btn")
        .forEach(function (btn) {
          btn.onclick = async function () {
            var email = this.dataset.email;
            var lockVersion = parseInt(this.dataset.lockVersion, 10);
            if (!CURRENT_POLL_DATA || !CURRENT_POLL_DATA.pollId) {
              await veritasAlert("No active poll found.", {
                title: "Error",
                destructive: true,
              });
              return;
            ...
            this.disabled = true;
            this.innerHTML =
              '<span class="material-symbols-outlined text-base animate-spin">progress_activity</span><span>Unlocking...</span>';
            await approveStudentUnlock(
              email,
              CURRENT_POLL_DATA.pollId,
              lockVersion,
              this,
    ...
    async function approveAllUnlocks() {
      ...
      if (lockedStudents.length === 0) {
        await veritasAlert("No students are currently locked.", {
          title: "No students to approve",
        });
        return;
      }

      var confirmed = await veritasConfirm(
        "Approve re-entry for all " +
          lockedStudents.length +
          " locked student(s)?",
        {
          title: "Approve All",
      ...
      if (approveAllBtn) {
        ...
        approveAllBtn.querySelector("span:last-child").textContent =
          "Approving...";
      }
      ...
      for (var i = 0; i < lockedStudents.length; i++) {
        ...
        try {
          ...
          if (response.data && response.data.success) {
            successCount++;
            // Update local status
            currentStudentStatusData = currentStudentStatusData.map(
              function (s) {
                if (s.email === student.email) {
                  return {
                    ...
                    lockVersion:
                      response.data.lockVersion || student.lockVersion,
      ...
      if (successCount > 0) {
        ...
        showToast(
          "error",
          "Approval Failed",
          "Failed to approve students. Please try again.",
          4000,
        );
      }
    }
    ...
    async function approveStudentUnlock(
      email,
      pollId,
      lockVersion,
      buttonElement,
      ...
      if (!options.skipConfirm) {
        var approved = await veritasConfirm(
          "Approve unlock for " + email + "?",
          {
      ...
      if (buttonElement) {
        ...
        buttonElement.querySelector("span:last-child").textContent =
          "Unlocking...";
      }
        ...
        .then(function (result) {
          ...
          if (response && response.success) {
            ...
            if (buttonElement) {
              ...
              setTimeout(function () {
                if (
                  currentManagedStudent &&
                  currentManagedStudent.email === email
                ) {
            ...
            showToast(
              "success",
              "Unlocked",
              email + " has been approved to rejoin.",
            );
          } else {
            throw new Error(
              (response && response.error) || "Failed to approve unlock",
            );
          }
        })
        .catch(function (error) {
      ...
      try {
        ...
        if (response.data && response.data.success) {
          ...
          showToast(
            "success",
            "Approval Successful",
            "Student can now re-enter fullscreen.",
            3000,
          );
        } else {
          await veritasAlert(
            response.data?.reason || "Failed to force unlock student.",
            {
              title: "Unlock failed",
              destructive: true,
            },
    ...
    function createResetButton(student) {
      var button = document.createElement("button");
      button.className =
        "reset-btn flex-shrink-0 flex items-center justify-center gap-1 h-7 px-2.5 rounded-md bg-primary/10 text-primary dark:text-brand-white dark:bg-brand-white/10 text-xs font-semibold hover:bg-primary/20 dark:hover:bg-brand-white/20 transition-colors";
    ...
    function createUnlockButton(student) {
      var button = document.createElement("button");
      button.className =
        "unlock-btn flex-shrink-0 flex items-center justify-center gap-1 h-8 px-3 rounded-md bg-red-600 text-white text-xs font-bold hover:bg-red-700 transition-colors shadow-sm";
      button.dataset.email = student.email || "";
      if (CURRENT_POLL_DATA && CURRENT_POLL_DATA.pollId !== undefined) {
        button.dataset.pollId = CURRENT_POLL_DATA.pollId;
      }
      button.dataset.lockVersion =
        student.lockVersion != null ? student.lockVersion : 0;
      var icon = document.createElement("span");
      icon.className = "material-symbols-outlined text-sm";
    ...
    function createBlockButton(student) {
      var button = document.createElement("button");
      button.className =
        "block-btn flex-shrink-0 flex items-center justify-center gap-1 h-8 px-3 rounded-md bg-amber-500/10 text-amber-700 dark:text-amber-300 dark:bg-amber-500/20 text-xs font-semibold hover:bg-amber-500/20 dark:hover:bg-amber-500/30 transition-colors";
      button.dataset.email = student.email || "";
      var icon = document.createElement("span");
      icon.className = "material-symbols-outlined text-sm";
    ...
    function createUnblockButton(student) {
      var button = document.createElement("button");
      button.className =
        "unblock-btn flex-shrink-0 flex items-center justify-center gap-1 h-8 px-3 rounded-md bg-green-500/10 text-green-700 dark:text-green-300 dark:bg-green-500/20 text-xs font-semibold hover:bg-green-500/20 dark:hover:bg-green-500/30 transition-colors";
      button.dataset.email = student.email || "";
      var icon = document.createElement("span");
      icon.className = "material-symbols-outlined text-sm";
      icon.textContent = "lock_open";
    ...
    function setButtonLoadingState(button, label) {
      ...
      button.disabled = true;
      button.innerHTML =
        '<span class="material-symbols-outlined text-sm animate-spin">progress_activity</span><span>' +
        label +
        "</span>";
      return function () {
        button.disabled = false;
    ...
    async function requestTeacherBlock(student, buttonElement) {
      ...
      var pollId = CURRENT_POLL_DATA && CURRENT_POLL_DATA.pollId;
      if (pollId === undefined) {
        await veritasAlert("Start a poll before blocking students.", {
      ...
      var confirmed = await veritasConfirm(
        "Temporarily block " +
          displayName +
          " from responding? This will be recorded for proctoring.",
        {
          title: "Block Student",
          confirmText: "Block",
          destructive: true,
    ...
    async function requestTeacherUnblock(email, buttonElement) {
      ...
      var pollId = CURRENT_POLL_DATA && CURRENT_POLL_DATA.pollId;
      if (pollId === undefined) {
        await veritasAlert("Start a poll before unblocking students.", {
          title: "No active poll",
        });
        return;
      }

      var student = currentStudentStatusData.find(function (s) {
    ...
    function buildStudentStatusTile(student) {
      ...
      if (isLocked || isAwaitingFullscreen)
        tileClass += " locked animate-pulse-gentle";
      ...
      if (student.confidence) {
        emoji =
          (CONFIDENCE_LEVEL_STYLES[student.confidence] || {}).emoji || "🧠";
      ...
      html +=
        '<div class="flex-1 min-w-0 font-bold text-xs truncate">' +
        escapeHtml(name) +
        "</div>";
    ...
    function buildStudentStatusRow(student) {
      var row = document.createElement("tr");
      row.className =
        "border-b border-brand-light-gray/30 dark:border-brand-dark-gray/20 hover:bg-brand-light-gray/20 dark:hover:bg-brand-dark-gray/20";

      var nameCell = document.createElement("td");
      nameCell.className =
        "px-3 py-2 text-sm font-semibold text-brand-dark-gray dark:text-brand-white truncate";
      nameCell.textContent = formatStudentName(student);
      row.appendChild(nameCell);

      var statusCell = document.createElement("td");
      statusCell.className =
        "px-3 py-2 text-sm text-brand-dark-gray/80 dark:text-brand-white/80";
      ...
      answerCell.className =
        "px-3 py-2 text-sm text-brand-dark-gray/70 dark:text-brand-white/70 truncate";
      answerCell.textContent = student.answer || "";
      row.appendChild(answerCell);

      var confidenceCell = document.createElement("td");
      confidenceCell.className =
        "px-3 py-2 text-sm text-brand-dark-gray/60 dark:text-brand-white/60 truncate";
      var confidenceText = describeConfidence(student.confidence);
      confidenceCell.textContent = confidenceText || "—";
      row.appendChild(confidenceCell);

      var correctCell = document.createElement("td");
      correctCell.className =
        "px-3 py-2 text-sm text-brand-dark-gray/70 dark:text-brand-white/70 text-center";
      ...
      var violations =
        (student.sessionViolations || 0) + (student.sessionExits || 0);
      var violationsCell = document.createElement("td");
      violationsCell.className =
        "px-3 py-2 text-sm text-brand-dark-gray/70 dark:text-brand-white/70 text-center";
    ...
    function renderStudentStatus() {
      ...
      var students = Array.isArray(currentStudentStatusData)
        ? currentStudentStatusData.slice()
        : [];
      ...
      var isLockedOrBlocked = function (s) {
        ...
        return (
          normalized === "LOCKED" ||
          normalized === "AWAITING_FULLSCREEN" ||
          normalized === "BLOCKED"
        );
      };
      ...
      if (studentStatusFilter === "waiting") {
        ...
        students = students.filter(function (s) {
          return s.status === "Submitted" || isLockedOrBlocked(s);
        });
      } else if (studentStatusFilter === "locked") {
        students = students.filter(function (s) {
          var normalized = deriveProctorStatus(s);
          return (
            normalized === "LOCKED" || normalized === "AWAITING_FULLSCREEN"
          );
        });
      } else if (studentStatusFilter === "blocked") {
        students = students.filter(function (s) {
          return deriveProctorStatus(s) === "BLOCKED";
        });
      }
        ...
        switch (studentTableSort.column) {
          case "firstName":
            result = compareStrings(
              a.firstName || a.displayName || a.name,
              b.firstName || b.displayName || b.name,
            );
            break;
          case "lastName":
          ...
          case "violations":
            result = compareNumbers(
              (a.sessionViolations || 0) + (a.sessionExits || 0),
              (b.sessionViolations || 0) + (b.sessionExits || 0),
            );
            break;
          case "recent":
      ...
      container.querySelectorAll(".reset-btn").forEach(function (btn) {
        btn.onclick = function () {
          ...
          var pollId = CURRENT_POLL_DATA.pollId;
          var questionIndex = CURRENT_POLL_DATA.questionIndex;
          if (!email || pollId === undefined || questionIndex === undefined)
            return;
          var button = this;
          var originalHtml = button.innerHTML;
          button.disabled = true;
          ...
          firebase
            .database()
            .ref("answers/" + pollId + "/" + questionIndex + "/" + emailKey)
            .set(null)
            .then(function () {
              button.disabled = false;
              button.innerHTML = originalHtml;
    ...
    function buildTelemetryIndicators(telemetry) {
      ...
      if (telemetry.usingCalculator) {
        ...
        calcIcon.className =
          "material-symbols-outlined text-base text-blue-500";
    ...
    function renderTelemetryPill(telemetry) {
      ...
      var timeLabel =
        typeof telemetry.timeOnQuestion === "number"
          ? formatTimeOnQuestion(telemetry.timeOnQuestion)
          : "";
      var calcIcon = telemetry.usingCalculator
        ? '<span class="material-symbols-outlined text-sm text-blue-500">calculate</span>'
        : "";
    ...
    function toggleSidebar() {
      ...
      if (toggleBtn) {
        ...
        toggleBtn.setAttribute(
          "aria-label",
          sidebarVisible ? "Hide student sidebar" : "Show student sidebar",
        );
      }
    }
    ...
    function updateDisplay() {
      ...
      var timerCompactContainer = document.getElementById(
        "timer-compact-container",
      );
      var timerDisplayCompact = document.getElementById(
      ...
      if (timerMainContainer) {
        ...
        timerMainContainer.classList.remove(
          "timer-safe",
          "timer-warning",
          "timer-critical",
        );
    ...
    function toggleButtons(running) {
      ...
      var mobileSubtract10sBtn = document.getElementById(
        "subtract-10s-btn-mobile",
      );
    ...
    function pauseTimer() {
      ...
      toggleButtons(false);
      updateTimerStatus(
        "Timer paused at " + formatTime(totalSeconds) + ".",
        "info",
      );
    }
    ...
    function resetTimer() {
      ...
      updateTimerStatus(
        "Timer reset to " + formatTime(timerPresetSeconds) + ".",
        "info",
      );
          ...
          firebase
            ...
            .then(function (result) {
              ...
              if (data && data.success) {
                ...
                handleError(
                  data.error || "Failed to pause poll on timer expiry",
                );
              }
            })
            .catch(handleError);
    ...
    function updateTimerStatus(message, tone) {
      ...
      if (tone === "danger") {
        statusEl.className =
          "text-xs text-red-600 dark:text-red-400 text-center font-medium";
      } else if (tone === "success") {
        statusEl.className =
          "text-xs text-green-600 dark:text-green-400 text-center font-medium";
      } else {
        statusEl.className =
          "text-xs text-brand-dark-gray/60 dark:text-brand-white/60 text-center";
      }
    }
    ...
    async function onSendLink() {
      ...
      var confirmed = await veritasConfirm(
        "This will email a new, unique poll link to all students in " +
          className +
          ". Are you sure?",
        {
          title: "Send poll links",
        ...
        .then(function (result) {
          ...
          return firebase
            ...
            .then(function (result) {
              ...
              if (response.success) {
                veritasAlert(
                  "Emails processed (SIMULATED) for " +
                    response.sentCount +
                ...
                if (viewLinksBtn) {
                  ...
                  showToast(
                    "info",
                    "Fallback",
                    "Opening link list for manual distribution.",
                    4000,
                  );
                }
              }
    ...
    async function onViewLinks() {
      ...
      if (PREVIEW_MODE) {
        ...
        var roster =
          (PREVIEW_PAYLOAD &&
            PREVIEW_PAYLOAD.rosters &&
            PREVIEW_PAYLOAD.rosters[className]) ||
          [];
        var html =
          '<div class="overflow-x-auto"><table class="w-full border-collapse">';
        html +=
          '<thead><tr class="bg-brand-light-gray dark:bg-brand-dark-gray/50 border-b-2 border-brand-light-gray dark:border-brand-dark-gray/40">';
        html +=
          '<th class="px-4 py-3 text-left text-sm font-semibold text-brand-dark-gray dark:text-brand-white">Student Name</th>';
        html +=
          '<th class="px-4 py-3 text-left text-sm font-semibold text-brand-dark-gray dark:text-brand-white">Email</th>';
        html +=
          '<th class="px-4 py-3 text-left text-sm font-semibold text-brand-dark-gray dark:text-brand-white">Link</th>';
        html +=
          '<th class="px-4 py-3 text-center text-sm font-semibold text-brand-dark-gray dark:text-brand-white">Actions</th>';
        html += "</tr></thead><tbody>";
        roster.forEach(function (student) {
          html +=
            '<tr class="border-b border-brand-light-gray/50 dark:border-brand-dark-gray/20">';
          html +=
            '<td class="px-4 py-3 text-sm text-brand-dark-gray dark:text-brand-white">' +
            escapeHtml(student.name || "Student") +
            "</td>";
          html +=
            '<td class="px-4 py-3 text-sm text-brand-dark-gray dark:text-brand-white">' +
            escapeHtml(student.email || "student@example.com") +
            "</td>";
          html +=
            '<td class="px-4 py-3 text-xs text-brand-dark-gray/70 dark:text-brand-white/70 italic">Preview links unavailable</td>';
          html +=
            '<td class="px-4 py-3 text-center text-xs text-brand-dark-gray/60 dark:text-brand-white/60">—</td>';
          html += "</tr>";
        });
        if (roster.length === 0) {
          html +=
            '<tr><td colspan="4" class="px-4 py-6 text-center text-sm text-brand-dark-gray/60 dark:text-brand-white/60">No roster data available in preview.</td></tr>';
        }
        ...
        linksModal.style.cssText =
          "display: flex !important; visibility: visible !important; position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; z-index: 99999 !important; background-color: rgba(0,0,0,0.6) !important;";
      ...
      firebase
        ...
        .then(function (result) {
          var response = result.data;
          if (!response.success)
            return handleError(new Error("Could not load links"));
          ...
          response.links.forEach(function (link) {
            ...
            html +=
              '<tr class="border-b border-brand-light-gray/50 dark:border-brand-dark-gray/20 hover:bg-brand-light-gray/30 dark:hover:bg-brand-dark-gray/10">';
            html +=
              '<td class="px-4 py-3 text-sm text-brand-dark-gray dark:text-brand-white">' +
            ...
            html +=
              ...
              "</td>";
            html +=
              '<td class="px-4 py-3 text-xs font-mono text-brand-dark-gray/70 dark:text-brand-white/70 max-w-[200px] truncate" title="' +
              escapeHtml(fullUrl) +
              '">' +
              escapeHtml(fullUrl) +
              "</td>";
            ...
            html +=
              '<button class="copy-link-btn px-3 py-1.5 text-xs font-semibold rounded-md bg-primary text-brand-white hover:bg-primary/90 transition-colors" data-url="' +
              escapeHtml(fullUrl) +
              '">Copy</button>';
            html += "</td></tr>";
          });
            ...
            btn.onclick = async function () {
              ...
              try {
                if (
                  ...
                  await veritasAlert(
                    "Automatic copy is not supported in this browser. Please copy the link manually.",
                    { title: "Copy link" },
                  );
                  return;
                }
                ...
                btn.className =
                  "copy-link-btn px-3 py-1.5 text-xs font-semibold rounded-md bg-green-600 text-brand-white";
                setTimeout(function () {
                  btn.textContent = "Copy";
                  btn.className =
                    "copy-link-btn px-3 py-1.5 text-xs font-semibold rounded-md bg-primary text-brand-white hover:bg-primary/90 transition-colors";
                }, 2000);
              } catch (err) {
                await veritasAlert(
                  "Could not copy link. Please copy it manually.",
                  { title: "Copy link", destructive: true },
                );
              }
            };
          });
    ...
    var previewQuestionDisplay = document.getElementById(
      "preview-question-display",
    );
    var previewQuestionCounter = document.getElementById(
      "preview-question-counter",
    );
    ...
    function openPollPreview(pollId) {
      ...
      if (!poll || !poll.questions || poll.questions.length === 0) {
        veritasAlert("This poll has no questions.", {
    ...
    function renderPreviewNav() {
      ...
      currentPreviewPoll.questions.forEach(function (q, index) {
        var div = document.createElement("div");
        div.className =
          "preview-nav-item cursor-pointer rounded-lg p-3 transition-colors " +
          (index === currentPreviewQuestionIndex
            ? "bg-primary/20 dark:bg-primary/30 border-l-4 border-primary"
            : "bg-brand-white dark:bg-brand-dark-gray/30 hover:bg-brand-light-gray/40 dark:hover:bg-brand-dark-gray/40");
        div.dataset.questionIndex = index;

        var html = '<div class="flex items-start gap-2">';
        if (isPreviewReorderMode) {
          html +=
            '<span class="material-symbols-outlined text-sm text-brand-dark-gray/40 dark:text-brand-white/40 cursor-move">drag_indicator</span>';
        }
        html += '<div class="flex-1 min-w-0">';
        html +=
          '<div class="text-xs font-semibold text-brand-dark-gray/80 dark:text-brand-white/80">Q' +
          (index + 1) +
          "</div>";
        html +=
          '<div class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60 truncate mt-1">' +
          escapeHtml(q.questionText || "No text") +
          "</div>";
        html += "</div></div>";
    ...
    function renderPreviewQuestion() {
      if (
        ...
        previewQuestionDisplay.innerHTML =
          '<p class="text-brand-dark-gray/60 dark:text-brand-white/60">No question selected</p>';
        return;
      }
      ...
      html += '<div class="flex items-center gap-2 mb-2">';
      html +=
        '<span class="text-sm font-semibold text-brand-dark-gray/60 dark:text-brand-white/60">Question ' +
        (currentPreviewQuestionIndex + 1) +
        " of " +
        currentPreviewPoll.questions.length +
        "</span>";
      if (q.timerSeconds) {
        html +=
          '<span class="text-sm text-brand-dark-gray/50 dark:text-brand-white/50">• Timer: ' +
          q.timerSeconds +
          "s</span>";
      }
      html += "</div>";
      html +=
        '<h3 class="text-2xl font-bold text-brand-dark-gray dark:text-brand-white">' +
      ...
      if (q.questionImageURL) {
        ...
        html +=
          '<img src="' +
          escapeHtml(q.questionImageURL) +
          '" alt="Question image" class="max-w-full max-h-96 rounded-lg border border-brand-light-gray dark:border-brand-dark-gray/40" referrerpolicy="no-referrer" onerror="this.style.display=\'none\';">';
        html += "</div>";
      }
      ...
      if (q.options && q.options.length > 0) {
        ...
        html +=
          '<h4 class="text-lg font-semibold text-brand-dark-gray dark:text-brand-white mb-3">Answer Options</h4>';
        q.options.forEach(function (opt, optIndex) {
          ...
          var optClass =
            ...
            (isCorrect
              ...
              : "bg-brand-light-gray/30 dark:bg-brand-dark-gray/20 border-brand-light-gray dark:border-brand-dark-gray/40");
          ...
          html +=
            ...
            (isCorrect
              ...
              : "bg-brand-dark-gray/10 dark:bg-brand-white/10 text-brand-dark-gray dark:text-brand-white") +
            '">' +
            letter +
            "</div>";
          html += '<div class="flex-1">';
          if (opt.text) {
            html +=
              '<p class="text-brand-dark-gray dark:text-brand-white">' +
              escapeHtml(opt.text) +
              "</p>";
          }
          if (opt.imageURL) {
            html +=
              ...
              '" alt="Option image" class="mt-2 max-w-[200px] max-h-[150px] rounded border border-brand-light-gray dark:border-brand-dark-gray/40" referrerpolicy="no-referrer" onerror="this.style.display=\'none\';">';
          }
          html += "</div>";
          if (isCorrect) {
            html +=
        ...
        html +=
          '<p class="text-brand-dark-gray/60 dark:text-brand-white/60">No answer options</p>';
      }

      previewQuestionDisplay.innerHTML = html;
      previewQuestionCounter.textContent =
      ...
      document.getElementById("preview-prev-question-btn").disabled =
        currentPreviewQuestionIndex === 0;
      document.getElementById("preview-next-question-btn").disabled =
    ...
    function previewOpenLinks() {
      ...
      if (!className) {
        veritasAlert("This poll has no class assigned.", {
          title: "No class",
          destructive: true,
        });
        return;
    ...
    async function previewDeleteQuestion() {
      if (
        !currentPreviewPoll ||
        !currentPreviewPoll.questions ||
        currentPreviewPoll.questions.length === 0
      )
        return;

      var confirmed = await veritasConfirm(
        "Delete this question from the poll?",
        {
      ...
      if (currentPreviewPoll.questions.length === 0) {
        await veritasAlert(
          "Cannot delete the last question. A poll must have at least one question.",
          {
            title: "Cannot delete",
            destructive: true,
          },
      ...
      if (PREVIEW_MODE) {
        renderPreviewNav();
        renderPreviewQuestion();
        await veritasAlert("Question deleted (preview mode).", {
      ...
      firebase
        ...
        .then(function (result) {
          if (result.data && result.data.success) {
            ...
            renderPreviewNav();
            renderPreviewQuestion();
            veritasAlert("Question deleted successfully.", {
              title: "Question deleted",
            });
          } else {
    ...
    function togglePreviewReorder() {
      ...
      if (isPreviewReorderMode) {
        originalQuestionOrder = JSON.parse(
          JSON.stringify(currentPreviewPoll.questions),
        );
        btn.innerHTML =
          '<span class="material-symbols-outlined text-sm align-middle">check</span>';
        btn.classList.add("bg-primary", "text-white");
        btn.classList.remove(
          "bg-brand-dark-gray/10",
          "dark:bg-brand-white/10",
        ...
        if (
          JSON.stringify(originalQuestionOrder) !==
          JSON.stringify(currentPreviewPoll.questions)
        ) {
    ...
    function saveReorderedQuestions() {
      ...
      if (PREVIEW_MODE) {
        veritasAlert("Question order updated (preview mode).", {
      ...
      firebase
        ...
        .then(function (result) {
          if (result.data && result.data.success) {
            veritasAlert("Question order updated successfully.", {
              title: "Questions reordered",
            });
          } else {
            throw new Error("Failed to update poll");
      ...
      div.addEventListener("dragover", function (e) {
        ...
        if (draggedItemIndex !== null && draggedItemIndex !== index) {
          ...
          if (currentPreviewQuestionIndex === draggedItemIndex) {
            ...
            draggedItemIndex < currentPreviewQuestionIndex &&
            index >= currentPreviewQuestionIndex
          ) {
            currentPreviewQuestionIndex--;
          } else if (
    ...
    function enforceQuestionOrder(preserveQuestionId) {
      ...
      questions = ordered;
      customQuestionOrder = ordered.map(function (question) {
        return question._id;
      });
      ...
      if (selectionId) {
        var newIndex = questions.findIndex(function (question) {
          return question && question._id === selectionId;
        });
        if (newIndex !== -1) {
    ...
    function setQuestionReorderHelp() {
      if (questionSortHelp) {
        questionSortHelp.textContent =
          "Drag questions to reorder them manually.";
      }
    }
    ...
    function normalizeSessionTypeClient(value) {
      ...
      if (normalized === "LIVE" || normalized === "LIVE_POLL")
        return "LIVE_POLL";
      if (
        normalized === "SECURE_ASSESSMENT" ||
        normalized === "SECURE" ||
        normalized === "INDIVIDUAL_TIMED"
      )
    ...
    function syncSessionTypeToggleUI() {
      ...
      sessionToggleButtons.forEach(function (btn) {
        var isActive =
          normalizeSessionTypeClient(btn.dataset.sessionType) ===
          normalizeSessionTypeClient(pollSessionType);
        if (isActive) {
    ...
    function populateSecureSettingsInputs() {
      if (secureTimeLimitInput) {
        secureTimeLimitInput.value =
          typeof secureAssessmentConfig.timeLimitMinutes === "number"
      ...
      if (secureAvailableInput) {
        secureAvailableInput.value = isoToLocalInput(
          secureAssessmentConfig.availableFrom,
        );
      }
      if (secureDueInput) {
    ...
    function readSecureSettingsFromInputs() {
      ...
      if (secureAvailableInput) {
        secureAssessmentConfig.availableFrom = localInputToIso(
          secureAvailableInput.value,
        );
      }
      if (secureDueInput) {
    ...
    function openPollCreator(existingPoll) {
      console.log(
        "openPollCreator called",
        existingPoll ? "with existing poll" : "for new poll",
      );
      try {
        ...
        if (!modal) {
          ...
          if (!modal) {
            ...
            veritasAlert(
              "Unable to open poll creator. Please refresh the page and try again.",
              { title: "Error" },
            );
            return;
          }
        ...
        if (existingPoll) {
          ...
          var existingSessionType = normalizeSessionTypeClient(
            existingPoll.sessionType,
          );
          secureAssessmentConfig = {
            timeLimitMinutes:
              typeof existingPoll.timeLimitMinutes === "number"
                ? existingPoll.timeLimitMinutes
                : existingPoll.secureSettings &&
                    typeof existingPoll.secureSettings.timeLimitMinutes ===
                      "number"
                  ? existingPoll.secureSettings.timeLimitMinutes
                  : "",
            accessCode: (
              existingPoll.accessCode ||
              (existingPoll.secureSettings &&
                existingPoll.secureSettings.accessCode) ||
              ""
            )
              .toString()
              .toUpperCase(),
            availableFrom:
              existingPoll.availableFrom ||
              (existingPoll.secureSettings &&
                existingPoll.secureSettings.availableFrom) ||
              "",
            dueBy:
              existingPoll.dueBy ||
              (existingPoll.secureSettings &&
                existingPoll.secureSettings.dueBy) ||
              "",
          };
        ...
        customQuestionOrder = questions.map(function (question) {
          return question._id;
        });
        ...
        console.log(
          "Modal current display style:",
          modal ? modal.style.display : "N/A",
        );
        ...
        console.log(
          "Modal should now be visible. Display:",
          modal.style.display,
          "Classes:",
          modal.className,
          "Parent:",
        ...
        veritasAlert("Error opening poll creator: " + e.message, {
          title: "Error",
        });
      }
    }
    ...
    function closePollCreator() {
      ...
      selectedQuestionIndex = 0;
      editingPollId = null;
      document.getElementById("save-poll-btn").innerHTML =
    ...
    function loadQuestionBank() {
      ...
      container.innerHTML =
        '<p class="text-center text-brand-dark-gray/60 py-8">Loading questions from your polls...</p>';
      ...
      (function () {
        try {
          ...
          container.innerHTML =
            '<p class="text-center text-red-600 py-8">Failed to synthesize question bank: ' +
            error.message +
            "</p>";
        }
      })();
    ...
    function populatePollFilter() {
      ...
      var pollNames = [
        ...new Set(
          questionBankData.map(function (q) {
            return q.pollName;
          }),
        ),
      ];
      pollFilter.innerHTML =
        '<option value="">All Polls (' +
        questionBankData.length +
        " questions)</option>";
      pollNames.forEach(function (name) {
        var count = questionBankData.filter(function (q) {
          return q.pollName === name;
        }).length;
        pollFilter.innerHTML +=
          '<option value="' +
          escapeHtml(name) +
    ...
    function renderQuestionBank(filterByPoll) {
      ...
      var filteredQuestions = filterByPoll
        ? questionBankData.filter(function (q) {
            return q.pollName === filterByPoll;
          })
        : questionBankData;

      if (filteredQuestions.length === 0) {
        container.innerHTML =
          '<p class="text-center text-brand-dark-gray/60 py-8">No questions found in your question bank.</p>';
        return;
      }

      container.innerHTML = filteredQuestions
        .map(function (q, idx) {
          var isSelected = selectedBankQuestions.has(
            q.pollId + ":" + q.questionIndex,
          );
          var diffBadge = q.difficultyLevel
          ...
          return (
            '<div class="bank-question-item p-4 rounded-lg border cursor-pointer transition-colors ' +
            (isSelected
              ? "border-amber-500 bg-amber-50 dark:bg-amber-900/20"
              : "border-brand-light-gray dark:border-brand-dark-gray/40 hover:bg-brand-light-gray/30") +
            '" data-bank-key="' +
            q.pollId +
            ":" +
            q.questionIndex +
            ...
            '<div class="flex-1">' +
            '<p class="text-sm font-medium text-brand-dark-gray dark:text-brand-white line-clamp-2">' +
            escapeHtml(q.questionText || "No question text") +
            "</p>" +
            '<div class="flex flex-wrap items-center gap-2 mt-2">' +
            '<span class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60">' +
            escapeHtml(q.pollName) +
            "</span>" +
            (q.topicTag
              ? '<span class="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">' +
                escapeHtml(q.topicTag) +
                "</span>"
              : "") +
            diffBadge +
            "</div>" +
            ...
            (isSelected ? "text-amber-600" : "text-brand-light-gray") +
            '">' +
      ...
      container
        .querySelectorAll(".bank-question-item")
        .forEach(function (item) {
          item.onclick = function () {
            ...
            renderQuestionBank(
              document.getElementById("bank-poll-filter")?.value,
            );
            updateBankSelectionCount();
          };
        });
    ...
    function importSelectedQuestions() {
          ...
          var newQuestion = {
            ...
            answers: (sourceQ.answers || []).map(function (a) {
              return typeof a === "string"
                ? { text: a }
                : { text: a.text || "", imageURL: a.imageURL || null };
            }),
      ...
      closeQuestionBank();
      veritasAlert(
        selectedBankQuestions.size + " question(s) imported successfully!",
        { title: "Question Bank" },
      );
    }
    ...
    function updateAICurrentQuestion() {
      ...
      if (currentQ && currentQ.questionText) {
        currentQEl.textContent =
          '"' +
          currentQ.questionText.substring(0, 150) +
          (currentQ.questionText.length > 150 ? "..." : "") +
          '"';
      } else {
    ...
    function initAPBioTagger() {
      ...
      ["bigidea-evo", "bigidea-ene", "bigidea-ist", "bigidea-syi"].forEach(
        function (id) {
    ...
    function updateTagPreview() {
      ...
      if (unitSelect.value && AP_BIO_CURRICULUM[unitSelect.value]) {
        parts.push(
          unitSelect.value + ": " + AP_BIO_CURRICULUM[unitSelect.value].name,
        );
      }
    ...
    function generateAISuggestions() {
      ...
      if (!unitSelect.value) {
        veritasAlert("Please select a unit before applying the tag.", {
          title: "AP Bio Topic Tagger",
        });
        return;
      }
      ...
      renderQuestionWorkspace();
      closeAISuggestions();
      veritasAlert("AP Bio topic tag applied: " + finalTag, {
        title: "Topic Tagged!",
      });
    }
    ...
    function createStimulusWithQuestions() {
      ...
      var questionCount =
        parseInt(
          document.getElementById("stimulus-question-count").value,
          10,
        ) || 2;

      if (!title && !content) {
        veritasAlert("Please enter a stimulus title or content.", {
      ...
      veritasAlert(
        questionCount +
          ' question(s) created with stimulus "' +
          (title || "Untitled") +
          '"',
        { title: "Stimulus Created" },
      );
    }
    ...
    async function deleteQuestion(index) {
      if (questions.length <= 1) {
        await veritasAlert("A poll must have at least one question.", {
          title: "Edit poll",
        });
        return;
      }
    ...
    async function duplicateQuestionLocal(index) {
      if (questions.length >= 50) {
        await veritasAlert("Maximum 50 questions per poll. Cannot duplicate.", {
          title: "Edit poll",
        });
        return;
      }
      ...
      var manualIndex = questionToDuplicate._id
        ? customQuestionOrder.indexOf(questionToDuplicate._id)
        : -1;
      var insertIndex =
        manualIndex === -1 ? customQuestionOrder.length : manualIndex + 1;
    ...
    function renderQuestionNavigator() {
      ...
      questions.forEach(function (q, index) {
        ...
        var div = document.createElement("div");
        div.className =
          "question-nav-item group relative flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer " +
          (isSelected
        ...
        var html =
          ...
          (isSelected
            ...
            : "text-brand-dark-gray/50 dark:text-brand-white/50") +
          '">drag_indicator</span>';
        html +=
          '<p class="' +
          (isSelected
            ? "text-primary dark:text-brand-white"
            : "text-brand-dark-gray dark:text-brand-white") +
          ' text-sm font-medium leading-normal">Question ' +
          (index + 1) +
          "</p>";
        ...
        html +=
          '<button class="duplicate-question-btn p-1 rounded-full hover:bg-brand-dark-gray/10 dark:hover:bg-brand-white/10" data-index="' +
          index +
          '" title="Duplicate question">';
        html +=
          '<span class="material-symbols-outlined text-brand-dark-gray dark:text-brand-white text-base">content_copy</span>';
        html += "</button>";
        if (questions.length > 1) {
          html +=
            '<button class="delete-question-btn p-1 rounded-full hover:bg-brand-dark-gray/10 dark:hover:bg-brand-white/10" data-index="' +
            index +
            '" title="Delete question">';
          html +=
            '<span class="material-symbols-outlined text-brand-dark-gray dark:text-brand-white text-base">close</span>';
          html += "</button>";
        }
        html += "</div>";
    ...
    function renderQuestionWorkspace() {
      ...
      html +=
        '<p class="text-brand-dark-gray dark:text-brand-white tracking-light text-[32px] font-bold leading-tight">Question ' +
      ...
      if (questionImagePreview || isUploading) {
        ...
        if (isUploading) {
          ...
          html +=
            ...
            '" class="rounded-lg max-w-xs h-auto" referrerpolicy="no-referrer">';
          html +=
            '<button type="button" class="remove-question-image-btn h-9 px-4 rounded-lg bg-red-600/10 text-red-600 dark:text-red-300 dark:bg-red-500/20 text-sm font-semibold hover:bg-red-600/20 dark:hover:bg-red-500/30 transition-colors">Remove Image</button>';
        }
        html += "</div>";
      }
      ...
      html += "<div>";
      html +=
        '<label class="text-brand-dark-gray dark:text-brand-white text-sm font-medium leading-normal block pb-2" for="question-topic-input">Topic/Standard Tag</label>';
      html +=
        '<input id="question-topic-input" type="text" class="w-full rounded-lg border border-brand-light-gray dark:border-brand-dark-gray/50 bg-background-light dark:bg-brand-dark-gray/30 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="e.g., Cell Division, Algebra, etc." value="' +
        escapeHtml(question.topicTag || question.topic || "") +
        '">';
      html +=
        '<p class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60 mt-1">Used for topic-level analytics.</p>';
      html += "</div>";

      // NEW FEATURE: Difficulty Level Selector
      html += "<div>";
      html +=
        '<label class="text-brand-dark-gray dark:text-brand-white text-sm font-medium leading-normal block pb-2" for="question-difficulty-select">Difficulty Level</label>';
      html +=
        '<select id="question-difficulty-select" class="w-full rounded-lg border border-brand-light-gray dark:border-brand-dark-gray/50 bg-background-light dark:bg-brand-dark-gray/30 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50">';
      var difficultyValue = question.difficultyLevel || "";
      html +=
        '<option value=""' +
        (difficultyValue === "" ? " selected" : "") +
        ">-- Select --</option>";
      html +=
        '<option value="easy"' +
      ...
      html +=
        '<option value="medium"' +
        (difficultyValue === "medium" ? " selected" : "") +
        ">🟡 Medium</option>";
      html +=
        '<option value="hard"' +
        (difficultyValue === "hard" ? " selected" : "") +
        ">🔴 Hard</option>";
      html += "</select>";
      html +=
        '<p class="text-xs text-brand-dark-gray/60 dark:text-brand-white/60 mt-1">Compare predicted vs actual difficulty.</p>';
      html += "</div>";

      html +=
        '<div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/30 rounded-lg p-3">';
      html += '<div class="flex items-start gap-2">';
      html +=
        '<input type="checkbox" id="question-metacognition-input" class="mt-0.5 h-4 w-4 rounded border-blue-300 dark:border-blue-600 text-blue-600 focus:ring-blue-500 cursor-pointer" ' +
        (question.metacognitionEnabled ? "checked" : "") +
        " />";
      html += '<div class="flex-1">';
      html +=
        '<label for="question-metacognition-input" class="text-brand-dark-gray dark:text-brand-white text-xs font-semibold leading-normal cursor-pointer block">Enable Metacognition Assessment</label>';
      html +=
        '<p class="text-xs text-brand-dark-gray/70 dark:text-brand-white/70 mt-1">Ask students how confident they are in their answer.</p>';
      html += "</div>";
      html += "</div>";
      html += "</div>";
      html += "</div>";
      ...
      question.answers.forEach(function (answer, aIndex) {
        html += renderAnswerChoice(
          answer,
          aIndex,
          question.correctAnswerIndex === aIndex,
        );
      });
      ...
      html += '<div class="mt-4">';
      html +=
        '<button id="add-answer-btn" class="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-brand-light-gray dark:bg-brand-dark-gray/40 text-brand-dark-gray dark:text-brand-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-brand-light-gray/80 dark:hover:bg-brand-dark-gray/60 transition-colors">';
            ...
            uploadFile(file)
              ...
              .catch(async function (error) {
                await veritasAlert(
                  ...
                  { title: "Upload image", destructive: true },
                );
                questions[selectedQuestionIndex].questionImage = null;
      ...
      var metacognitionInput = document.getElementById(
        "question-metacognition-input",
      );
      ...
      var difficultySelect = document.getElementById(
      ...
      document.getElementById("add-answer-btn").onclick = function () {
        questions[selectedQuestionIndex].answers.push({
          text: "",
          image: null,
          imageURL: null,
          imageFileId: null,
      ...
      workspace
        .querySelectorAll(".answer-text-input")
      ...
      workspace
        .querySelectorAll(".correct-radio")
      ...
      workspace
        .querySelectorAll(".delete-answer-btn")
      ...
      workspace
        .querySelectorAll(".answer-image-input")
        .forEach(function (input, index) {
          input.onchange = function () {
            if (this.files && this.files[0]) {
              ...
              uploadFile(file)
                .then(function (fileId) {
                  // Store the fileId (not URL) - URLs generated at render time via proxy
                  questions[selectedQuestionIndex].answers[index].imageFileId =
                    fileId;
                  questions[selectedQuestionIndex].answers[index].image = null;
                  questions[selectedQuestionIndex].answers[index].imageURL =
                    null;
                  markPollDirty();
                  renderQuestionWorkspace();
                })
      ...
      workspace
        .querySelectorAll(".remove-answer-image-btn")
        .forEach(function (btn) {
          btn.onclick = function () {
            var idx = parseInt(this.dataset.index, 10);
    ...
    function renderAnswerChoice(answer, index, isCorrect) {
      ...
      html +=
        '<input class="correct-radio peer h-5 w-5 cursor-pointer appearance-none rounded-full border border-brand-light-gray dark:border-brand-white/30 checked:border-primary checked:bg-primary transition-all" name="correct_answer" type="radio" ' +
        (isCorrect ? "checked" : "") +
        "/>";
      html +=
      ...
      html += "</label>";
      html +=
        '<input class="answer-text-input form-input flex-1 rounded-lg text-brand-dark-gray dark:text-brand-white focus:outline-0 focus:ring-2 focus:ring-primary/50 border border-brand-light-gray dark:border-brand-dark-gray/50 bg-background-light dark:bg-brand-dark-gray/30 focus:border-primary placeholder:text-brand-dark-gray/50 dark:placeholder:text-brand-white/50 p-3 text-base" placeholder="Answer ' +
      ...
      html +=
        '<input type="file" class="answer-image-input hidden" accept="image/*">';
      html += "</label>";
      ...
      var answerPreview = getImagePreviewUrl(
        answer.image || answer.imageURL,
        answer.imageFileId,
      );
      ...
      if (answerPreview || isAnswerUploading) {
        html +=
          '<div class="mt-2 flex items-center gap-3 bg-primary/5 dark:bg-primary/10 p-2 rounded-lg">';
        if (isAnswerUploading) {
          ...
          html +=
            '<img src="' +
            answerPreview +
            '" class="rounded-md max-w-[160px] h-auto" referrerpolicy="no-referrer">';
          html +=
            '<button type="button" class="remove-answer-image-btn h-8 px-3 rounded-md bg-red-600/10 text-red-600 dark:text-red-300 dark:bg-red-500/20 text-xs font-semibold hover:bg-red-600/20 dark:hover:bg-red-500/30 transition-colors" data-index="' +
            index +
            '">Remove</button>';
        }
        html += "</div>";
    ...
    async function deleteAnswer(index) {
      if (questions[selectedQuestionIndex].answers.length <= 2) {
        await veritasAlert(
          "A question must have at least two answer choices.",
          { title: "Edit poll" },
        );
        return;
      }
      ...
      if (questions[selectedQuestionIndex].correctAnswerIndex >= index) {
        questions[selectedQuestionIndex].correctAnswerIndex = Math.max(
          0,
          questions[selectedQuestionIndex].correctAnswerIndex - 1,
        );
      }
      markPollDirty();
      renderQuestionWorkspace();
    ...
    async function savePoll() {
      ...
      var defaultLabel = editingPollId
        ? '<span class="truncate">Save Changes</span>'
        : '<span class="truncate">Publish Poll</span>';

      savePollBtn.disabled = true;
      savePollBtn.innerHTML =
        '<div class="h-5 w-5 animate-spin rounded-full border-2 border-primary border-t-transparent"></div><span>Saving...</span>';
      ...
      if (!pollName || !className) {
        await veritasAlert("Please enter a poll name and select a class.", {
          title: "Save poll",
        });
      ...
      if (isSecureSessionTypeClient(currentSessionType)) {
        if (
          !securePayload.timeLimitMinutes ||
          securePayload.timeLimitMinutes <= 0
        ) {
          await veritasAlert(
            "Secure Assessments require a positive time limit.",
            { title: "Save poll" },
          );
          savePollBtn.disabled = false;
          savePollBtn.innerHTML = defaultLabel;
      ...
      if (questions.length === 0) {
        await veritasAlert("Please add at least one question.", {
          title: "Save poll",
        });
        savePollBtn.disabled = false;
        savePollBtn.innerHTML = defaultLabel;
      ...
      questions.forEach(function (question, index) {
        ...
        var answerTexts = question.answers.map(function (answer) {
          return (answer.text || "").trim();
        });
        var filledAnswers = answerTexts.filter(function (text) {
          return text !== "";
        });
      ...
      var sanitizedQuestions = questions.map(function (question) {
        ...
        if (typeof question.timerSeconds === "number") {
          ...
          typeof question.timerSeconds === "string" &&
          question.timerSeconds.trim() !== ""
        ) {
      ...
      try {
        var payloadQuestions = sanitizedQuestions.map(function (question) {
          ...
          var correctAnswerText = options[correctIndex]
        ...
        if (!payloadQuestions || payloadQuestions.length === 0) {
          await veritasAlert(
            "❌ Error: No valid questions to save. Please check your poll data.",
            { title: "Save poll", destructive: true },
          );
          savePollBtn.disabled = false;
          savePollBtn.innerHTML = defaultLabel;
          return;
        ...
        if (PREVIEW_MODE) {
          ...
          if (editingPollId) {
            var existingIndex = ALL_POLLS.findIndex(function (p) {
              return p.pollId === editingPollId;
            });
            if (existingIndex !== -1) {
              newPollRecord.createdAt =
                ALL_POLLS[existingIndex].createdAt || newPollRecord.createdAt;
          ...
          closePollCreator();
          await veritasAlert(
            "Poll saved locally for preview. Use the live dashboard to publish changes.",
            { title: "Save poll" },
          );
          return;
        }
        ...
        try {
          ...
          if (result.data && result.data.success) {
            ...
            var successMessage = editingPollId
              ? "✅ Poll updated successfully with " +
                questionCount +
                " question" +
                (questionCount === 1 ? "" : "s") +
                "!"
              : "✅ Poll created successfully with " +
                questionCount +
                " question" +
                (questionCount === 1 ? "" : "s") +
                "!";
        ...
        await veritasAlert("❌ Upload error: " + (err.message || err), {
          title: "Save poll",
          destructive: true,
        });
        savePollBtn.disabled = false;
    ...
    function uploadFile(file) {
      return new Promise(function (resolve, reject) {
        ...
        fileToBase64(file)
            ...
            function doUpload() {
              ...
              imageRef
                ...
                .catch(function (error) {
                  ...
                  var isRetryable =
                    msg.includes("retry") || msg.includes("network");

                  if (isRetryable && attempts < maxAttempts) {
                    console.warn(
                      "Upload attempt " + attempts + " failed. Retrying...",
                      error,
                    );
          ...
          .catch(function (error) {
            reject(
              new Error("File conversion error: " + (error.message || error)),
            );
          });
      });
    }
    ...
    async function handleError(error) {
      ...
      if (
        ...
        errorString.includes("HTTP 0") ||
        errorString.includes("Connection failure")
      ) {
        errorMsg =
    ...
    window.expandQuestionImage = function () {
      ...
      var modalHtml =
        '<div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.9);z-index:9999;display:flex;align-items:center;justify-content:center;" onclick="this.remove()">';
      modalHtml +=
        '<img src="' + imgSrc + '" style="max-width:90%;max-height:90%;"/>';
      modalHtml += "</div>";
      document.body.insertAdjacentHTML("beforeend", modalHtml);
    ...
    var closeAnalyticsBtn = document.getElementById(
      "close-analytics-modal-btn",
    );
    if (closeAnalyticsBtn) {
      closeAnalyticsBtn.addEventListener("click", function () {
        document.getElementById("post-poll-analytics-modal").style.display =
          "none";
      });
    }
    ...
    document
      .getElementById("post-poll-analytics-modal")
    ...
    var dismissViolationsBtn = document.getElementById(
      "dismiss-violations-alert",
    );
    if (dismissViolationsBtn) {
      dismissViolationsBtn.addEventListener("click", function () {
        violationsAlertDismissed = true;
        document
          .getElementById("violations-alert-panel")
          .classList.add("hidden");
      });
    }
    ...
    var toggleExitedPanelBtn = document.getElementById(
      "toggle-exited-panel-btn",
    );
    ...
    window.checkAllRespondedState = function (allHaveResponded) {
      ...
      var isLastQuestion =
        CURRENT_POLL_DATA.questionIndex >= CURRENT_POLL_DATA.totalQuestions - 1;
    ...
    (function initFirebaseEagerly() {
      ...
      if (typeof firebase !== "undefined" && FIREBASE_CONFIG) {
        ...
        if (typeof firebase === "undefined") {
          console.error(
            "[Firebase] SDK not loaded! Check script tags in Teacher_View.html",
          );
        }
        if (!FIREBASE_CONFIG) {
      ...
      console.log(
        "[Firebase Init] databaseURL=" +
          debugInfo.databaseURL +
          ", firebasePresent=" +
          debugInfo.firebasePresent +
          ", appsLength=" +
          debugInfo.appsLengthAfter +
          ", initCalled=" +
          debugInfo.initCalled +
          ", success=" +
          debugInfo.success,
      );
    })();
    ...
    async function broadcastSessionState(pollId, state) {
      ...
      var questionDef =
        ...
        typeof state.questionIndex === "number"
          ? CURRENT_POLL_DATA.questions[state.questionIndex]
          : null;

      var finalQuestionText =
        state.questionText ||
        (questionDef ? questionDef.questionText : "") ||
        "Question " + ((state.questionIndex || 0) + 1);
      var finalOptions =
        state.options || (questionDef ? questionDef.options : []);
      ...
      if (firebase && firebase.functions) {
        try {
          ...
          var setLiveSessionState = firebase
            ...
            .httpsCallable("setLiveSessionState");
          console.log(
            "[Firebase] Calling Cloud Function: setLiveSessionState...",
          );

          var correctAns =
            state.correctAnswer ||
            (questionDef ? questionDef.correctAnswer : null);
          ...
          console.warn(
            "[Firebase] Cloud Function failed userspace, falling back to legacy RTDB write:",
            e,
          );
      ...
      var payload = {
        ...
        questionImageURL:
          ...
          (questionDef ? questionDef.questionImageURL : ""),
        options: finalOptions,
        metadata: {
          sessionPhase: state.sessionPhase || "LIVE",
          resultsVisibility: state.resultsVisibility || "HIDDEN",
    ...
    function initFirebaseMissionControl(pollId) {
      ...
      if (firebaseRef && currentMissionControlPollId !== pollId) {
        console.log(
          "[Firebase] Cleaning up listeners for previous pollId:",
          currentMissionControlPollId,
        );
        firebaseRef.off();
        firebaseRef = null;
      ...
      if (typeof firebase !== "undefined") {
        ...
        currentConnectedRef.on("value", function (snap) {
          ...
          console.log(
            "[Firebase] Connection status changed:",
          ...
          if (missionControlHeartbeat && missionControlHeartbeat.isRunning()) {
            console.log(
              "[Adaptive Polling] Updating Mission Control heartbeat to " +
                newInterval +
                "ms",
            );
          ...
          if (pollInterval) {
            console.log(
              "[Adaptive Polling] Updating Live Poll interval to " +
                newInterval +
                "ms",
            );
            clearInterval(pollInterval);
        ...
        firebaseRef.on("child_changed", async function (snapshot) {
          ...
          console.log(
            "[Firebase] Student status changed:",
            studentKey.substring(0, 8) + "...",
            previousStatus,
            "→",
            newStatus,
          );
          ...
          if (
            ...
            if (missionControlStudents && currentMissionControlPollId) {
              for (var i = 0; i < missionControlStudents.length; i++) {
                ...
                var key = await window.VeritasShared.generateStudentKey(
                  s.email,
                  currentMissionControlPollId,
                );
                if (key === studentKey) {
            ...
            if (
              ...
              for (var j = 0; j < currentStudentStatusData.length; j++) {
                ...
                var sKey = await window.VeritasShared.generateStudentKey(
                  student.email,
                  CURRENT_POLL_DATA.pollId,
                );
                if (sKey === studentKey) {
            ...
            showToast(
              "info",
              "Student Resumed",
              (studentName || "A student") + " has re-entered the session.",
              3000,
            );
          }
        });
        ...
        firebaseRef.on("child_added", function (snapshot) {
          ...
          var exists = missionControlStudents.some(function (s) {
            return s.email === studentEmail;
          });

          if (!exists) {
            console.log(
              "[Firebase] New student detected via Fast Path:",
              studentEmail,
            );
        ...
        firebaseRef.on("value", function (snapshot) {
          ...
          console.log(
            "[Firebase] Status update received for pollId:",
            pollId,
            "Students:",
            Object.keys(realtimeStatuses).length,
          );
        ...
        console.log(
          "[Firebase] Mission Control initialized for",
          path,
          "Connected:",
          realtimeConnected,
        );
    ...
    function handleRealtimeAnswer(payload) {
      ...
      if (!CURRENT_POLL_DATA || CURRENT_POLL_DATA.pollId !== payload.pollId)
        return;

      // Find student in current list
      var student = currentStudentStatusData.find(function (s) {
        return s.email === payload.studentEmail;
      });
      if (student) {
        ...
        var card = document.querySelector(
          '.student-status-card[data-email="' +
            CSS.escape(student.email) +
            '"]',
        );
        if (card) {
          ...
          if (progressBar && student.totalQuestions > 0) {
            var pct = Math.min(
              100,
              Math.round((newProgress / student.totalQuestions) * 100),
            );
          ...
          if (progressText) {
            progressText.textContent =
              newProgress + "/" + student.totalQuestions;
          }
        ...
        console.log(
          "[Firebase] Realtime Answer processed for:",
    ...
    function updateRealtimeIndicator() {
      ...
      if (!indicator) {
        ...
        if (headerMeta) {
          ...
          badge.className =
            "ml-4 inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-bold";
      ...
      if (badgeEl) {
        if (realtimeConnected) {
          badgeEl.className =
            "ml-4 inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-bold bg-green-100 text-green-700 border border-green-200";
          badgeEl.innerHTML =
            '<span class="h-2 w-2 rounded-full bg-green-500 animate-pulse"></span> Realtime Active';
        } else {
          badgeEl.className =
            "ml-4 inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-bold bg-red-100 text-red-700 border border-red-200";
          badgeEl.innerHTML =
            '<span class="h-2 w-2 rounded-full bg-red-500"></span> Realtime OFF (Polling mode)';
        }
      }
    }
    ...
    async function updateRealtimeUI() {
      ...
      if (missionControlStudents && currentMissionControlPollId) {
        await Promise.all(
          missionControlStudents.map(async function (student) {
            var key = await window.VeritasShared.generateStudentKey(
              student.email,
              currentMissionControlPollId,
            );
            ...
            if (rtStatus) {
              ...
              var newConnection = student.connection
      ...
      if (
        currentStudentStatusData &&
        currentStudentStatusData.length > 0 &&
        CURRENT_POLL_DATA &&
        CURRENT_POLL_DATA.pollId
      ) {
        var livePollUpdatesFound = false;
        await Promise.all(
          currentStudentStatusData.map(async function (student) {
            var key = await window.VeritasShared.generateStudentKey(
              student.email,
              CURRENT_POLL_DATA.pollId,
            );
    ...
    function updateDebugHud() {
      ...
      if (!hud) {
        ...
        hud.style.cssText =
          "position:fixed;bottom:10px;right:10px;background:rgba(0,0,0,0.95);color:#4ade80;font-family:monospace;font-size:11px;padding:12px;z-index:9999;pointer-events:none;max-width:380px;border:2px solid #4ade80;border-radius:8px;line-height:1.4;";
        document.body.appendChild(hud);
      }

      var connDot = realtimeConnected
        ? '<span style="color:#4ade80">●</span>'
        : '<span style="color:#f87171">●</span>';
      var html =
        '<strong style="color:#fff">🔥 FIREBASE DEBUG (TEACHER)</strong><br>';
      html +=
        connDot +
        ...
        "<br>";
      html +=
        "<strong>PollId (sessionKey):</strong> " +
        (currentMissionControlPollId || "None") +
        "<br>";

      // P0-2 FIX: Show ACTUAL subscribed path (not string-built)
      html +=
        "<strong>Actual RTDB Path:</strong> " +
        (actualSubscribedPath || "Not subscribed") +
        "<br>";
      html +=
        "<strong>Students observed:</strong> " +
        Object.keys(realtimeStatuses).length +
        "<br>";
      ...
      if (keys.length > 0) {
        ...
        keys.slice(0, 2).forEach(function (key) {
          var statusColor =
            realtimeStatuses[key] === "LOCKED"
              ? "#f87171"
              : realtimeStatuses[key] === "ACTIVE"
                ? "#4ade80"
                : "#facc15";
          html +=
            "  • " +
      ...
      if (lastRTDBEvent.timestamp) {
        var eventElapsed = Math.round(
          (Date.now() - lastRTDBEvent.timestamp) / 1000,
        );
        html +=
          "<strong>Last RTDB event:</strong> " +
          lastRTDBEvent.type +
          " (" +
          eventElapsed +
          "s ago)<br>";
      ...
      if (lastStatusChange.timestamp && lastStatusChange.studentKey) {
        var changeElapsed = Math.round(
          (Date.now() - lastStatusChange.timestamp) / 1000,
        );
        html +=
          "<strong>Last change:</strong> " +
          lastStatusChange.studentKey.substring(0, 8) +
          "... → " +
      ...
      if (lastViolation.timestamp && lastViolation.studentKey) {
        var violationElapsed = Math.round(
          (Date.now() - lastViolation.timestamp) / 1000,
        );
        html +=
          '<strong style="color:#f87171">Last violation:</strong> ' +
      ...
      if (lastTeacherAction.timestamp) {
        var elapsed = Math.round(
          (Date.now() - lastTeacherAction.timestamp) / 1000,
        );
        html +=
    ...
    function onStartIndividualTimed() {
      var pollId = pollSelect.value;
      if (!pollId) {
        veritasAlert("Please select a poll first.", {
          title: "Individual Timed Session",
        });
        return;
      }
      setButtonLoading(startPollBtn, true, "Starting...");
      google.script.run
        .withSuccessHandler(function (view) {
          // Soft-fail check
          if (view && (view.success === false || view.error)) {
            setButtonLoading(startPollBtn, false);
            var msg =
              view.message || view.error || "Unknown error starting session";
            veritasAlert(msg, {
    ...
    function showIndividualTimedView(view) {
      ...
      CURRENT_POLL_DATA.questionCount =
        view.totalQuestions || view.questionCount || 0;
      ...
      if (metaEl) {
        var limitLabel = view.timeLimitMinutes
          ? view.timeLimitMinutes + " min limit"
          : "Untimed";
        metaEl.textContent =
          limitLabel +
          " • " +
          (view.students ? view.students.length : 0) +
          " learners";
      }
    ...
    function renderIndividualStudentCards(view) {
      ...
      students.forEach(function (student) {
        var isLocked =
          ...
          (student.status && student.status.indexOf("Locked") === 0);
        if (isLocked) {
          if (!previouslyLockedEmails.has(student.email)) {
            showToast(
              "error",
              "Security Violation",
              student.name + " has been locked.",
            );
      ...
      if (students.length === 0) {
        ...
        empty.textContent =
          "Students will appear here as they join the assessment.";
        ...
        students.forEach(function (student) {
          ...
          var connectionStatus = student.connection
            ? student.connection.status
            : "GRAY";
          ...
          var timeLabel =
            student.timeRemainingSeconds != null
              ? formatTime(student.timeRemainingSeconds)
              : "--:--";
          var progressPct =
            student.progressPct != null ? student.progressPct : 0;
          ...
          var scoreSummary =
            answeredCount > 0 && correctCount !== null
              ? `${correctCount} correct`
              : answeredCount > 0
                ? "Responses recorded"
                : "Awaiting responses";
          var scorePctSummary =
            typeof student.scorePct === "number" &&
            totalQuestions > 0 &&
            answeredCount > 0
          ...
          var requiresUnlock =
            student.proctorStatus === "LOCKED" ||
            student.proctorStatus === "BLOCKED" ||
            statusLower.indexOf("locked") === 0 ||
            statusLower.indexOf("blocked") === 0;

          var card = document.createElement("div");
          var baseCardClass =
            "mission-student-card rounded-2xl border border-slate-200 dark:border-brand-dark-gray/60 bg-white dark:bg-brand-dark-gray/30 p-4 shadow-sm flex flex-col gap-3 transition-all";
          if (selected) {
          ...
          <div class="flex flex-col gap-2 pt-1">
            ...
            ${
              requiresUnlock
                ? `<button type="button" class="inline-flex items-center justify-center gap-2 rounded-full border border-blue-200 text-blue-700 bg-blue-50 px-4 py-2 text-xs font-semibold hover:bg-blue-100" data-action="unlock-student" data-email="${student.email}" data-lock-version="${student.lockVersion || 0}">
    ...
    function pruneMissionSelection(students) {
      if (!missionControlSelectedEmails.size) {
        return;
      }
    ...
    function attachSelectionHandlers() {
      var toggles = document.querySelectorAll(
        '#individual-session-students-grid [data-action="toggle-select"]',
      );
    ...
    function attachUnlockHandlers() {
      var buttons = document.querySelectorAll(
        '#individual-session-students-grid [data-action="unlock-student"]',
      );
    ...
    function syncMissionSelectionIndicators() {
      var cards = document.querySelectorAll(
        "#individual-session-students-grid .mission-student-card",
      );
    ...
    function updateBulkControlsState() {
      ...
      if (bulkSelectionClear) {
        bulkSelectionClear.classList.toggle(
          "hidden",
          missionControlSelectedEmails.size === 0,
        );
      }
      if (!bulkControlsLoading && bulkSelectedLabel) {
        bulkSelectedLabel.textContent =
          missionControlSelectedEmails.size > 0
      ...
      if (bulkTimeAllBtn) {
        ...
        if (allLabel) {
          allLabel.textContent =
            isLoading && scope === "ALL" ? "Adding..." : "Add to entire class";
        }
      }
    ...
    async function handleBulkTimeAdjust(scope) {
      if (!currentMissionControlPollId || !currentSecureSessionId) {
        await veritasAlert(
          "Launch a secure assessment before extending time.",
          { title: "Extend time" },
        );
        return;
      }
      if (scope === "SELECTED" && missionControlSelectedEmails.size === 0) {
        await veritasAlert("Select at least one student first.", {
          title: "Extend time",
        });
        return;
      }
      ...
      if (isNaN(minutes) || minutes <= 0) {
        await veritasAlert("Enter a positive number of minutes to add.", {
          title: "Extend time",
        });
        bulkTimeInput.focus();
        return;
        ...
        .withSuccessHandler(function (response) {
          ...
          if (response && response.success) {
            var audience =
              scope === "ALL"
                ? "the entire class"
                : missionControlSelectedEmails.size + " student(s)";
            showToast(
              "success",
              "Time extended",
              "Added " + minutes + " minute(s) for " + audience + ".",
              4000,
            );
            pollIndividualSessionState();
          } else {
            veritasAlert("Unable to extend time. Please try again.", {
              title: "Extend time failed",
              destructive: true,
            });
          }
        })
      ...
      if (scope === "ALL") {
        runner.adjustSecureAssessmentTimeForAll(
          currentMissionControlPollId,
          currentSecureSessionId,
          minutes,
        );
      } else {
        runner.adjustSecureAssessmentTimeBulk(
          currentMissionControlPollId,
          currentSecureSessionId,
    ...
    function pollIndividualSessionState() {
      ...
      if (missionControlBackoffMs > 0) {
        ...
        if (remaining <= 0) {
          ...
          console.log(
            "Mission control polling backing off:",
      ...
      google.script.run
        .withSuccessHandler(function (view) {
          ...
          missionControlBackoffMs = 0;
          renderIndividualStudentCards(view);
          // Re-apply realtime statuses immediately to prevent flickering
          updateRealtimeUI();
        })
        .withFailureHandler(function (error) {
          ...
          if (
            ...
            missionControlPollFailures++;
            // Exponential backoff: 5s, 10s, 20s, 40s (max)
            missionControlBackoffMs = Math.min(
              5000 * Math.pow(2, missionControlPollFailures - 1),
              40000,
            );
            console.warn(
              "Rate limit hit, backing off for " +
                missionControlBackoffMs +
                "ms",
            );

            // Show a less alarming message to teachers
            if (missionControlPollFailures === 1) {
              showToast(
                "warning",
                "Slow down",
                "Reducing update frequency to prevent rate limits",
                3000,
              );
            }
          } else {
            // For other errors, show the error but do not stop polling
            console.error("Mission control poll error:", error);
            if (missionControlPollFailures < 3) {
              // Only show error alert if it persists
              missionControlPollFailures++;
            } else {
              handleError(error);
            }
    ...
    function updateMissionControlSummary(summary) {
      ...
      if (readyEl) {
        readyEl.textContent = summary.notStarted || 0;
      }
      if (inProgressEl) {
        inProgressEl.textContent = summary.inProgress || 0;
      }
    ...
    function attachManageHandlers() {
      var buttons = document.querySelectorAll(
        '#individual-session-students-grid [data-action="manage-student"]',
      );
      buttons.forEach(function (btn) {
        btn.addEventListener("click", function () {
          var email = btn.dataset.email;
          var target = missionControlStudents.find(function (student) {
    ...
    function openStudentManager(student) {
      if (!studentManagerModal || !student) {
        return;
      }
    ...
    function syncManagedStudentState() {
      ...
      var updated = missionControlStudents.find(function (student) {
        return student.email === currentManagedStudent.email;
      });
      if (updated) {
    ...
    function updateStudentManagerUi(student) {
      ...
      if (studentManagerProgress) {
        ...
        var progressText =
          "Q " + answeredQuestions + " / " + (student.totalQuestions || 0);
      ...
      if (studentManagerTime) {
        studentManagerTime.textContent =
          student.timeRemainingSeconds != null
            ? formatTime(student.timeRemainingSeconds)
            : "--:--";
      }
      if (studentManagerExtra) {
        ...
        studentManagerExtra.textContent =
      ...
      if (studentManagerConnectionDot) {
        studentManagerConnectionDot.classList.remove(
          "bg-emerald-400",
          "bg-amber-400",
          "bg-red-500",
          "bg-slate-300",
      ...
      if (studentManagerMessage) {
        if (student.lastHeartbeatMs) {
          studentManagerMessage.textContent =
            "Last heartbeat " +
            getTimeAgo(new Date(student.lastHeartbeatMs)) +
            ".";
        } else {
          studentManagerMessage.textContent = "";
        }
      ...
      if (studentManagerPause) {
        studentManagerPause.innerHTML =
          ...
          (student.pauseActive ? "play_arrow" : "pause_circle") +
          "</span><span>" +
          (student.pauseActive ? " Resume Timer" : " Pause Timer") +
          "</span>";
      }
      if (studentManagerUnlock) {
        var lockedState =
          student.proctorStatus === "LOCKED" || student.isLocked;
    ...
    function setStudentManagerLoading(isLoading) {
      ...
      [
        ...
        studentManagerAdd10,
        studentManagerPause,
        studentManagerUnlock,
        studentManagerForceSubmit,
      ].forEach(function (btn) {
        if (btn) {
    ...
    function openBookView() {
      ...
      google.script.run
        .withSuccessHandler(function (result) {
          if (result.success) {
            currentBookViewData = result;
            renderBookView(result);
            loading.classList.add("hidden");
            content.classList.remove("hidden");
          } else {
            showBookViewError(result.error || "Failed to load book view data");
          }
        })
        .withFailureHandler(function (error) {
          showBookViewError("Error: " + error.message);
        })
        .getSecureAssessmentBookView(currentMissionControlPollId);
    }
    ...
    function renderClassOverview(overview, questionCount) {
      ...
      if (average) {
        var avgPct =
          questionCount > 0
            ? Math.round((overview.mean / questionCount) * 100)
            : 0;
      ...
      if (interpretation && overview.interpretation) {
        ...
        if (overview.interpretation.participation) {
          html +=
            '<div class="mb-2"><strong>Participation:</strong> ' +
            overview.interpretation.participation.message +
            "</div>";
        }
        if (overview.interpretation.meanScore) {
          html +=
            '<div class="mb-2"><strong>Class Performance:</strong> ' +
            overview.interpretation.meanScore.message +
            "</div>";
        }
        if (overview.interpretation.stdDev) {
          html +=
            "<div><strong>Score Distribution:</strong> " +
            overview.interpretation.stdDev.message +
            "</div>";
        }
    ...
    function renderStudentRoster(students) {
      ...
      students.forEach(function (student) {
        var row = document.createElement("tr");
        row.className =
          "hover:bg-slate-50 dark:hover:bg-brand-dark-gray/20 transition-colors cursor-pointer";
        ...
        var rankBadge = "";
        if (student.rank === 1)
          rankBadge = '<span class="ml-2 text-yellow-500">🥇</span>';
        else if (student.rank === 2)
          rankBadge = '<span class="ml-2 text-gray-400">🥈</span>';
        else if (student.rank === 3)
          rankBadge = '<span class="ml-2 text-orange-600">🥉</span>';

        var percentageClass = "text-slate-700 dark:text-slate-300";
        if (student.percentage >= 90)
          percentageClass = "text-green-600 dark:text-green-400 font-bold";
        ...
        row.innerHTML =
          ...
          rankBadge +
          "</td>" +
          '<td class="px-4 py-3 text-sm font-medium text-brand-dark-gray dark:text-white">' +
    ...
    function renderQuestionAnalysis(items, students) {
      ...
      items.forEach(function (item, idx) {
        var card = document.createElement("div");
        card.className =
          "bg-white dark:bg-brand-dark-gray/30 rounded-xl border border-slate-200 dark:border-brand-dark-gray/40 p-6 shadow-sm";
        ...
        var html =
          ...
          '<div class="flex-1"><h4 class="text-lg font-bold text-brand-dark-gray dark:text-white mb-2">Question ' +
          (idx + 1) +
          "</h4>" +
        ...
        html +=
          ...
          '<div class="bg-slate-50 dark:bg-brand-dark-gray/40 rounded-lg p-3"><p class="text-xs text-slate-500 mb-1">Difficulty (P-Value)</p><p class="text-2xl font-bold" style="color:' +
          difficultyColor +
          '">' +
          (item.difficultyPct || 0) +
          '%</p><p class="text-xs text-slate-600 dark:text-slate-400">' +
          item.interpretation.difficulty.message +
          "</p></div>" +
          '<div class="bg-slate-50 dark:bg-brand-dark-gray/40 rounded-lg p-3"><p class="text-xs text-slate-500 mb-1">Discrimination</p><p class="text-2xl font-bold" style="color:' +
          discriminationColor +
          '">' +
          ...
          "</p></div>" +
          '<div class="bg-slate-50 dark:bg-brand-dark-gray/40 rounded-lg p-3"><p class="text-xs text-slate-500 mb-1">Responses</p><p class="text-2xl font-bold text-slate-700 dark:text-slate-300">' +
          item.responseCount +
          '</p><p class="text-xs text-slate-600 dark:text-slate-400">students answered</p></div>' +
          '<div class="bg-slate-50 dark:bg-brand-dark-gray/40 rounded-lg p-3"><p class="text-xs text-slate-500 mb-1">Overall Quality</p><p class="text-sm font-bold" style="color:' +
          overallColor +
          '">' +
        ...
        if (item.distractorAnalysis && item.distractorAnalysis.length > 0) {
          html +=
            '<div class="border-t border-slate-200 dark:border-brand-dark-gray/40 pt-4"><h5 class="text-sm font-semibold text-brand-dark-gray dark:text-white mb-3">Answer Choice Analysis</h5>' +
            '<div class="space-y-2">';

          item.distractorAnalysis.forEach(function (dist) {
            var isCorrect = dist.isCorrect;
            var bgClass = isCorrect
              ? "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800"
              : "bg-slate-50 dark:bg-brand-dark-gray/40 border-slate-200 dark:border-brand-dark-gray/60";
            var icon = isCorrect ? "✓" : "";
            ...
            html +=
              ...
              bgClass +
              '">' +
              '<div class="flex-shrink-0 w-8 h-8 rounded-full bg-white dark:bg-brand-dark-gray flex items-center justify-center font-bold text-sm">' +
              dist.optionLetter +
              "</div>" +
              '<div class="flex-1"><p class="text-sm font-medium text-brand-dark-gray dark:text-white">' +
              icon +
              " " +
              escapeHtml(dist.option) +
              (isCorrect
                ? ' <span class="text-green-600 dark:text-green-400">(Correct Answer)</span>'
                : "") +
              "</p></div>" +
              '<div class="text-right"><p class="text-sm font-bold text-brand-dark-gray dark:text-white">' +
              dist.totalPct.toFixed(1) +
              '%</p><p class="text-xs text-slate-500">selected</p></div>' +
              '<div class="text-right"><p class="text-xs text-slate-500">High: ' +
              dist.highGroupPct.toFixed(1) +
              '%</p><p class="text-xs text-slate-500">Low: ' +
    ...
    function renderResponseMatrix(students, questionCount) {
      ...
      studentHeader.className =
        "sticky left-0 bg-slate-100 dark:bg-brand-dark-gray/50 px-3 py-2 text-left font-semibold border-r border-slate-300 dark:border-brand-dark-gray/60";
      ...
      for (var i = 0; i < questionCount; i++) {
        ...
        th.className =
          "bg-slate-100 dark:bg-brand-dark-gray/50 px-3 py-2 text-center font-semibold border-l border-slate-300 dark:border-brand-dark-gray/60";
      ...
      totalHeader.className =
        "bg-slate-100 dark:bg-brand-dark-gray/50 px-3 py-2 text-center font-semibold";
      ...
      students.forEach(function (student) {
        ...
        var nameCell = document.createElement("td");
        nameCell.className =
          "sticky left-0 bg-white dark:bg-brand-dark-gray px-3 py-2 text-sm font-medium border-r border-slate-300 dark:border-brand-dark-gray/60";
        ...
        student.questionResponses.forEach(function (qResp) {
          ...
          cell.className =
            "px-3 py-2 text-center text-xs border-l border-slate-200 dark:border-brand-dark-gray/60";
          ...
          if (qResp.answered) {
            ...
            cell.className +=
              " bg-slate-100 dark:bg-brand-dark-gray/40 text-slate-400";
            cell.textContent = "—";
          }
        ...
        totalCell.className =
          "px-3 py-2 text-center font-bold text-sm bg-slate-100 dark:bg-brand-dark-gray/50";
    ...
    function openStudentDetail(student) {
      ...
      if (score)
        score.textContent = student.totalScore + "/" + student.maxScore;
      if (percentage) percentage.textContent = student.percentage + "%";
      if (rank) rank.textContent = "#" + student.rank;
      if (percentile)
        percentile.textContent = student.percentileRank + "th percentile";

      // Render responses
      var responsesContainer = document.getElementById(
      ...
      if (responsesContainer) {
        ...
        student.questionResponses.forEach(function (qResp, idx) {
          var card = document.createElement("div");
          card.className =
            "bg-white dark:bg-brand-dark-gray/30 rounded-lg border border-slate-200 dark:border-brand-dark-gray/40 p-4";

          var statusClass = qResp.isCorrect
          ...
          if (!qResp.answered) {
            statusClass =
              "bg-slate-100 text-slate-600 dark:bg-brand-dark-gray/40 dark:text-slate-400";
            statusIcon = "—";
    ...
    function switchBookTab(tabName) {
      ...
      tabs.forEach(function (tab) {
        ...
        if (btn) {
          if (tab === tabName) {
            btn.className =
              "px-6 py-3 font-semibold text-sm transition-colors border-b-2 border-veritas-gold text-veritas-gold";
          } else {
            btn.className =
              "px-6 py-3 font-semibold text-sm transition-colors border-b-2 border-transparent text-slate-600 dark:text-slate-400 hover:text-veritas-gold hover:border-veritas-gold/30";
          }
        }
    ...
    bindClick("secure-view-links-btn", function () {
      ...
      if (!className) {
        veritasAlert("No class assigned to this assessment.", {
          title: "View Links",
          destructive: true,
        });
        return;
        ...
        firebase
          ...
          .then(function (snapshot) {
            ...
            if (roster && roster.length > 0) {
              ...
              veritasAlert("No students found in this class roster.", {
                title: "View Links",
                destructive: true,
              });
            }
    ...
    if (bookTabStudents)
      bookTabStudents.addEventListener("click", function () {
        switchBookTab("students");
      });
    if (bookTabQuestions)
      bookTabQuestions.addEventListener("click", function () {
        switchBookTab("questions");
      });
    if (bookTabMatrix)
      bookTabMatrix.addEventListener("click", function () {
        switchBookTab("matrix");
      });
    ...
    function runMissionControlAction(fnName, args) {
      ...
      var emailKey =
        typeof generateStudentKey === "function"
          ? generateStudentKey(studentEmail)
          : studentEmail.replace(/[.$#[\]]/g, "_");

      var studentRef = firebase
        .database()
        .ref("sessions/" + pollId + "/students/" + emailKey);
      var dbUpdate = null;

      if (fnName === "adjustSecureAssessmentTime") {
        var delta = args[3];
        dbUpdate = studentRef
          .child("timeAdjustment")
    ...
    google.script.run
      ...
      .saveRosterData(currentClasses, rosterData);
    */
    ...
    function handleAdjustStudentTime(deltaMinutes) {
      ...
      runMissionControlAction("adjustSecureAssessmentTime", [
        currentMissionControlPollId,
        currentSecureSessionId,
        currentManagedStudent.email,
        deltaMinutes,
      ]);
    }
    ...
    function handlePauseToggle() {
      ...
      var fn = currentManagedStudent.pauseActive
        ? "resumeSecureAssessmentStudent"
        : "pauseSecureAssessmentStudent";
      runMissionControlAction(fn, [
        currentMissionControlPollId,
        currentSecureSessionId,
        currentManagedStudent.email,
      ]);
    ...
    async function handleForceSubmitAction() {
      ...
      var confirmed = await veritasConfirm(
        "Force submit and lock " + currentManagedStudent.name + "?",
        {
          title: "Force submit",
          destructive: true,
      ...
      runMissionControlAction("forceSubmitSecureAssessmentStudent", [
        currentMissionControlPollId,
        currentSecureSessionId,
        currentManagedStudent.email,
      ]);
    ...
    document
      .querySelectorAll('[data-student-manager-close="true"]')
      .forEach(function (el) {
        el.addEventListener("click", closeStudentManager);
      });
    if (studentManagerAdd5) {
      studentManagerAdd5.addEventListener("click", function () {
        handleAdjustStudentTime(5);
      });
    }
    if (studentManagerAdd10) {
      studentManagerAdd10.addEventListener("click", function () {
        handleAdjustStudentTime(10);
      });
    }
    if (studentManagerPause) {
    ...
    if (studentManagerForceSubmit) {
      studentManagerForceSubmit.addEventListener("click", function () {
        handleForceSubmitAction();
      });
    }
    if (studentManagerUnlock) {
      studentManagerUnlock.addEventListener("click", function () {
        if (!currentManagedStudent || !currentMissionControlPollId) {
          return;
        }
        // 1. FIREBASE (Instant)
        ...
        approveStudentUnlock(
          currentManagedStudent.email,
          currentMissionControlPollId,
          currentManagedStudent.lockVersion || 0,
          studentManagerUnlock,
        );
      });
    }
    ...
    if (bulkTimeAllBtn) {
      bulkTimeAllBtn.addEventListener("click", function () {
        handleBulkTimeAdjust("ALL");
      });
    }
    if (bulkTimeSelectedBtn) {
      bulkTimeSelectedBtn.addEventListener("click", function () {
        handleBulkTimeAdjust("SELECTED");
      });
    }
    if (bulkSelectionClear) {
      bulkSelectionClear.addEventListener("click", function () {
    ...
    function openPollWizard(pollData) {
      ...
      if (isEdit && pollData) {
        ...
        if (wizardState.mode === "secure") {
          ...
          wizardState.availabilityStart =
            ...
            (pollData.secureSettings &&
              pollData.secureSettings.availableFrom) ||
            null;
          wizardState.availabilityEnd =
            pollData.dueBy ||
            (pollData.secureSettings && pollData.secureSettings.dueBy) ||
            null;
          wizardState.calculatorEnabled =
            pollData.calculatorEnabled === true ||
            (pollData.secureSettings &&
              pollData.secureSettings.calculatorEnabled === true);
        }
      ...
      if (classSelect) {
        ...
        var classesList =
          window.ALL_CLASSES && window.ALL_CLASSES.length > 0
            ? window.ALL_CLASSES
            : typeof BASE_CLASSES !== "undefined"
              ? BASE_CLASSES
              : [];
          ...
          setTimeout(function () {
            ...
            if (retryList.length > 0) {
              ...
              loadingOpt.textContent =
                "No classes found (Create one in Dashboard)";
            }
          }, 1500);
        } else {
      ...
      var headerWizardProgress = document.getElementById(
    ...
    function loadWizardWithData(pollId) {
      ...
      if (poll) {
        ...
        veritasAlert("Failed to load poll data for editing.", {
          title: "Error",
        });
      }
    }
    ...
    function goToWizardStep(stepNumber) {
      ...
      progressContainers.forEach(function (containerSelector) {
        ...
        for (var i = 1; i <= 4; i++) {
          var progressStep = container.querySelector(
      ...
      if (stepNumber === 2) {
        ...
        var calculatorToggle = document.getElementById(
          "wizard-calculator-toggle",
        );
        ...
        if (wizardState.isEditMode && wizardState.mode) {
          var modeLabel = document
            .getElementById("wizard-step-2")
            ?.querySelector("span.mode-label");
          if (modeLabel) {
            modeLabel.textContent =
              wizardState.mode === "secure" ? "Secure Assessment" : "Live Poll";
          }
        }
      }
    ...
    function updateWizardFooter() {
      ...
      if (wizardState.currentStep === 4) {
        ...
        var publishBtnIcon = publishBtn.querySelector(
          ".material-symbols-outlined",
        );
        var publishBtnText =
          publishBtn.querySelector(".publish-btn-text") || publishBtn;

        if (wizardState.isEditMode) {
          if (publishBtnIcon) publishBtnIcon.textContent = "save";
          publishBtnText.innerHTML =
            '<span class="material-symbols-outlined">save</span>Update Poll';
        } else {
          if (publishBtnIcon) publishBtnIcon.textContent = "rocket_launch";
          publishBtnText.innerHTML =
            '<span class="material-symbols-outlined">rocket_launch</span>Publish Poll';
        }
      } else {
    ...
    function saveWizardStepData(stepNumber) {
      if (stepNumber === 2) {
        wizardState.pollName =
          document.getElementById("wizard-poll-name")?.value || "";
        wizardState.className =
          document.getElementById("wizard-class-select")?.value || "";

        if (wizardState.mode === "secure") {
          wizardState.timeLimitMinutes =
            parseInt(document.getElementById("wizard-time-limit")?.value) ||
            null;
          wizardState.accessCode =
            document.getElementById("wizard-access-code")?.value || "";
          wizardState.calculatorEnabled =
            document.getElementById("wizard-calculator-toggle")?.checked ||
            false;
        }
      }
    }
    ...
    function validateWizardStep(stepNumber) {
      ...
      if (stepNumber === 2) {
        var pollName = document
          .getElementById("wizard-poll-name")
          ?.value.trim();
        ...
        if (wizardState.mode === "secure") {
          var timeLimit = parseInt(
            document.getElementById("wizard-time-limit")?.value,
          );
    ...
    function renderWizardQuestions() {
      ...
      wizardState.questions.forEach(function (q, qIndex) {
        ...
        questionDiv.className =
          "question-card bg-white border border-brand-light-gray rounded-xl overflow-hidden shadow-sm transition-all duration-200";
        questionDiv.draggable = true;
        questionDiv.dataset.qIndex = qIndex;

        // Compact Question Header with drag handle
        var headerHtml =
          '<div class="question-card-header flex justify-between items-center px-4 py-2.5 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-brand-light-gray cursor-move" data-drag-handle>' +
          '<div class="flex items-center gap-2">' +
          '<span class="material-symbols-outlined text-slate-400 text-lg" title="Drag to reorder">drag_indicator</span>' +
          '<div class="h-7 w-7 rounded-lg bg-primary text-white flex items-center justify-center font-bold text-xs">' +
          (qIndex + 1) +
          "</div>" +
          '<span class="font-bold text-brand-dark-gray">Q' +
          (qIndex + 1) +
          "</span>" +
          "</div>" +
          ...
          '<input type="checkbox" class="wizard-meta-cb h-3.5 w-3.5 rounded" data-q-index="' +
          qIndex +
          '" ' +
          (q.metacognitionEnabled ? "checked" : "") +
          ">" +
        ...
        var bodyHtml =
          '<div class="question-card-body p-4 space-y-3">' +
          // Question Row: Text + Image Upload
          '<div class="flex gap-3">' +
          '<div class="flex-1">' +
          '<textarea class="wizard-q-text w-full rounded-lg border border-brand-light-gray px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary overflow-hidden min-h-[60px]" style="resize:none;height:auto;" placeholder="Enter question text..." data-q-index="' +
          qIndex +
          '">' +
        ...
        var qImagePreview = getImagePreviewUrl(
          q.questionImageURL,
          q.questionImageFileId,
        );
        var qIsUploading = q.questionImageURL === "UPLOADING";

        if (qIsUploading) {
          bodyHtml +=
            '<div class="h-16 w-20 rounded-lg bg-primary/10 flex items-center justify-center">' +
          ...
          bodyHtml +=
            ...
            '<img src="' +
            qImagePreview +
            '" class="h-16 w-20 object-cover rounded-lg border border-brand-light-gray" referrerpolicy="no-referrer">' +
            '<button type="button" class="wizard-remove-q-img absolute -top-1.5 -right-1.5 h-5 w-5 bg-red-500 text-white rounded-full flex items-center justify-center text-xs" data-q-index="' +
            qIndex +
            '" title="Remove">&times;</button>' +
            "</div>";
        } else {
          bodyHtml +=
            '<label class="h-16 w-20 rounded-lg border-2 border-dashed border-brand-light-gray flex flex-col items-center justify-center cursor-pointer hover:border-primary hover:bg-primary/5 transition-all">' +
            ...
            '<input type="file" class="wizard-q-img-input hidden" accept="image/*" data-q-index="' +
            qIndex +
            '">' +
            "</label>";
        }
        ...
        q.options.forEach(function (opt, optIndex) {
          ...
          var correctBorder = isCorrect
            ? "border-green-400 bg-green-50/30"
            : "border-brand-light-gray/80";
          var optImagePreview = getImagePreviewUrl(
            opt.imageURL,
            opt.imageFileId,
          );
          ...
          bodyHtml +=
            '<div class="answer-option flex items-center gap-2 p-2 rounded-lg border ' +
            correctBorder +
            ' transition-all">' +
            ...
            qIndex +
            '" class="hidden wizard-correct-radio" data-q-index="' +
            qIndex +
            '" data-opt-index="' +
            optIndex +
            '" ' +
            ...
            (isCorrect
              ? "bg-green-500 text-white"
              : "bg-slate-100 text-slate-500 hover:bg-primary/10") +
            '">' +
            optionLabels[optIndex] +
            "</div>" +
            "</label>" +
            // Option text
            '<input type="text" class="wizard-opt-text flex-1 rounded border border-brand-light-gray/70 px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary/30" placeholder="Answer ' +
            optionLabels[optIndex] +
            '..." data-q-index="' +
            qIndex +
            '" data-opt-index="' +
          ...
          if (optIsUploading) {
            bodyHtml +=
              '<div class="h-8 w-10 rounded bg-primary/10 flex items-center justify-center shrink-0">' +
            ...
            bodyHtml +=
              ...
              '<img src="' +
              optImagePreview +
              '" class="h-8 w-10 object-cover rounded border" referrerpolicy="no-referrer">' +
              '<button type="button" class="wizard-remove-opt-img absolute -top-1 -right-1 h-4 w-4 bg-red-500 text-white rounded-full flex items-center justify-center text-[10px]" data-q-index="' +
              qIndex +
              '" data-opt-index="' +
              optIndex +
              '">&times;</button>' +
            ...
            bodyHtml +=
              '<label class="h-8 w-10 rounded border border-dashed border-brand-light-gray/70 flex items-center justify-center cursor-pointer hover:border-primary shrink-0">' +
              '<span class="material-symbols-outlined text-sm text-slate-400">add_photo_alternate</span>' +
              '<input type="file" class="wizard-opt-img-input hidden" accept="image/*" data-q-index="' +
              qIndex +
              '" data-opt-index="' +
              optIndex +
              '">' +
          ...
          // Remove option
          bodyHtml +=
            '<button type="button" class="p-1 rounded text-slate-300 hover:text-red-500 transition-colors" onclick="removeWizardOption(' +
            qIndex +
            "," +
            optIndex +
            ')">' +
        ...
        if (q.options.length < 6) {
          bodyHtml +=
            '<button type="button" class="flex items-center gap-1 text-xs font-medium text-primary hover:text-primary/80" onclick="addWizardOption(' +
            qIndex +
            ')">' +
        ...
        bodyHtml +=
          ...
          '<input type="number" class="wizard-timer w-14 rounded border border-brand-light-gray/70 px-1.5 py-0.5 text-xs text-center" placeholder="sec" min="5" max="600" data-q-index="' +
          qIndex +
          '" value="' +
          (q.timerSeconds || "") +
          '">' +
        ...
        // Option text inputs
        questionDiv
          .querySelectorAll(".wizard-opt-text")
          .forEach(function (input) {
        ...
        questionDiv
          .querySelectorAll(".wizard-opt-img-input")
          .forEach(function (input) {
        ...
        questionDiv
          .querySelectorAll(".wizard-remove-opt-img")
          .forEach(function (btn) {
            btn.addEventListener("click", function () {
              var qi = parseInt(this.getAttribute("data-q-index"));
              var oi = parseInt(this.getAttribute("data-opt-index"));
        ...
        questionDiv
          .querySelectorAll(".wizard-correct-radio")
        ...
        if (timerInput) {
          timerInput.addEventListener("input", function () {
            ...
            wizardState.questions[qIndex].timerSeconds = isNaN(val)
              ? null
              : val;
          });
        }
        ...
        google.script.run
          .withSuccessHandler(function (response) {
            if (response && response.fileId) {
              if (optIndex === null) {
                wizardState.questions[qIndex].questionImageURL =
                  response.url || null;
                wizardState.questions[qIndex].questionImageFileId =
                  response.fileId;
              } else {
                wizardState.questions[qIndex].options[optIndex].imageURL =
                  response.url || null;
                wizardState.questions[qIndex].options[optIndex].imageFileId =
                  response.fileId;
              }
            } else {
    ...
    function renderReviewSummary() {
      var modeText =
        wizardState.mode === "live" ? "Live Poll" : "Secure Assessment";
      document.getElementById("review-mode").textContent = modeText;
      document.getElementById("review-name").textContent =
        wizardState.pollName || "(Untitled)";
      document.getElementById("review-class").textContent =
        wizardState.className || "(No class selected)";

      var qSummary = document.getElementById("review-questions-summary");
      if (qSummary) {
        if (wizardState.questions.length === 0) {
          qSummary.innerHTML =
            '<p class="text-red-500 font-medium">No questions added. Please go back and add at least one question.</p>';
        } else {
          var optionLabels = ["A", "B", "C", "D", "E", "F"];
          var html = '<div class="space-y-4">';
          html +=
            '<p class="text-sm text-brand-dark-gray/60 mb-3">' +
            wizardState.questions.length +
            " question(s) ready to publish</p>";
          ...
          wizardState.questions.forEach(function (q, i) {
            ...
            html +=
              '<div class="bg-slate-50 rounded-lg p-3 border border-brand-light-gray hover:border-primary hover:bg-primary/5 cursor-pointer transition-all group" data-edit-question="' +
              i +
              '" title="Click to edit this question">';
            html += '<div class="flex items-start gap-2 mb-2">';
            html +=
              '<span class="h-6 w-6 rounded bg-primary text-white text-xs font-bold flex items-center justify-center shrink-0">' +
              (i + 1) +
              "</span>";
            html +=
              '<span class="font-medium text-brand-dark-gray text-sm group-hover:text-primary">' +
              escapeHtml(text) +
              "</span>";
            html +=
              '<span class="material-symbols-outlined text-xs text-slate-400 ml-auto opacity-0 group-hover:opacity-100 transition-opacity">edit</span>';
            html += "</div>";
            ...
            if (q.options && q.options.length > 0) {
              ...
              q.options.forEach(function (opt, j) {
                if (opt.text && opt.text.trim() !== "") {
                  ...
                  var optClass = isCorrect
                    ? "bg-green-100 text-green-700 border-green-300"
                    : "bg-white text-brand-dark-gray/70 border-brand-light-gray";
                  var optText =
                    opt.text.length > 20
          ...
          if (wizardState.mode === "secure") {
            ...
            html +=
              '<h5 class="text-sm font-bold text-amber-800 mb-2 flex items-center gap-2"><span class="material-symbols-outlined text-base">lock</span>Secure Assessment Settings</h5>';
            html += '<div class="text-sm text-amber-700 space-y-1">';
            html +=
              "<p><strong>Time Limit:</strong> " +
              (wizardState.timeLimitMinutes || "Not set") +
              " minutes</p>";
            ...
            html +=
              "<p><strong>Calculator:</strong> " +
              (wizardState.calculatorEnabled ? "Enabled" : "Disabled") +
              "</p>";
            html += "</div></div>";
          }
            ...
            .forEach(function (card) {
              card.addEventListener("click", function () {
                var questionIndex = parseInt(
                  this.getAttribute("data-edit-question"),
                );
                ...
                setTimeout(function () {
                  ...
                  if (questionCards[questionIndex]) {
                    ...
                    questionCards[questionIndex].classList.add(
                      "ring-2",
                      "ring-primary",
                      "ring-offset-2",
                    );
                    setTimeout(function () {
                      questionCards[questionIndex].classList.remove(
                        "ring-2",
                        "ring-primary",
                        "ring-offset-2",
                      );
    ...
    function publishWizardPoll() {
      ...
      if (wizardState.mode === "secure") {
        if (
          !wizardState.timeLimitMinutes ||
          wizardState.timeLimitMinutes <= 0
        ) {
      ...
      var apiQuestions = wizardState.questions.map(function (q) {
        ...
        return {
          ...
          questionImageURL:
            q.questionImageURL && q.questionImageURL !== "UPLOADING"
              ? q.questionImageURL
              : null,
          questionImageFileId: q.questionImageFileId || null,
          options: validOptions.map(function (opt) {
            return {
              text: opt.text.trim(),
              imageURL:
                opt.imageURL && opt.imageURL !== "UPLOADING"
                  ? opt.imageURL
                  : null,
              imageFileId: opt.imageFileId || null,
            };
      ...
      var metadata = {
        sessionType:
          wizardState.mode === "secure" ? "SECURE_ASSESSMENT" : "LIVE_POLL",
      };

      if (wizardState.mode === "secure") {
        metadata.timeLimitMinutes =
          parseInt(wizardState.timeLimitMinutes) || 60;
      ...
      // FIREBASE MIGRATION: Use Cloud Functions for secure poll creation
      if (wizardState.isEditMode && wizardState.pollId) {
        // UPDATE EXISTING POLL
        var updatePollFn = firebase.functions().httpsCallable("updatePoll");
        updatePollFn({
          pollId: wizardState.pollId,
          ...
          .then(async function (result) {
            console.log("Poll updated via Cloud Function:", wizardState.pollId);
            await veritasAlert("Poll updated successfully!", {
              title: "Success",
            });
          ...
          .catch(async function (error) {
            ...
            await veritasAlert("Failed to update: " + error.message, {
              title: "Error",
              destructive: true,
            });
            if (publishBtn) {
        ...
        createPollFn({
          ...
          .then(async function (result) {
            var newPollId = result.data.pollId;
            console.log("Poll created via Cloud Function:", newPollId);
            await veritasAlert("Poll published successfully!", {
              title: "Success",
            });
          ...
          .catch(async function (error) {
            ...
            await veritasAlert("Failed to publish: " + error.message, {
              title: "Error",
              destructive: true,
            });
            if (publishBtn) {
    ...
    function handleLightweightSessionUpdate(data) {
      ...
      var questionDef =
        CURRENT_POLL_DATA && CURRENT_POLL_DATA.questions
      ...
      broadcastSessionState(data.pollId, {
        ...
        resultsVisibility: data.metadata
      ...
      if (!CURRENT_POLL_DATA || !CURRENT_POLL_DATA.questions) {
        console.warn(
          "[Optimization] CURRENT_POLL_DATA missing, falling back to full refresh",
        );
        ...
        console.error(
          "[Firebase] Failed to locate poll definition for sync:",
          data.pollId,
        );
        return;
      }
      ...
      if (!questionDef) {
        console.error(
          "[Optimization] Question definition not found for index:",
          qIndex,
        );
        return;
      }
      ...
      setTimeout(function () {
        console.log(
          "[Optimization] Triggering background fetch for consistency",
        );
    ...
    function openMigrationModal() {
      ...
      document
        .getElementById("migration-progress-container")
        .classList.add("hidden");
    ...
    function handleMigrationFileSelect(input) {
      ...
      reader.onload = function (e) {
        try {
          ...
          logMigration(
            "Found " + pCount + " polls and " + rCount + " class rosters.",
          );
    ...
    async function startMigration() {
      ...
      btn.disabled = true;
      btn.innerHTML =
        '<span class="material-symbols-outlined animate-spin">sync</span> Importing...';
        ...
        await Promise.all(
          batch.map(async (poll) => {
            try {
              ...
              console.error(err);
              logMigration(
                "ERROR importing " + poll.pollName + ": " + err.message,
              );
            } finally {
              updateProgress();
            }
      ...
      for (let i = 0; i < rosters.length; i++) {
        ...
        try {
          ...
          await firebase
            .database()
            .ref("rosters/" + roster.className)
            .set(roster.students);
        } catch (err) {
          console.error(err);
          logMigration(
            "ERROR importing roster " + roster.className + ": " + err.message,
          );
        } finally {
          updateProgress();
        }
