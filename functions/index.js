...
/**
 * Manage Roster (Teacher Action)
 * Supports ADD_STUDENT and BULK_IMPORT.
 */
exports.manageRoster = onCall({ cors: true }, async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "User must be authenticated.");
  ...
  const { action, classId, studentData, csvString } = request.data;
  if (!classId) {
  ...
  if (action === "ADD_STUDENT") {
    ...
    if (!email) throw new HttpsError("invalid-argument", "Missing student email");

    const studentRef = classRef.collection("students").doc(sanitizeEmail(email));
    await studentRef.set({
    ...
    if (!csvString) throw new HttpsError("invalid-argument", "Missing csvString");

    // Simple CSV Parser (assumes: email, firstName, lastName)
    ...
    if (count > 0) {
      await batch.commit();
      // Update studentCount in class metadata (simplified)
      await classRef.update({
        studentCount: admin.firestore.FieldValue.increment(count),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      });
    }
