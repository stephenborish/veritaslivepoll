 ...
 * @param {Array} itemScores - Scores for the item
 * @param {Array} totalScores - Total scores for the students
 * @return {number} Point-biserial correlation coefficient
 */
function calculatePointBiserial(itemScores, totalScores) {
  ...
  const variance = totalScores.reduce(
      (sum, score) => sum + Math.pow(score - overallMean, 2), 0) / n;
 ...
 * @param {Object} studentTotalScores - Map of student emails to total scores
 * @return {number} Discrimination index
 */
function calculateDiscriminationIndex(responses, studentTotalScores) {
  ...
  const highCorrect = highGroup.filter((r) => r.isCorrect).length /
    highGroup.length;
  const lowCorrect = lowGroup.filter((r) => r.isCorrect).length /
    lowGroup.length;
...
const interpretations = {
  difficulty: (pValue) => {
    ...
    if (pValue >= 0.5) {
      return {
        level: "moderate",
        message: "Moderate (50-75% correct)",
        color: "green",
      };
    }
    if (pValue >= 0.3) {
      return {
        level: "hard",
        message: "Hard (30-50% correct)",
        color: "yellow",
      };
    }
    return {
      level: "very-hard",
      message: "Very Hard (<30% correct)",
      color: "red",
    };
  },
  discrimination: (discrimination) => {
    if (discrimination >= 0.4) {
      return {level: "excellent", message: "Excellent (>0.4)", color: "green"};
    }
    if (discrimination >= 0.3) {
      return {level: "good", message: "Good (0.3-0.4)", color: "green"};
    }
    if (discrimination >= 0.15) {
      return {level: "fair", message: "Fair (0.15-0.3)", color: "yellow"};
    }
    if (discrimination >= 0) {
      return {level: "poor", message: "Poor (0-0.15)", color: "orange"};
    }
    return {
      level: "negative",
      message: "NEGATIVE (<0) - FLAWED",
      color: "red",
    };
  },
};
 ...
 * @param {Object} historyData - History data from RTDB
 * @param {Object} rostersData - Rosters data from RTDB
 * @return {Object} Computed analytics
 */
function computeAnalytics(historyData, rostersData) {
  ...
  const sessionAggregates = allSessions.map((session) => {
    ...
    const masteryPct = totalAnswered > 0 ?
      (totalCorrect / totalAnswered) * 100 : 0;
    const participationPct = totalStudents > 0 ?
      (participatingStudents.size / totalStudents) * 100 : 0;
  ...
  const masteryLast5 = last5.length > 0 ?
    last5.reduce((sum, s) => sum + s.masteryPct, 0) / last5.length : 0;
  const participationPctAvg = last5.length > 0 ?
    last5.reduce((sum, s) => sum + s.participationPct, 0) / last5.length : 0;
  ...
  const kpis = [
    ...
    {label: "Participation", value: Math.round(participationPctAvg) + "%"},
  ];
 ...
 * @return {Array} Item analysis array
 */
function computeItemAnalysis(poll, answers) {
  // poll: { questions: [...], pollId, pollName }
  // answers: { studentEmail: { qIdx: { isCorrect, answer, confidence } } }
 ...
 * @param {Array} allSessions - All sessions array
 * @param {Array} roster - Roster array
 * @return {Array} Student profiles with insights
 */
function computeStudentInsights(allSessions, roster) {
  ...
  return Object.values(studentProfiles).map((profile) => {
    ...
    if (profile.accuracy < 50 && profile.totalAnswered > 5) {
      profile.flags.push("struggling");
    }
    if (profile.accuracy > 85 && profile.totalAnswered > 5) {
