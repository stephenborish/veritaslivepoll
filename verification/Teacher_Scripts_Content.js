    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      pollCombobox = new VeritasCombobox({
        ...
        onSelect: function (item) {
          // Selection logic is handled by the input event triggering refreshPollViews
          // But we can add specific logic here if needed, e.g., jump to item
          if (item.type === 'class') {
            // Maybe set filter?
            ...
            if (filterSelect) {
              ...
              // Clear search query since we used a filter? Or keep it?
              // Keeping it aligns with "autocomplete" behavior.
            }
          }
        }
      });
      ...
      analyticsStudentCombobox = new VeritasCombobox({
        ...
        onSelect: function (item) {
          if (item.student) {
            ...
            // Hide suggestions handled by combobox
            // Clear input?
      ...
      // Image zoom functionality (student-view-style)
      ...
      // Add click handler to live question image for zoom
    ...
    function createVeritasModal() {
      ...
      function openModal(mode, options) {
        ...
        // Now that focus is inside the modal, it's safe to hide the background
    ...
    function setDashboardSidebarVisibility(visible) {
      ...
      if (visible) {
        ...
        dashboardSidebar.style.display = ''; // Clear inline style if any
      } else {
    ...
    function attachPollActionHandlers(root) {
      ...
      // Bind card-view specific buttons (cards still use individual buttons for now, or we can update them too?
      // The request specifically showed a dropdown menu which usually replaces a row of buttons in a table.
      // The card view has buttons at the bottom. I'll leave card view as is unless requested,
      // but I must ensure card view handlers still work if they exist.)
      ...
      google.script.run
        .withSuccessHandler(function (data) {
          if (data && data.success) {
            ...
            // Handle backend-level failures with user feedback
        ...
        .withFailureHandler(function (error) {
          ...
          // FIX: Show user-visible error message instead of silent console log
          var errorMsg = error && error.message ? error.message : 'Network error loading insights';
          showStudentInsightsError(errorMsg);
        })
    ...
    async function onStartPoll() {
        ...
        google.script.run
          .withSuccessHandler(function (result) {
            ...
            if (result && (result.success === false || result.error)) {
              setButtonLoading(startPollBtn, false);
              var msg = result.message || result.error || 'Unknown error starting secure session';
      ...
      // Default to live session
      google.script.run
        .withSuccessHandler(function (data) {
          // Soft-fail check
          if (data && (data.success === false || data.error)) {
            setButtonLoading(startPollBtn, false);
            var msg = data.message || data.error || 'Unknown error starting poll';
            veritasAlert(msg, { title: 'Unable to Start Poll', destructive: true });
            return;
          }
          setButtonLoading(startPollBtn, false);

          // OPTIMIZATION: Check for lightweight mode
    ...
    function onResumePoll() {
      ...
      google.script.run
        .withSuccessHandler(function(data) {
          // Soft-fail check
          if (data && (data.success === false || data.error)) {
            var msg = data.message || data.error || 'Unknown error resuming poll';
            veritasAlert(msg, { title: 'Unable to Resume Poll', destructive: true });
            return;
          }
          updateLiveView(data);
          if (data && data.pollId) {
        ...
        .withSuccessHandler(function (data) {
          ...
          if (data && data.pollId) {
             broadcastSessionState(data.pollId, {
                 questionIndex: data.questionIndex,
                 status: 'PAUSED',
                 sessionPhase: data.sessionPhase || 'RESULTS_HOLD',
                 resultsVisibility: data.resultsVisibility
             });
          }
        })
    ...
    function onPreviousQuestion() {
      ...
      google.script.run
        .withSuccessHandler(function (data) {
          ...
          if (data.mode === 'lightweight') {
              handleLightweightSessionUpdate(data);
              // Clear navigation flags here since handleLightweightSessionUpdate triggers bg fetch
              // but we want to unblock navigation immediately
              isNavigating = false;
      ...
      google.script.run
        .withSuccessHandler(function (data) {
          ...
          if (data.mode === 'lightweight') {
              isPaused = false;
              handleLightweightSessionUpdate(data);
              showToast('success', 'Question Reset', 'All responses have been cleared.');
              return;
          }
    ...
    function onToggleLiveCalculator() {
      ...
      google.script.run
        .withSuccessHandler(function (enabled) {
          ...
          // FIX TASK D: Server returns boolean directly, not {calculatorEnabled: true}
          var isEnabled = enabled === true;
          setLiveCalculatorToggleState(isEnabled, true);
          // Show toast feedback
          showToast('success', isEnabled ? 'Calculator enabled' : 'Calculator disabled',
                    isEnabled ? 'Students can now access the calculator.' : 'Calculator has been disabled for students.');
        })
        .withFailureHandler(function (error) {
          liveCalculatorToggle.removeAttribute('aria-busy');
          liveCalculatorToggle.disabled = false;
        ...
        .toggleSessionCalculator(pollId);
    }
    ...
    async function onEndPoll() {
      ...
      google.script.run
        .withSuccessHandler(function (data) {
          ...
          } else if (CURRENT_POLL_DATA && CURRENT_POLL_DATA.pollId) {
             // Fallback if data doesn't contain pollId (e.g. status only)
             broadcastSessionState(CURRENT_POLL_DATA.pollId, {
                 questionIndex: -1,
                 status: 'CLOSED',
                 sessionPhase: 'ENDED',
                 resultsVisibility: 'HIDDEN'
    ...
    function updateLiveView(data) {
      ...
      // This fixes the bug where calculator button does nothing because pollSessionType wasn't updated
      if (data.sessionType) {
      ...
      // This ensures that "LOCKED" status from Firebase (which is faster) isn't overwritten by "ACTIVE" from stale server poll
    ...
    function renderResultsBars(results, correctAnswer, studentsList) {
      ...
      optionDefs.forEach(function (option, index) {
        ...
        if (answerText) {
          cardHtml += '<p class="text-sm font-medium text-brand-dark-gray dark:text-brand-white break-words leading-tight">' + escapeHtml(answerText) + '</p>';
        } else if (!imageURL) {
    ...
    function renderLiveMetacognition(metacognition) {
      ...
      matrixDefs.forEach(function (def) {
        ...
        html += '</div>';
        html += '<div class="confidence-bar-track"><div class="confidence-bar-fill ' + def.barClass + '" style="width:' + pct + '%"></div></div>';
        html += '<div class="flex items-center justify-between text-[11px] text-brand-dark-gray/60 dark:text-brand-white/60">';
    ...
    function updateViolationsAlert(flaggedStudents) {
      ...
      var summaryLabel = summaryParts.join(' • ');
      if (summaryLabel) {
      ...
      flaggedStudents.forEach(function (student) {
        if (isBlockedStatus(student)) {
          ...
          html += '</button>';
        } else if (needsUnlockAttention(student)) {
        ...
        try {
          var response = await new Promise(function (resolve, reject) {
            ...
            google.script.run
              ...
              .teacherApproveUnlock(student.email, CURRENT_POLL_DATA.pollId, safeLockVersion);
          });
    ...
    async function approveStudentUnlock(email, pollId, lockVersion, buttonElement, options) {
      ...
      // Server call FIRST, then Firebase write on success
      // This prevents desync where Firebase shows ACTIVE but server has no lock record
      google.script.run
        .withSuccessHandler(async function (response) {
          if (response.ok) {
            ...
            var errorMsg = 'Failed to unlock student. Please try again.';
            if (response.reason === 'not_locked') {
              // CRITICAL FIX: Student is locked client-side (Firebase shows locked, student has poison pill)
              // but server has no record. Use force unlock to create the record and let student resume.
              console.warn('[Proctor] Server has no lock record - using force unlock for:', email);
              teacherForceUnlock(email, pollId, buttonElement);
              return;
            } else if (response.reason === 'version_mismatch') {
              errorMsg = 'Lock version mismatch (student may have violated again). Refreshing status and retrying...';
              // Auto-retry with correct version
              if (response.lockVersion !== undefined) {
                setTimeout(function() {
                  approveStudentUnlock(email, pollId, response.lockVersion, buttonElement, { skipConfirm: true });
                }, 500);
                return;
              }
            }
            await veritasAlert(response.message || errorMsg, {
              title: 'Unlock failed',
              destructive: true
            });
            if (buttonElement) {
      ...
      google.script.run
        .withSuccessHandler(async function (response) {
          if (response.ok) {
            ...
            currentStudentStatusData = currentStudentStatusData.map(function (s) {
              if (s.email === email) {
                return {
                  ...
                  email: s.email,
                  status: 'AWAITING_FULLSCREEN',
                  lockVersion: response.lockVersion || 0,
                  lockReason: s.lockReason,
                  lockedAt: s.lockedAt,
                  answer: s.answer,
                  isCorrect: s.isCorrect,
            ...
            await veritasAlert(response.message || 'Failed to force unlock student.', {
              title: 'Unlock failed',
              destructive: true
            });
            if (buttonElement) {
        ...
        .teacherForceUnlock(email, pollId);
    }
    ...
    function runTeacherBlock(email, pollId, reason) {
      return new Promise(function (resolve, reject) {
        google.script.run
          .withSuccessHandler(resolve)
          .withFailureHandler(reject)
          // FIX: Correct parameter order - API expects (pollId, studentEmail, reason)
          .teacherBlockStudent(pollId, email, reason || '');
      });
    }
    ...
    function runTeacherUnblock(email, pollId) {
      return new Promise(function (resolve, reject) {
        google.script.run
          .withSuccessHandler(resolve)
          .withFailureHandler(reject)
          // FIX: Correct parameter order - API expects (pollId, studentEmail)
          .teacherUnblockStudent(pollId, email);
      });
    }
    ...
    function buildStudentStatusTile(student) {
      ...
      var name = formatStudentName(student);
      var violationCount = (student.sessionViolations || 0) + (student.sessionExits || 0);
      ...
      var statusStyles = {
        'Submitted': {
          ...
          aria: isCorrect ? 'Submitted and correct' : 'Submitted and incorrect'
        },
        'LOCKED': {
          icon: 'lock',
          iconClass: 'locked',
          tileClass: 'locked animate-pulse-gentle',
          aria: 'Locked after violation'
        },
        'BLOCKED': {
          icon: 'block',
          iconClass: 'blocked',
          tileClass: 'blocked',
          aria: 'Blocked after violation'
        },
        'AWAITING_FULLSCREEN': {
          icon: 'fullscreen',
          iconClass: 'fullscreen',
          tileClass: 'awaiting',
          aria: 'Needs fullscreen'
        },
        'Waiting...': {
          icon: 'schedule',
          iconClass: 'waiting',
          tileClass: '',
          aria: 'Waiting for response'
        },
        'default': {
          icon: 'person',
          iconClass: 'waiting',
          tileClass: '',
          aria: status || 'Status pending'
        }
      };

      var style = statusStyles[normalizedStatus] || statusStyles.default;
      tile.className = 'student-tile ' + style.tileClass;

      var badges = [];
      if (student.confidence) {
        var cStyle = CONFIDENCE_LEVEL_STYLES[student.confidence] || {};
        var confLabel = cStyle.label || describeConfidence(student.confidence) || 'Confidence noted';
        var confBadgeClass = cStyle.badgeClass || 'bg-slate-100 text-slate-700';
        badges.push('<span class="student-badge ' + confBadgeClass + '">' + (cStyle.emoji || '🧠') + ' ' + escapeHtml(confLabel) + '</span>');
      }
      if (violationCount > 0) {
        badges.push('<span class="student-badge badge-violation" title="Proctoring violations">' +
          '<span class="material-symbols-outlined text-sm">gavel</span>' +
          '<span>' + violationCount + ' issue' + (violationCount === 1 ? '' : 's') + '</span>' +
          '</span>');
      }

      var html = '<div class="student-top">';
      html += '<span class="student-status-icon ' + style.iconClass + '" aria-label="' + escapeHtml(style.aria || 'Status') + '"><span class="material-symbols-outlined">' + style.icon + '</span></span>';
      html += '<div class="flex-1 min-w-0">';
      html += '<div class="student-name">' + escapeHtml(name) + '</div>';
      if (badges.length > 0) {
        html += '<div class="student-badges">' + badges.join('') + '</div>';
      }
      html += '</div>';
      html += '<div class="student-actions">';
      if (normalizedStatus === 'LOCKED') {
        html += '<button class="approve-reentry-btn student-action-btn" title="Approve re-entry" data-email="' + escapeHtml(student.email) + '" data-lock-version="' + (student.lockVersion || 0) + '"><span class="material-symbols-outlined text-base text-rose-600">lock_open</span></button>';
      }
      if (normalizedStatus === 'BLOCKED' || violationCount > 0) {
        html += '<button class="resume-access-btn student-action-btn" title="Allow student to resume" data-email="' + escapeHtml(student.email || '') + '"><span class="material-symbols-outlined text-base text-emerald-600">door_open</span></button>';
      }
      html += '</div>';
      html += '</div>';
      ...
      if (normalizedStatus === 'LOCKED') {
        var approveBtn = tile.querySelector('.approve-reentry-btn');
        if (approveBtn) {
          approveBtn.addEventListener('click', function (e) {
            e.stopPropagation();
            var email = this.getAttribute('data-email');
            var lockVersion = parseInt(this.getAttribute('data-lock-version'), 10);
            if (!CURRENT_POLL_DATA || !CURRENT_POLL_DATA.pollId) return;
            approveStudentUnlock(email, CURRENT_POLL_DATA.pollId, lockVersion, this, { skipConfirm: true });
          });
        }
        // GRID INTERACTION: Clicking the entire LOCKED tile triggers unlock approval flow
        tile.style.cursor = 'pointer';
        tile.addEventListener('click', function (e) {
          // Don't trigger if clicking a button (button handler will handle it)
      ...
      var resumeBtn = tile.querySelector('.resume-access-btn');
      if (resumeBtn) {
    ...
    function pollForResults() {
      ...
      google.script.run
        .withSuccessHandler(function (data) {
          ...
          updateLiveView(data);
          updateActivityMetrics(pollId, questionIndex);
        })
    ...
    /**
     * Update activity metrics panel with real-time data
     */
    function updateActivityMetrics(pollId, questionIndex) {
      google.script.run
        .withSuccessHandler(function(metrics) {
          var avgTimeEl = document.getElementById('activity-avg-time');
          var changesEl = document.getElementById('activity-answer-changes');
          var activeEl = document.getElementById('activity-active-students');
          var focusLossEl = document.getElementById('activity-focus-loss');
          var lastUpdatedEl = document.getElementById('activity-last-updated');

          if (avgTimeEl) {
            avgTimeEl.textContent = metrics.averageTimeOnQuestion + 's';
          }
          if (changesEl) {
            changesEl.textContent = metrics.answerChanges || 0;
          }
          if (activeEl) {
            activeEl.textContent = metrics.activeStudents || 0;
          }
          if (focusLossEl) {
            focusLossEl.textContent = metrics.focusLossEvents || 0;
          }
          if (lastUpdatedEl) {
            var now = new Date();
        ...
        .withFailureHandler(function(error) {
          ...
          // HIGH PRIORITY FIX: Show user-visible error notification
          showToast('error', 'Activity Metrics Error', 'Unable to load real-time activity metrics. Data will refresh on next update.', 4000);
        })
      ...
      google.script.run
        .withSuccessHandler(async function (response) {
          ...
          if (response.success && response.failedCount === 0) {
            showToast('success', 'Emails Sent', 'Successfully sent ' + response.sentCount + ' links.');
          } else {
            // Partial or full failure
            var msg = 'Sent: ' + response.sentCount + ', Failed: ' + response.failedCount + '.';
        ...
        .withFailureHandler(function (error) {
          ...
          var errorStr = (error && error.toString()) || '';
          if (errorStr.indexOf('Authorization is required') !== -1 || errorStr.indexOf('ScriptApp.invalidateAuth') !== -1) {
             veritasConfirm('Email sending requires authorization. Would you like to run the authorization check now?', { title: 'Authorization Required' })
               .then(function(yes) {
                  if(yes) {
                    google.script.run.authorizeEmail(); // Trigger auth prompt
                  }
               });
          } else {
             handleError(error);
        ...
        .sendPollLinkToClass(className, CURRENT_POLL_DATA.pollId);
    }
    ...
    function loadQuestionBank() {
      ...
      google.script.run
        ...
        .withFailureHandler(function (error) {
          container.innerHTML = '<p class="text-center text-red-600 py-8">Failed to load question bank: ' + (error.message || error) + '</p>';
        })
    ...
    async function savePoll() {
      ...
      try {
        ...
        if (editingPollId) {
          request.updatePoll(editingPollId, pollName, className, payloadQuestions, pollMetadata);
        } else {
          request.createNewPoll(pollName, className, payloadQuestions, pollMetadata);
        }
      } catch (err) {
        ...
        fileToBase64(file).then(function (dataUrl) {
          ...
          function doUpload() {
            ...
            google.script.run
              ...
              .withFailureHandler(function (error) {
                // Check for retryable errors (NetworkError, 429, etc.)
                var msg = (error.message || error || '').toString();
                var isRetryable = msg.includes('NetworkError') || msg.includes('429') || msg.includes('Exceeded memory limit') || msg.includes('Service unavailable');

                if (isRetryable && attempts < maxAttempts) {
                  console.warn('Upload attempt ' + attempts + ' failed. Retrying in ' + (attempts * 2000) + 'ms...', error);
                  setTimeout(doUpload, attempts * 2000); // Exponential-ish backoff: 2s, 4s
                } else {
    ...
    window.checkAllRespondedState = function (allHaveResponded) {
      ...
      // Only pulse if it's not the last question
    ...
    function broadcastSessionState(pollId, state) {
        ...
        // P0-1: Signal-to-Poll Optimization
        // Write lightweight state signal to Firebase so clients can react instantly without polling loop
        ...
        signalRef.set(payload).then(function() {
    ...
    function initFirebaseMissionControl(pollId) {
        ...
        if (typeof firebase !== 'undefined') {
            ...
            connectedRef.on('value', function(snap) {
                ...
                var newInterval = realtimeConnected ? 15000 : 2500;
            ...
            firebaseRef.on('child_changed', function(snapshot) {
                ...
                console.log('[Firebase] Student status changed:', studentKey.substring(0, 8) + '...', '→', newStatus);
            });
      ...
      google.script.run
        .withSuccessHandler(function (view) {
          ...
          if (view && (view.success === false || view.error)) {
            setButtonLoading(startPollBtn, false);
            var msg = view.message || view.error || 'Unknown error starting session';
            veritasAlert(msg, { title: 'Unable to Start Session', destructive: true });
            return;
          }
          setButtonLoading(startPollBtn, false);
    ...
    async function handleBulkTimeAdjust(scope) {
      ...
      var runner = google.script.run
        .withSuccessHandler(function (response) {
          ...
          if (response && response.success) {
            var audience = scope === 'ALL' ? 'the entire class' : missionControlSelectedEmails.size + ' student(s)';
            showToast('success', 'Time extended', 'Added ' + minutes + ' minute(s) for ' + audience + '.', 4000);
            pollIndividualSessionState();
          } else {
            veritasAlert('Unable to extend time. Please try again.', { title: 'Extend time failed', destructive: true });
          }
        })
        .withFailureHandler(function (error) {
          setBulkControlsLoading(false, scope);
      ...
      if (scope === 'ALL') {
        runner.adjustSecureAssessmentTimeForAll(currentMissionControlPollId, currentSecureSessionId, minutes);
      } else {
    ...
    function pollIndividualSessionState() {
      ...
      // Skip this poll if we're in backoff period
      ...
      google.script.run
        ...
        .withFailureHandler(function (error) {
          ...
          if (errorMsg.includes('429') || errorMsg.includes('Rate limit') || errorMsg.includes('quota')) {
            ...
            missionControlBackoffMs = Math.min(5000 * Math.pow(2, missionControlPollFailures - 1), 40000);
            console.warn('Rate limit hit, backing off for ' + missionControlBackoffMs + 'ms');

            // Show a less alarming message to teachers
            if (missionControlPollFailures === 1) {
              showToast('warning', 'Slow down', 'Reducing update frequency to prevent rate limits', 3000);
            }
          } else {
            // For other errors, show the error but don't stop polling
        ...
        .getIndividualTimedSessionTeacherView(currentMissionControlPollId, currentSecureSessionId);
    }
        ...
        google.script.run
          .withSuccessHandler(function (response) {
            if (response.success && response.links) {
              openLinksModalWithData(className, response.links);
            } else {
              veritasAlert('Could not load student links.', { title: 'View Links', destructive: true });
            }
          })
          .withFailureHandler(function (error) {
            handleError(error);
      ...
      google.script.run
        .withSuccessHandler(function (poll) {
          if (poll) {
            ...
            veritasAlert('Failed to load poll data.', { title: 'Error' });
          }
        })
        .withFailureHandler(function (error) {
          handleError(error);
        ...
        google.script.run
          .withSuccessHandler(function (response) {
            if (response && response.fileId) {
              if (optIndex === null) {
                wizardState.questions[qIndex].questionImageURL = response.url || null;
                wizardState.questions[qIndex].questionImageFileId = response.fileId;
              } else {
                wizardState.questions[qIndex].options[optIndex].imageURL = response.url || null;
                wizardState.questions[qIndex].options[optIndex].imageFileId = response.fileId;
              }
            } else {
        ...
        google.script.run
          .withSuccessHandler(async function (response) {
            ...
            if (response && !response.error) {
              ...
              // Update poll data and navigate to dashboard instead of reloading
              completeWizardPublish(response);
            } else {
              await veritasAlert('Failed to update: ' + (response.error || response.message || 'Unknown error'), { title: 'Error', destructive: true });
              if (publishBtn) {
          ...
          .withFailureHandler(async function (error) {
            ...
            await veritasAlert('Error: ' + (error.message || error), { title: 'Error', destructive: true });
            if (publishBtn) {
              publishBtn.disabled = false;
              publishBtn.innerHTML = '<span class="material-symbols-outlined">' + actionIcon + '</span> Update Poll';
            }
          })
          .updatePoll(wizardState.pollId, wizardState.pollName.trim(), wizardState.className.trim(), apiQuestions, metadata);
      } else {
        // Create new poll
        google.script.run
          .withSuccessHandler(async function (response) {
            ...
            if (response && !response.error) {
              ...
              await veritasAlert('Failed to publish: ' + (response.error || response.message || 'Unknown error'), { title: 'Error', destructive: true });
              if (publishBtn) {
                publishBtn.disabled = false;
          ...
          .createNewPoll(wizardState.pollName.trim(), wizardState.className.trim(), apiQuestions, metadata);
      }
    }
        ...
        if (!CURRENT_POLL_DATA || !CURRENT_POLL_DATA.questions) {
            ...
            google.script.run
                .withSuccessHandler(function(fullData) {
                    updateLiveView(fullData);
                })
                .withFailureHandler(handleError)
                .getLivePollData(data.pollId, data.questionIndex);
            return;
        }
