import './LoginManager.js';
  ...
  (function () {
    ...
    try {
      console.log('[Info] Teacher Scripts Initializing...');

      // --- Main App State ---
      // Wrap DOM query in safe block
      ...
      console.error('[Critical] DOM Init Failed:', e);
    }
    ...
    // Pulse Heartbeat - Safe Init
    var missionControlHeartbeat = null;
    try {
      if (window.SecureAssessmentShared && window.SecureAssessmentShared.createHeartbeat) {
        missionControlHeartbeat = window.SecureAssessmentShared.createHeartbeat({
          intervalMs: 2500,
          onBeat: function () {
            if (typeof pollIndividualSessionState === 'function') {
              pollIndividualSessionState();
            }
          }
        });
      } else {
        console.warn('[Warn] SecureAssessmentShared not found, heartbeat disabled');
      }
    } catch (e) {
      console.error('[Error] Heartbeat init failed:', e);
    }
    ...
    function renderRichText(htmlContent) {
      ...
      if (typeof DOMPurify !== 'undefined') {
        cleanHtml = DOMPurify.sanitize(htmlContent, {
          ADD_TAGS: ['math', 'annotation', 'semantics', 'mrow', 'mi', 'mo', 'mn', 'msup', 'sub', 'sup'],
          ADD_ATTR: ['class', 'style', 'src', 'alt', 'width', 'height'] // Ensure images pass through
        });
      }
      return '<div class="ql-editor">' + cleanHtml + '</div>';
    }

    // Helper: Trigger MathJax/KaTeX
    function triggerMathRender() {
      ...
      } else if (window.MathLive) {
        // MathLive auto-render might be needed if using <math-field> read-only, but usually KaTeX handles global render
        // If using MathLive's renderMathInDocument equivalent:
        if (window.MathLive.renderMathInDocument) {
    ...
    // === Quill Rich Text Manager ===
    var QuillManager = {
      instances: {}, // map id -> quill instance
      lastFocusedQuillId: null,

      // Initialize a new editor
      init: function (containerId, initialContent, placeholder, onChange) {
        // Safe check for container
        var container = document.getElementById(containerId);
        if (!container) return null;

        // If already initialized, return existing instance
        // (Though typically we rebuild DOM so this might be a new element with same ID)
        if (this.instances[containerId]) {
          // If the element in DOM is different from the one Quill attached to, we must re-init
          if (this.instances[containerId].container !== container) {
        ...
        // Set initial content
        if (initialContent) {
          if (typeof initialContent === 'object' && initialContent.ops) {
        ...
        quill.on('text-change', function (delta, oldDelta, source) {
          if (onChange) {
            ...
            var html = editor ? editor.innerHTML : quill.root.innerHTML;
            var text = quill.getText();
            var content = quill.getContents();
            onChange({ html: html, text: text, delta: content });
          }
        });

        // Add focus listener to track active editor for Math Symbols
        quill.on('selection-change', function (range, oldRange, source) {
          if (range) {
            QuillManager.lastFocusedQuillId = containerId;
          }
          if (range && range.length === 0) { // Cursor selection
            var [leaf, offset] = quill.getLeaf(range.index);
            if (leaf && leaf.statics.blotName === 'formula') {
              openFormulaModal(quill, range, leaf);
            }
          }
        });
      ...
      clearAll: function () {
        this.instances = {};
        this.lastFocusedQuillId = null;
      },

      insertMathSymbol: function (symbol) {
        if (!this.lastFocusedQuillId) {
          // If no focus tracked, try to find the first visible quill editor
          var keys = Object.keys(this.instances);
          if (keys.length > 0) this.lastFocusedQuillId = keys[0];
          else return;
        }

        var quill = this.instances[this.lastFocusedQuillId];
        if (!quill) return;
    ...
    // ROBUST INIT: Ensure auth flows run even if DOMContentLoaded missed or delayed
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', function () {
        // Redundant safety delay
        setTimeout(initTeacherAuth, 50);
      });
    } else {
      // Document already ready, fire immediately
      console.log('[Info] Document already ready, firing initTeacherAuth immediately');
      setTimeout(initTeacherAuth, 50);
    }
    ...
    // FIREBASE MIGRATION: PREVIEW_MODE is permanently disabled.
    // The application relies exclusively on Firebase.
    const PREVIEW_MODE = false;

    // === Auth & Backend Initialization ===

    // Global Logout Function
    function renderLogoutButton(user) {
      console.log('[Auth] Rendering Logout button for:', user.email);
      var headerRight = document.querySelector('.header-right');
      if (headerRight && !document.getElementById('teacher-logout-btn')) {
        var logoutBtn = document.createElement('button');
        logoutBtn.id = 'teacher-logout-btn';
        logoutBtn.className = 'flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all bg-white/10 hover:bg-white/20 text-white border border-white/20';
        logoutBtn.innerHTML = `
        ...
        // Add user email tooltip
        logoutBtn.title = 'Signed in as ' + (user.email || 'Teacher');
    ...
    function logoutTeacher() {
      if (confirm('Are you sure you want to log out?')) {
        ...
        firebase.auth().signOut().then(function () {
          ...
          sessionStorage.removeItem('veritas_session_token');
          sessionStorage.removeItem('veritas_teacher_email');
          localStorage.removeItem('veritas_teacher_email');

          // Clear global state
          window.SESSION_TOKEN = null;
          window.TEACHER_EMAIL = null;
          window.DATA_LOAD_INITIATED = false;
    ...
    // Map global logout to the new function
    window.logout = logoutTeacher;

    function initializeBackend() {
      console.log('[Veritas] Initializing Firebase backend...');

      if (typeof FIREBASE_CONFIG !== 'undefined' && FIREBASE_CONFIG) {
        if (typeof firebase === 'undefined') {
          console.error('[Firebase] SDK not loaded! Retrying in 500ms...');
          setTimeout(initializeBackend, 500);
          return;
        }
        ...
        if (!firebase.apps.length) {
          try {
            ...
            showToast('error', 'Critical Error', 'Failed to initialize backend. Please refresh.');
            return;
          }
        }
      } else {
        console.error('[Firebase] FIREBASE_CONFIG missing!');
        showToast('error', 'Configuration Error', 'Backend config missing.');
        return;
      }

      // Auth-Bound Data Loading and State Management
      // AUTH_READY flag ensures token is validated before allowing API calls
      window.AUTH_READY = false;

      try {
        firebase.auth().onAuthStateChanged(function (user) {
          if (user) {
            // alert('[DEBUG] Auth Change: User logged in');
            ...
            // UI Updates
            if (window.LoginManager) window.LoginManager.hide();
            ...
            // alert('[DEBUG] Dashboard Unhidden from initializeBackend');

            renderLogoutButton(user);

            // Validate token before allowing API calls
            user.getIdToken().then(function () {
              window.AUTH_READY = true;
              console.log('[Auth] Token validated - API calls enabled');

              // Optimistic Data Loading (Decoupled from verifyTeacher)
              if (!window.DATA_LOAD_INITIATED) {
                window.DATA_LOAD_INITIATED = true;
                loadInitialData();
            ...
            // Verify Teacher Role (Background Check)
            const verifyTeacher = firebase.functions().httpsCallable('verifyTeacher');
            verifyTeacher()
              .then(function (result) {
                var duration = Date.now() - startTime;
                const isTeacher = result.data.isTeacher;

                if (isTeacher) {
                  console.log('[Auth] Teacher authorized:', user.email, '(Verified in ' + duration + 'ms)');
                } else {
                  console.warn('[Auth] Unauthorized teacher access attempt:', user.email, result.data.reason || '');
                  // Optional: Logout if strict enforcement is needed, but for now allow generic auth
                  // firebase.auth().signOut(); 
                }
              })
              .catch(function (error) {
                console.error('[Auth] VerifyTeacher Error:', error);
                // Don't block UI on verification error
              });
            ...
            // Hide Dashboard (redundant but safe)
            var dashboardRoot = document.getElementById('dashboard-root');
            ...
            if (window.LoginManager) {
              ...
              setTimeout(function () {
                if (window.LoginManager) window.LoginManager.show();
              }, 500);
            }
          }
        });
      } catch (e) {
        console.error('[Auth] Error setting up auth listener:', e);
        showToast('error', 'Auth Setup Failed', 'Please refresh the page.');
      }
    }
    ...
    // --- Missing UI Synchronization Functions ---
    function syncStatusFilterButtons() {
      ...
      if (lockedBtn) {
        if (typeof studentStatusFilter !== 'undefined' && studentStatusFilter === 'locked') {
      ...
      if (allBtn) {
        if (typeof studentStatusFilter !== 'undefined' && studentStatusFilter === 'all') {
    ...
    function setDashboardSidebarVisibility(visible) {
      ...
      if (visible) {
        // EXPAND: Restore width, padding, and border
        dashboardSidebar.classList.remove('w-0', 'p-0', 'border-none', 'opacity-0');
        dashboardSidebar.classList.add('w-64', 'p-4', 'border-r', 'flex');
      } else {
        // COLLAPSE: Animate width to 0 and hide contents
        dashboardSidebar.classList.remove('w-64', 'p-4', 'border-r');
        dashboardSidebar.classList.add('w-0', 'p-0', 'border-none', 'opacity-0');
    ...
    var startPollBtn = document.getElementById('start-session-main-btn');
    ...
    function initializeDashboardBindings() {
      try {
        console.log('[App] Initializing dashboard bindings...');
        // Refresh critical element references
        startPollBtn = document.getElementById('start-session-main-btn');
        pollSelect = document.getElementById('poll-select');
        ...
        bindClick('nav-create-poll', function () {
          ...
          if (window.Wizard && window.Wizard.init) {
            ...
            console.error('Wizard system not loaded');
          }
        });
        bindClick('nav-polls-list', function () {
          showDashboard();
          setDashboardSidebarVisibility(true);
        });
        bindClick('nav-rosters', function () {
          showRosterManager();
          setDashboardSidebarVisibility(false);
        });
        bindClick('nav-archived', function () {
          showArchivedView();
          setDashboardSidebarVisibility(false);
        });
        bindClick('nav-analytics', function () {
          showAnalyticsView();
          setDashboardSidebarVisibility(false);
        });
        ...
        bindClick('add-stimulus-btn', function () {
          if (typeof openStimulusModal === 'function') {
            openStimulusModal();
            // Initialize Quill for Stimulus content when modal opens
            setTimeout(function () {
              if (window.QuillManager && QuillManager.init) {
                QuillManager.init('stimulus-content-editor', '', 'Enter passage content...', function (content) {
                });
              }
            }, 100);
          }
        ...
        if (sessionToggleButtons && sessionToggleButtons.length) {
          sessionToggleButtons.forEach(function (btn) {
            btn.addEventListener('click', function () {
              ...
              if (typeof setPollSessionType === 'function') {
        ...
        if (typeof syncSessionTypeToggleUI === 'function') syncSessionTypeToggleUI();
        if (typeof updateSecureSettingsVisibility === 'function') updateSecureSettingsVisibility();
        if (typeof populateSecureSettingsInputs === 'function') populateSecureSettingsInputs();
        ...
        if (startPollBtn) {
          console.log('[DashboardBindings] Found startPollBtn (ID: start-session-main-btn). Binding click event...');
          startPollBtn.removeEventListener('click', onStartPoll);
          startPollBtn.addEventListener('click', onStartPoll);
          console.log('[DashboardBindings] Bound onStartPoll to start-session-main-btn');
        } else {
          console.error('[DashboardBindings] CRITICAL: startPollBtn (ID: start-session-main-btn) NOT FOUND in DOM.');
        }
        ...
        bindClick('live-more-actions-btn', function (e) {
          e.stopPropagation();
          console.log('[LiveUI] Three-dots menu clicked');
          var menu = document.getElementById('live-more-menu');
          if (menu) {
            menu.classList.toggle('hidden');
            console.log('[LiveUI] Menu hidden:', menu.classList.contains('hidden'));
          }
        });
        ...
        document.addEventListener('click', function (e) {
          ...
          // Close Student Inspector Panel
          var panel = document.getElementById('student-inspector-panel');
          // Don't close if clicking inside panel, or if clicking a card (which opens it)
          if (panel && !panel.classList.contains('hidden')) {
            var isClickInside = panel.contains(e.target);
            var isCardClick = e.target.closest('.student-status-card') || e.target.closest('.student-row');
            if (!isClickInside && !isCardClick) {
        ...
        console.log('[DashboardBindings] Bound link action buttons.');

        // Timer presets from dropdown menu
        bindClick('add-30s-btn-preset', function () {
          parseInputToTotalSeconds();
          editTime(30);
          // Close the menu
          var menu = document.getElementById('live-more-menu');
        ...
        bindClick('add-60s-btn-preset', function () {
          parseInputToTotalSeconds();
          editTime(60);
          // Close the menu
          var menu = document.getElementById('live-more-menu');
        ...
        bindClick('filter-all-btn', function () { setStudentFilter('ALL'); });
        bindClick('filter-locked-only-btn', function () { setStudentFilter('LOCKED'); });
        ...
        console.error('[App] Binding Error:', e);
      }
    }

    function setMainSection(section) {
      console.log('[UI] setMainSection called via:', section);
      ...
      // Explicitly fetch all view containers to ensure we aren't using stale globals
      ...
      // Focus mode: collapse sidebar for task views, keep it open for the dashboard
      ...
      function updateUI() {
        // FIX: Fetch header elements for toggling between dashboard and live headers
        ...
        if (section === 'create-poll') {
          if (dashboardRoot) dashboardRoot.style.display = 'none';
          if (createPollView) createPollView.style.display = 'flex';
        } else {
          if (dashboardRoot) dashboardRoot.style.display = 'flex';
          if (createPollView) createPollView.style.display = 'none';
        }

        // FIX: Toggle between default header (dashboard) and live header (live session)
        // This was missing - causing UI to not transition when starting a session
        if (section === 'live') {
        ...
        // Toggle visibility for each main section
        if (dashboardView) dashboardView.style.display = section === 'dashboard' ? 'block' : 'none';
        if (liveView) liveView.style.display = section === 'live' ? 'grid' : 'none'; // Ensure grid for live view
        if (rosterManager) rosterManager.style.display = section === 'roster' ? 'grid' : 'none';
        if (archivedView) archivedView.style.display = section === 'archived' ? 'grid' : 'none';
        if (analyticsView) analyticsView.style.display = section === 'analytics' ? 'flex' : 'none';
        if (individualTimedView) individualTimedView.style.display = section === 'individual-timed' ? 'block' : 'none';
        ...
        // Handle Header Controls Visibility
    ...
    function loadDashboardSummary() {
      // FIREBASE MIGRATION: Firestore listener disabled to prevent permission errors
      // until Firestore is fully configured and indexes are built.
      console.log('[Firebase] loadDashboardSummary: Skipped Firestore listener.');

      // If we have local data, we could render it.
      // renderDashboardSummary([]);
    }
    ...
    function loadInitialData() {
      ...
      // Fetch both Polls and Classes in parallel
      Promise.all([
        db.ref('polls').once('value'),
        db.ref('rosters/classes').once('value')
      ]).then(function (snapshots) {
        var pollsSnap = snapshots[0];
        var classesSnap = snapshots[1];

        var pollsObj = pollsSnap.val() || {};
        var classesArr = classesSnap.val() || [];

        // Convert polls object to array for current frontend format
        // Expected format: array of poll objects
        var pollsArray = Object.keys(pollsObj).map(function (key) {
          var p = pollsObj[key];
          p.id = key; // Ensure ID is set
          return p;
        });

        // Sort by updatedAt descending
        pollsArray.sort(function (a, b) {
          return (b.updatedAt || 0) - (a.updatedAt || 0);
        });

        console.log('[Firebase] Loaded ' + pollsArray.length + ' polls and ' + classesArr.length + ' classes.');
        ...
        console.error('[Firebase] Failed to load initial data:', error);
        showToast('error', 'Data Load Error', 'Failed to load polls. Please refresh.');
      });
    }
    ...
    function startEditPoll(pollId, secondArg) {
      if (!pollId) return;
      var button = (secondArg && secondArg.tagName) ? secondArg : null;
      var startQuestionIndex = (typeof secondArg === 'number') ? secondArg : null;
      ...
      if (PREVIEW_MODE) {
        ...
        if (previewPoll) {
          ...
          openPollWizard(deepClone(previewPoll), startQuestionIndex);
        } else {
      ...
      db.ref('polls/' + pollId).once('value')
        .then(function (snapshot) {
          ...
          if (poll) {
            ...
            poll.pollId = pollId;
            openPollWizard(poll, startQuestionIndex);
          } else {
    ...
    function renderPollCards(polls) {
      ...
      polls.forEach(function (poll) {
        try {
          ...
          htmlParts.push('<button type="button" class="flex-1 poll-card-btn primary poll-preview-btn" data-poll-id="' + (poll && poll.pollId ? poll.pollId : '') + '" aria-label="Preview & Edit ' + safeName + '"><span class="material-symbols-outlined text-base">visibility</span>Preview & Edit</button>');
    ...
    async function handleCopyPoll(pollId) {
      ...
      // Call Cloud Function to create a new poll (copy)
      var createPollFn = firebase.functions().httpsCallable('createPoll');

      createPollFn(payload).then(function (result) {
        if (result.data && result.data.success) {
          veritasAlert('Poll copied successfully.', { title: 'Success' });
          // The dashboard listener will automatically refresh ALL_POLLS
        } else {
          throw new Error(result.data ? result.data.message : 'Failed to copy poll');
        }
      }).catch(function (error) {
        handleError(error);
      });
    ...
    function onPollSelectChange() {
      console.log('[onPollSelectChange] Triggered.');
      // Refresh all dashboard control references
      startPollBtn = document.getElementById('start-session-main-btn');
      sendLinkBtn = document.getElementById('send-link-btn');
      viewLinksBtn = document.getElementById('view-links-btn');
      pollSelect = document.getElementById('poll-select');

      if (pollSelect) console.log('[onPollSelectChange] Current value:', pollSelect.value);
      ...
      console.log('[onPollSelectChange] Poll:', pollId, 'Found:', hasSelection, 'Class:', className);
    ...
    // --- Roster Management ---
    function ensureFunctionsCallable(name) {
      var functionsInstance = firebaseFunctions;

      if (!functionsInstance) {
        // Try to initialize if missing
        try {
          functionsInstance = firebaseFunctions = firebase.functions();
        } catch (e) {
          console.error('[Firebase] Functions SDK not initialized:', e);
        }
      }

      if (!functionsInstance) {
        return Promise.reject(new Error('Backend is unavailable. Please refresh and sign in again.'));
      }

      // V2 SDK handles auth token automatically if user is signed in
      return Promise.resolve({
        callable: functionsInstance.httpsCallable(name)
      });
    }

    function callManageRoster(payload) {
      return ensureFunctionsCallable('manageRoster').then(function (ctx) {
        // No need to inject authToken manualy for V2 callables
        return ctx.callable(payload || {});
      });
    }
    ...
    function loadRosterData() {
      ...
      // Check AUTH_READY flag to ensure token is validated before making API calls
      if (!firebase.auth().currentUser || !window.AUTH_READY) {
        rosterStatus.textContent = '';
        if (!firebase.auth().currentUser) {
          veritasAlert('You must be authenticated to load rosters.', { title: 'Authentication required', destructive: true });
          if (window.LoginManager) {
            window.LoginManager.show();
          }
        } else {
          // Auth in progress - retry after a short delay
          console.log('[Firebase] Auth token not ready, retrying loadRosterData in 500ms...');
          setTimeout(loadRosterData, 500);
        }
        return;
      }

      callManageRoster({ action: 'GET_DATA' }).then(function (result) {
    ...
    async function saveRoster() {
      ...
      // FIREBASE MIGRATION: Save Roster Class via Cloud Function
      callManageRoster({
        action: 'SAVE',
        ...
        onRosterDataLoaded({
          rosters: rosterData,
          classes: ALL_CLASSES.includes(selectedRosterClass)
            ? ALL_CLASSES
            : ALL_CLASSES.concat([selectedRosterClass])
        }, selectedRosterClass);
      }).catch(function (error) {
        saveRosterBtn.disabled = false;
        handleError(error);
    ...
    async function confirmBulkAddStudents() {
      ...
      // FIREBASE MIGRATION: Bulk Add via Cloud Function
      callManageRoster({
        action: 'BULK_ADD',
        className: selectedRosterClass,
        students: entries
    ...
    function loadArchivedPolls(force) {
      ...
      console.warn('[Firebase] loadArchivedPolls: Legacy RTDB history disabled.');
      archivedStatus.textContent = 'Archived polls are temporarily unavailable.';
      return;
    ...
    async function onStartPoll(e) {
      console.log('[onStartPoll] EVENT TRIGGERED. Element:', e && e.target ? e.target.id : 'N/A');
      if (e) e.preventDefault();

      if (!pollSelect) pollSelect = document.getElementById('poll-select');
      console.log('[onStartPoll] Current pollSelect value:', pollSelect ? pollSelect.value : 'N/A');
      try {
        var pollId = pollSelect.value;
        if (!pollId) {
          console.warn('[onStartPoll] No poll selected.');
        ...
        console.log('[onStartPoll] Selected Poll found:', !!selectedPoll, selectedPoll);

        if (!selectedPoll) {
          console.error('[onStartPoll] Could not find poll data for ID:', pollId);
          await veritasAlert('Could not find the selected poll data.', { title: 'Error' });
          setButtonLoading(startPollBtn, false);
          return;
        }

        // FIX: Ensure global current poll data is set before starting session
        ...
        if (isSecureSessionTypeClient(selectedPoll.sessionType)) {
          ...
          if (PREVIEW_MODE) {
            console.log('[onStartPoll] Preview mode active.');
            setButtonLoading(startPollBtn, false);
          ...
          ensureFunctionsCallable('setLiveSessionState').then(function (ctx) {
            ...
            console.log('[onStartPoll] secure: callable result:', result);
            var data = result.data;
            setButtonLoading(startPollBtn, false);
            if (data && data.success) {
            ...
            console.error('[onStartPoll] secure: error:', error);
            setButtonLoading(startPollBtn, false);
            handleError(error);
          });
          return;
        ...
        console.log('[onStartPoll] Starting Live Session...');
        ...
        ensureFunctionsCallable('setLiveSessionState').then(function (ctx) {
          ...
          console.log('[onStartPoll] live: callable result:', result);
          setButtonLoading(startPollBtn, false);
          var res = result.data;
          if (res && res.success) {
          ...
          console.error('[onStartPoll] live: error:', error);
          setButtonLoading(startPollBtn, false);
          handleError(error);
        });

      } catch (e) {
        console.error('[onStartPoll] Synchronous error:', e);
        if (typeof setButtonLoading === 'function' && startPollBtn) {
          setButtonLoading(startPollBtn, false);
        }
        handleError(e);
      }
    ...
    function onEndQuestionAndShowAnswer() {
      ...
      // Update Status via Cloud Function
      // We do NOT use finalizeSession here, as that ends the ENTIRE poll.
      // We just want to reveal the answer for the current question.

      var setSessionFn = firebase.functions().httpsCallable('setLiveSessionState');
      var pollId = CURRENT_POLL_DATA.pollId;
      var qIndex = CURRENT_POLL_DATA.questionIndex;
      var currentVisibility = (CURRENT_POLL_DATA.resultsVisibility || 'HIDDEN');
      var isRevealed = currentVisibility === 'REVEALED';

      var targetStatus = isRevealed ? 'OPEN' : 'REVEAL'; // Toggle logic
      // If hiding, we might want 'PAUSED' instead of OPEN if timer was 0, but OPEN is safer resume state.
      // Actually 'OPEN' restarts the flow. 
      // If we want to just "Hide Answer" but keep session active/paused:
      // Maybe set metadata `resultsVisibility: 'HIDDEN'`.

      // Let's stick to status 'REVEAL' vs 'OPEN' for simplicity, assuming 'OPEN' just shows the question again.
    ...
    function updateLiveOverviewFromData(data) {
      ...
      // Update Metacognition (Confidence) Index
      var hudMetacognitionValue = document.getElementById('hud-metacognition-value');
      if (hudMetacognitionValue) {
        if (!data || !data.studentStatuses || countedResponses === 0) {
          hudMetacognitionValue.textContent = '--';
        } else {
          var totalConf = 0;
          var confCount = 0;
          var confScores = { 'guessing': 1, 'somewhat-sure': 2, 'very-sure': 3, 'certain': 4 };

          Object.values(data.studentStatuses).forEach(function (s) {
            if (s.confidence && confScores[s.confidence]) {
              totalConf += confScores[s.confidence];
              confCount++;
            }
          });

          if (confCount > 0) {
            var avgConf = (totalConf / confCount).toFixed(1);
            hudMetacognitionValue.textContent = avgConf + '/4';
          } else {
            hudMetacognitionValue.textContent = '--';
          }
        }
    ...
    function updateLiveView(data) {
      ...
      if (!isLiveSession && !isPreLive) {
        if (status === 'LIVE' || status === 'OPEN') {
          console.log('[updateLiveView] Transitioning to LIVE session state');
          isLiveSession = true;
        } else {
          return;
        }
      ...
      // FIX: Always ensure UI is in live view when status is LIVE/OPEN
      // This was previously inside the isLiveSession check, causing race conditions
      if (status === 'LIVE' || status === 'OPEN') {
        setMainSection('live');
      }
      ...
      safeSetDisabled('header-prev-btn', data.questionIndex <= 0); // Visible Prev button

      var isLastQuestion = data.questionIndex >= data.totalQuestions - 1;
      safeSetDisabled('header-next-btn', isLastQuestion);
      safeSetDisabled('mobile-next-btn', isLastQuestion);
      ...
      if (isResultsRevealed) {
        ...
        safeSetDisplay('header-start-btn', 'none');
        safeSetDisplay('header-end-show-btn', 'none');
        ...
        safeSetDisplay('header-start-btn', 'flex');
        safeSetDisplay('header-end-show-btn', 'flex');
      ...
      safeSetText('live-question-info', 'Q' + (data.questionIndex + 1) + '/' + data.totalQuestions);
      // Use safeSetInnerHTML explicitly for known rich text fields
      safeSetInnerHTML('live-question-text', data.questionText);
      safeSetInnerHTML('live-question-text-compact', data.questionText);

      // Handle question image with cache-busting
      var questionImageContainer = document.getElementById('question-image-container');
      var questionImageEls = document.querySelectorAll('[id="live-question-image"]');
      var questionLayoutContainer = document.getElementById('question-layout-container');
      if (data.questionImageURL) {
        if (questionImageContainer) questionImageContainer.style.display = 'flex';
        ...
        questionImageEls.forEach(function (el) {
          el.src = imageUrl + separator + cacheBuster;
          el.style.display = 'block';
          el.setAttribute('referrerpolicy', 'no-referrer');
        });
      } else {
        if (questionImageContainer) questionImageContainer.style.display = 'none';
        questionImageEls.forEach(function (el) { el.src = ''; });
    ...
    function jumpToQuestion(index) {
      ...
      if (index === CURRENT_POLL_DATA.questionIndex) return;
      ...
      timerRemainingSeconds = timerPresetSeconds;
      updateTimerDisplay();
      updateTimerStatus('Navigating...', 'info');
      isPaused = false;
      isNavigating = true;
    ...
    function updateQuestionProgress(currentLive, total, currentLocal) {
      ...
      // Limit number of circles to prevent overflow in header
      var maxVisible = 10;
      var startIdx = 0;
      var endIdx = total;
      ...
      if (startIdx > 0) {
        ...
        ellipsis.className = 'text-white/30 text-xs mx-1';
      ...
      if (endIdx < total) {
        ...
        ellipsisEnd.className = 'text-white/30 text-xs mx-1';
    ...
    function renderResultsBars(results, correctAnswer, studentsList) {
      ...
      optionDefs.forEach(function (option, index) {
        ...
        if (answerText) {
          // Use renderRichText for proper formatting of math/bold in results
          cardHtml += '<div class="text-[1.125rem] font-medium text-brand-dark-gray dark:text-brand-white break-words leading-relaxed ql-editor-reset">' + renderRichText(answerText) + '</div>';
        } else if (!imageURL) {
      ...
      // Render any math formulas in the newly added cards
      triggerMathRender();
    }
    ...
    function renderLiveMetacognition(metacognition) {
      ...
      var matrixDefs = [
        ...
        { key: 'confidentIncorrect', emoji: '⚠️', label: 'Sure & Wrong', barClass: 'bg-rose-500', textClass: 'text-rose-700 dark:text-rose-200' },
    ...
    function buildStudentStatusTile(student) {
      if (!student) return null;
      var name = formatStudentName(student);
      var normalizedStatus = deriveProctorStatus(student);
      ...
      // Card Container
      var card = document.createElement('div');
      var cardClasses = 'relative group p-4 rounded-xl border transition-all duration-200 hover:shadow-md';

      // Status Coloring
      if (isLocked || isAwaitingFullscreen) {
        cardClasses += ' bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-800/30';
      } else if (isBlocked) {
        cardClasses += ' bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-600 opacity-60';
      } else if (student.status === 'Submitted') {
        if (isCorrect) {
          cardClasses += ' bg-emerald-50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800/30';
        } else if (student.isCorrect === false) {
          cardClasses += ' bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800/30';
        } else {
      ...
      // Status Copy Logic
      var displayStatus = 'INVITED';
      var rawStatus = (student.status || '').toUpperCase();

      if (isLocked) displayStatus = 'VIOLATION DETECTED';
      else if (isBlocked) displayStatus = 'BLOCKED';
      else if (isAwaitingFullscreen) displayStatus = 'ALLOW RE-ENTRY';
      else if (rawStatus === 'SUBMITTED') displayStatus = 'ANSWERED';
      else if (rawStatus === 'IN PROGRESS') displayStatus = 'WORKING...';
      else if (rawStatus === 'ACTIVE') displayStatus = 'THINKING...';
      else if (rawStatus === 'CONNECTED') displayStatus = 'JOINED';
      else if (rawStatus === 'TYPING') displayStatus = 'TYPING...';

      // Header: Name & Status Dot
      // Layout: Name takes available space, Dot is fixed on right, preventing overlap
      var html = '<div class="flex items-start justify-between mb-2 gap-2">';

      // Left: Name and Status Text
      html += '<div class="flex flex-col min-w-0 flex-1">';
      html += '<span class="font-bold text-sm text-gray-900 dark:text-white truncate" title="' + escapeHtml(name) + '">' + escapeHtml(name) + '</span>';
      html += '<span class="text-[10px] uppercase font-bold tracking-wide text-gray-400 dark:text-gray-500 truncate">' + escapeHtml(displayStatus) + '</span>';
      html += '</div>';

      // Right: Status Dot (Fixed)
      var dotColor = 'bg-gray-300';
      if (isLocked) dotColor = 'bg-red-500 animate-pulse';
      else if (student.status === 'Submitted') dotColor = 'bg-emerald-500';
      else if (rawStatus === 'IN PROGRESS' || rawStatus === 'TYPING') dotColor = 'bg-indigo-400 animate-pulse';
      else if (rawStatus === 'ACTIVE' || rawStatus === 'CONNECTED') dotColor = 'bg-blue-400';

      html += '<span class="flex-shrink-0 w-2.5 h-2.5 rounded-full shadow-sm mt-1 ' + dotColor + '"></span>';
      html += '</div>';

      // Footer: Answer & Confidence
      html += '<div class="flex items-center justify-between border-t border-gray-100 dark:border-gray-700/50 pt-3 mt-1">';

      // Answer Display
      var answerDisplay = student.answer ? escapeHtml(student.answer) : '<span class="text-gray-300 italic">Thinking...</span>';
      var answerClass = 'text-gray-900 dark:text-white';
      if (student.status === 'Submitted') {
        if (isCorrect) answerClass = 'text-emerald-700 dark:text-emerald-300';
        else if (student.isCorrect === false) answerClass = 'text-amber-700 dark:text-amber-300 line-through decoration-amber-500/50';
      }

      html += '<div class="flex flex-col">';
      html += '<span class="text-[10px] font-mono text-gray-400 mb-0.5">ANSWER</span>';
      html += '<span class="text-sm font-bold ' + answerClass + ' truncate max-w-[100px]">' + answerDisplay + '</span>';
      html += '</div>';

      // Confidence Visualization
      if (student.confidence) {
        var confStyle = CONFIDENCE_LEVEL_STYLES[student.confidence] || {};
        var confEmoji = confStyle.emoji || '•';
        var confColor = confStyle.textClass || 'text-gray-400';
        html += '<div class="flex flex-col items-end" title="' + (confStyle.label || '') + '">';
        html += '<span class="text-[10px] font-mono text-gray-400 mb-0.5">CONFIDENCE</span>';
        html += '<span class="text-lg ' + confColor + '">' + confEmoji + '</span>';
        html += '</div>';
      }
      ...
      var showOverlay = isLocked || student.status === 'Submitted' || student.status === 'Waiting...';

      if (showOverlay) {
        html += '<div class="absolute inset-0 bg-white/95 dark:bg-gray-800/95 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 rounded-xl backdrop-blur-[2px] z-10 pointer-events-none">';
        html += '<span class="text-xs font-bold text-gray-500 mb-1">ACTIONS</span>';
        html += '<div class="action-btn-container flex gap-2 pointer-events-auto"></div>';
        html += '</div>';
      }
      ...
      // Inject Buttons
      if (showOverlay) {
        var btnContainer = card.querySelector('.action-btn-container');
        if (btnContainer) {
          if (isLocked) {
            var unlockBtn = createUnlockButton(student);
            unlockBtn.className = 'px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded shadow-sm';
            unlockBtn.innerHTML = 'UNLOCK';
            btnContainer.appendChild(unlockBtn);
          }

          if (student.status === 'Submitted') {
            var resetBtn = createResetButton(student);
            resetBtn.className = 'px-3 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 text-xs font-bold rounded shadow-sm';
    ...
    // Global filter state
    var activeStudentFilter = 'ALL'; // 'ALL' or 'LOCKED'

    function setStudentFilter(filter) {
      if (activeStudentFilter === filter) return;
      activeStudentFilter = filter;

      // Update UI button states immediately
      var allBtn = document.getElementById('filter-all-btn');
      var lockedBtn = document.getElementById('filter-locked-only-btn');

      if (allBtn) {
        if (filter === 'ALL') {
          allBtn.className = 'px-3 py-1 text-xs font-bold rounded-md bg-white dark:bg-gray-600 text-gray-800 dark:text-white shadow-sm transition-all';
        } else {
          allBtn.className = 'px-3 py-1 text-xs font-bold rounded-md text-gray-500 hover:text-gray-700 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600 transition-all';
        }
      }

      if (lockedBtn) {
        if (filter === 'LOCKED') {
          lockedBtn.className = 'px-3 py-1 text-xs font-bold rounded-md bg-white dark:bg-gray-600 text-red-600 dark:text-red-400 shadow-sm transition-all flex items-center gap-1';
        } else {
          lockedBtn.className = 'px-3 py-1 text-xs font-bold rounded-md text-gray-500 hover:text-gray-700 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600 transition-all flex items-center gap-1';
        }
      }
    ...
    function renderStudentStatus() {
      // Find the container based on ID
      var grid = document.getElementById('student-status-grid');
      // If we are in the new view, this is the main container.
      if (!grid) return;

      // Filter Logic (Simplified for card view)
      var rawStudents = Array.isArray(currentStudentStatusData) ? currentStudentStatusData : [];
      var students = [];

      if (activeStudentFilter === 'LOCKED') {
        students = rawStudents.filter(function (s) {
          var status = deriveProctorStatus(s);
          return status === 'LOCKED' || status === 'BLOCKED';
        });
      } else {
        students = rawStudents.slice();
      }

      // Filter: Show LOCKED/BLOCKED always
      // Filter: If submitted, show submitted + locked/blocked

      // Simplify logic for High Fidelity: Just show everyone sorted by status
      // Priority: Locked > Active (Typing) > Submitted > Idle

      students.sort(function (a, b) {
        // Status Priority
        var getRank = function (s) {
          var norm = deriveProctorStatus(s);
          if (norm === 'LOCKED' || norm === 'BLOCKED') return 0;
          if (s.status === 'Submitted') {
            if (s.isCorrect === true) return 3; // Correct at bottom
            return 2; // Incorrect/Submitted
          }
      ...
      students.forEach(function (student) {
        var card = buildStudentStatusTile(student); // Using new card function
        if (card) {
          card.style.cursor = 'pointer';
          card.onclick = function () {
            openStudentInspector(student.email);
          };
      ...
      var submittedCount = students.filter(s => s.status === 'Submitted').length;
      var correctCount = students.filter(s => s.status === 'Submitted' && s.isCorrect === true).length;
      var lockedCount = students.filter(s => deriveProctorStatus(s) === 'LOCKED').length;
      var waitingCount = students.filter(s => s.status !== 'Submitted').length;
      var activeCount = students.length;

      // Accuracy
      var accuracy = submittedCount > 0 ? Math.round((correctCount / submittedCount) * 100) : 0;
      var accEl = document.getElementById('hud-accuracy-value');
      if (accEl) accEl.textContent = submittedCount > 0 ? accuracy + '%' : '--%';

      // Response Rate
      var respEl = document.getElementById('hud-response-count');
      if (respEl) respEl.textContent = submittedCount + '/' + activeCount;

      // Confidence Index (0-100)
      var confTotal = 0;
      var confCount = 0;
      students.forEach(function (s) {
        if (s.status === 'Submitted' && s.confidence) {
          if (s.confidence === 'guessing') confTotal += 0;
          else if (s.confidence === 'somewhat-sure') confTotal += 50;
          else if (s.confidence === 'very-sure') confTotal += 100;
          confCount++;
        }
      });
      var avgConf = confCount > 0 ? Math.round(confTotal / confCount) : null;
      var metaEl = document.getElementById('hud-metacognition-value');
      if (metaEl) {
        if (avgConf !== null) {
          metaEl.textContent = avgConf;
          // Color code
          metaEl.className = 'text-xl font-bold ' +
            (avgConf >= 80 ? 'text-emerald-600' :
              (avgConf >= 50 ? 'text-amber-600' : 'text-red-500'));
        } else {
          metaEl.textContent = '--';
          metaEl.className = 'text-xl font-bold text-gray-400';
        }
      }

      updateLiveOverviewCounts(lockedCount, 0, waitingCount);
    }

    function openStudentInspector(email) {
      var student = currentStudentStatusData.find(function (s) { return s.email === email; });
      if (!student) return;
      ...
      var name = formatStudentName(student);
      var status = deriveProctorStatus(student);
      var isLocked = status === 'LOCKED';
      var isBlocked = status === 'BLOCKED';

      var statusColor = isLocked ? 'text-red-600 bg-red-50' : (isBlocked ? 'text-gray-600 bg-gray-100' : 'text-emerald-600 bg-emerald-50');
      ...
      var html = `
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-3">
              ${isLocked ? `
              <button onclick="proctorAction('UNLOCK', '${email}')" class="flex items-center justify-center gap-2 p-3 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 transition-all shadow-md">
              ...
              ${!isBlocked ? `
              <button onclick="proctorAction('BLOCK', '${email}')" class="flex items-center justify-center gap-2 p-3 bg-red-50 text-red-600 border border-red-200 rounded-lg font-bold hover:bg-red-100 transition-all">
                <span class="material-symbols-outlined">block</span>
                <span>Block Student</span>
              </button>` : `
              <button onclick="proctorAction('UNBLOCK', '${email}')" class="flex items-center justify-center gap-2 p-3 bg-gray-100 text-gray-700 border border-gray-300 rounded-lg font-bold hover:bg-gray-200 transition-all">
          ...
          <div class="space-y-2">
            ...
            <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg text-sm space-y-2 border border-gray-200 dark:border-gray-700">
              <div class="flex justify-between">
                ...
                <span class="text-gray-900 dark:text-white font-mono">${escapeHtml(student.email)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500">Attempt Count</span>
                <span class="text-gray-900 dark:text-white font-mono">${student.attemptCount || 0}</span>
              </div>
              ...
              <div class="flex justify-between">
                ...
                <span class="text-gray-900 dark:text-white font-mono">${student.timeOnQuestionMs ? (Math.round(student.timeOnQuestionMs / 1000) + 's') : '--'}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500">Calculator Used</span>
                <span class="font-bold border px-1.5 rounded text-xs ${student.usingCalculator ? 'text-amber-600 bg-amber-50 border-amber-200' : 'text-gray-400 border-gray-200'}">
                  ${student.usingCalculator ? 'YES' : 'NO'}
                </span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500">Confidence</span>
                <span class="text-gray-900 dark:text-white">${student.confidence ? student.confidence.replace('-', ' ').toUpperCase() : '--'}</span>
              </div>
               ...
               <div class="flex justify-between">
                ...
                <span class="text-red-600 font-mono text-xs">${escapeHtml(student.lastViolationReason || 'Unknown Violation')}</span>
               </div>
               ` : ''}
    ...
    function proctorAction(action, email) {
      ...
      if (!CURRENT_POLL_DATA || !CURRENT_POLL_DATA.pollId) {
        console.error('[Proctor] No active poll data found.');
        showToast('Error: No active poll found', 'error');
        return;
      }

      var manageProctoringFn = firebase.functions().httpsCallable('manageProctoring');
      manageProctoringFn({
        action: action,
        pollId: CURRENT_POLL_DATA.pollId,
        studentEmail: email // FIX: Backend expects 'studentEmail'
      })
        .then(function (result) {
          console.log('[Proctor] Action success:', result);
          showToast('Action ' + action + ' successful', 'success');
          closeStudentInspector();
        })
        .catch(function (error) {
          console.error('[Proctor] Action failed:', error);
          handleError(error);
        });
    }
    ...
    function updateDisplay() {
      ...
      // Only update title with timer when timer is actively running
      if (isRunning) {
     ...
     */
    function startTimer() {
      console.log('[Timer] startTimer called. isRunning:', isRunning, 'totalSeconds:', totalSeconds);
      if (isRunning || totalSeconds <= 0) {
        console.log('[Timer] Cannot start - already running or no time');
        return;
      }
    ...
    /**
     * Generates the HTML email body for a student.
     */
    function generateEmailHtml(studentName, link, isSecure, studentEmail) {
      var titleText = isSecure ? "Secure Assessment Session" : "Live Poll Session";
      var introText = isSecure ?
        "Your personalized link for today's secure assessment is ready. Please click the button below to begin." :
        "Your personalized link for today's live poll is ready. Please click the button below to begin.";
...
<body>
    <div class="container">
        ...
        <div style="background-color: #12385d; padding: 20px 30px;">
            ...
            <span style="color: #e5e7eb; font-size: 22px;">${titleText}</span>
        </div>
        <div class="content-padding">
            <p>Hello <strong>${studentName}</strong>,</p>
            <p>${introText}</p>
            <div style="text-align: center;">
                <a href="${link}" class="btn">Begin Session</a>
            </div>
            <hr class="divider">
            ...
            <div class="footer">
                <p style="margin-bottom: 8px;">🔒 This is a unique link for <strong>${studentEmail}</strong>. Do not share it.</p>
    ...
    async function onSendLink() {
      console.log('[onSendLink] Triggered');
      var btn = sendLinkBtn || document.getElementById('send-link-btn');
      if (!btn) {
      ...
      // FIX: Ensure links point to the student view, not the teacher dashboard
      var baseUrl = window.location.origin + '/student.html';
      ...
      callManageRoster({
        ...
        // Map payload.links to payload.students with generated bodies
        var isSecure = (CURRENT_POLL_DATA.sessionType || "").toUpperCase().includes("SECURE");
        ...
        payload.students = payload.links.map(function (s) {
          ...
          return {
            email: s.email,
            name: s.name,
            subject: payload.subject, // Use the subject from manageRoster
            body: htmlBody
          };
        });

        // FIREBASE MIGRATION: Send emails via Cloud Function
        return firebase.functions().httpsCallable('sendEmail')(payload)
          .then(function (result) {
            ...
            if (response.success) {
              veritasAlert('Emails processed for ' + response.sentCount + ' students.', { title: 'Emails Processed' });
            } else {
    ...
    // --- REDESIGNED: Poll Editor & Preview Functions ---
    var previewModal = document.getElementById('poll-preview-modal');
    var editorSidebar = document.getElementById('editor-sidebar');
    ...
    var editorQuestionInputContainer = document.getElementById('editor-question-input');
    var editorTimerInput = document.getElementById('editor-timer-input');
    var editorImgPreviewContainer = document.getElementById('editor-img-preview-container');
    var editorImgThumbnail = document.getElementById('editor-img-thumbnail');
    var editorOptionsList = document.getElementById('editor-options-list');

    // Preview Pane Elements
    var truePreviewContainer = document.getElementById('true-preview-container');
    var previewStudentStem = document.getElementById('preview-student-stem');
    var previewStudentVisual = document.getElementById('preview-student-visual');
    var previewStudentOptions = document.getElementById('preview-student-options');

    var currentPreviewPoll = null;
    var currentPreviewQuestionIndex = 0;
    var editorQuill = null;
    var isPreviewDirty = false;
    var draggedItemIndex = null;

    function openPollPreview(pollId) {
      var poll = ALL_POLLS.find(function (p) { return p.pollId === pollId; });
      if (!poll) {
        showToast('error', 'Error', 'Poll not found.');
        return;
      }

      // Deep copy to avoid mutating the global state until save
      // JSON.parse/stringify is simple way to deep copy for this data structure
      currentPreviewPoll = JSON.parse(JSON.stringify(poll));

      if (!currentPreviewPoll.questions) currentPreviewPoll.questions = [];
      if (currentPreviewPoll.questions.length === 0) {
        // Auto-add first question if empty
        currentPreviewPoll.questions.push(createNewQuestionTemplate());
      }
    ...
    function handlePreviewKeydown(e) {
      if (e.key === 'Escape') closePollPreview();
      // Add more shortcuts if needed (Ctrl+S to save?)
    ...
    function createNewQuestionTemplate() {
      return {
        questionText: 'New Question',
        options: [
          { text: 'Option A', imageURL: '' },
          { text: 'Option B', imageURL: '' },
          { text: 'Option C', imageURL: '' },
          { text: 'Option D', imageURL: '' }
        ],
        correctAnswer: 'A',
        timerSeconds: 60,
        questionImageURL: ''
    ...
    function updatePreviewMeta() {
      var count = currentPreviewPoll.questions.length;
      var classText = currentPreviewPoll.className || 'No Class';
      previewPollMeta.textContent = classText + ' • ' + count + ' Question' + (count !== 1 ? 's' : '');
      document.getElementById('preview-question-count-badge').textContent = count;
    }
    ...
    function renderPreviewNav() {
      ...
      currentPreviewPoll.questions.forEach(function (q, index) {
        ...
        el.className = 'group flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all border border-transparent ' +
          (index === currentPreviewQuestionIndex ? 'bg-veritas-navy/5 border-veritas-navy/10 shadow-sm' : 'hover:bg-gray-50 dark:hover:bg-gray-800');
        el.draggable = true;

        // Drag Handle
        var handle = document.createElement('span');
        handle.className = 'material-symbols-outlined text-gray-400 text-lg cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity';
        handle.textContent = 'drag_indicator';

        // Index Number
        var num = document.createElement('span');
        num.className = 'text-xs font-bold w-5 h-5 flex items-center justify-center rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 ' + (index === currentPreviewQuestionIndex ? 'bg-veritas-navy text-white dark:bg-veritas-navy dark:text-white' : '');
        num.textContent = index + 1;

        // Text Preview
        var text = document.createElement('div');
        text.className = 'flex-1 min-w-0';
        // Strip HTML for sidebar preview
        var tmp = document.createElement('div');
        tmp.innerHTML = q.questionText || 'New Question';
        var plainText = tmp.textContent || tmp.innerText || 'New Question';

        text.innerHTML = '<p class="text-xs font-semibold text-gray-700 dark:text-gray-200 truncate">' + escapeHtml(plainText) + '</p>';
        ...
        el.addEventListener('drop', function (e) {
          ...
          if (draggedItemIndex !== null && draggedItemIndex !== index) {
            // Reorder
            var item = currentPreviewPoll.questions.splice(draggedItemIndex, 1)[0];
            currentPreviewPoll.questions.splice(index, 0, item);
        ...
        previewQuestionsNav.appendChild(el);
      });

      // Scroll to active
      // setTimeout(() => {
      //     var active = previewQuestionsNav.children[currentPreviewQuestionIndex];
      //     if(active) active.scrollIntoView({block: 'nearest', behavior: 'smooth'});
      // }, 50);
    }

    /* --- Editor Logic --- */
    function initEditorQuill() {
      if (!editorQuill) {
        editorQuill = QuillManager.init('editor-question-input', '', 'Type your question here...', function (data) {
          if (!currentPreviewPoll) return;
          currentPreviewPoll.questions[currentPreviewQuestionIndex].questionText = data.html;
          updateTruePreview();
          renderPreviewNav(); // Update sidebar text
          isPreviewDirty = true;
        });
      }
    }
    ...
    function loadQuestionIntoEditor(index) {
      currentPreviewQuestionIndex = index;
      var q = currentPreviewPoll.questions[index];

      // Update Sidebar Highlight
      var items = previewQuestionsNav.children;
      for (var i = 0; i < items.length; i++) {
        if (i === index) {
          items[i].classList.add('bg-veritas-navy/5', 'border-veritas-navy/10');
          items[i].classList.remove('hover:bg-gray-50');
          items[i].querySelector('span.w-5').classList.add('bg-veritas-navy', 'text-white');
        } else {
          items[i].classList.remove('bg-veritas-navy/5', 'border-veritas-navy/10');
          items[i].classList.add('hover:bg-gray-50');
          items[i].querySelector('span.w-5').classList.remove('bg-veritas-navy', 'text-white');
        }
      }

      document.getElementById('editor-current-num').textContent = index + 1;

      // Quill Content
      if (editorQuill) {
        var delta = editorQuill.clipboard.convert(q.questionText || '');
        editorQuill.setContents(delta, 'silent');
      }

      // Timer
      editorTimerInput.value = q.timerSeconds || 60;
      editorTimerInput.onchange = function () {
        q.timerSeconds = parseInt(this.value) || 60;
        isPreviewDirty = true;
      };

      // Image
      if (q.questionImageURL) {
        editorImgThumbnail.src = q.questionImageURL;
        editorImgPreviewContainer.classList.remove('hidden');
      } else {
        editorImgPreviewContainer.classList.add('hidden');
      }
    ...
    function renderEditorOptions() {
      ...
      q.options.forEach(function (opt, i) {
        ...
        row.className = 'flex flex-col gap-2 bg-white dark:bg-brand-dark-gray p-3 rounded-lg border ' +
          (isCorrect ? 'border-green-500 ring-1 ring-green-500/20' : 'border-gray-200 dark:border-gray-700');

        // Top Row: Radio, Text, Actions
        var topRow = document.createElement('div');
        topRow.className = 'flex items-center gap-3 w-full';

        // Correct Answer Radio
        var radioContainer = document.createElement('label');
        radioContainer.className = 'flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full border cursor-pointer transition-colors ' +
          (isCorrect ? 'bg-green-500 border-green-500 text-white' : 'bg-gray-50 border-gray-300 hover:border-gray-400 text-gray-400');
        radioContainer.innerHTML = '<span class="text-sm font-bold">' + letter + '</span>' +
          '<input type="radio" name="correct_answer" class="hidden" ' + (isCorrect ? 'checked' : '') + '>';
        ...
        textInput.placeholder = 'Answer option ' + letter;
        textInput.className = 'flex-1 min-w-0 px-3 py-1.5 rounded-md border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-veritas-navy/50 outline-none text-sm';
        ...
        // Actions: Image, Remove
        var actionsDiv = document.createElement('div');
        actionsDiv.className = 'flex items-center gap-1';

        // Image Upload Btn
        var imgBtn = document.createElement('label');
        imgBtn.className = 'p-1.5 text-gray-400 hover:text-veritas-navy cursor-pointer transition-colors rounded hover:bg-gray-100';
        imgBtn.title = 'Add Image';
        imgBtn.innerHTML = '<span class="material-symbols-outlined text-lg">image</span>' +
          '<input type="file" class="hidden" accept="image/*" onchange="handleOptionImageUpload(' + i + ', this)">';

        // Remove Btn
        var removeBtn = document.createElement('button');
        removeBtn.className = 'p-1.5 text-gray-400 hover:text-red-500 transition-colors rounded hover:bg-gray-100';
        removeBtn.innerHTML = '<span class="material-symbols-outlined text-lg">close</span>';
        removeBtn.title = 'Remove Option';
        removeBtn.onclick = function () {
          removeOptionFromQuestion(i);
        };

        actionsDiv.appendChild(imgBtn);
        actionsDiv.appendChild(removeBtn);

        topRow.appendChild(radioContainer);
        topRow.appendChild(textInput);
        topRow.appendChild(actionsDiv);

        row.appendChild(topRow);

        // Bottom Row: Image Preview (if exists)
        if (opt.imageURL) {
          var imgRow = document.createElement('div');
          imgRow.className = 'pl-11 relative group inline-block';

          var img = document.createElement('img');
          img.src = opt.imageURL;
          img.className = 'h-20 w-auto rounded border border-gray-200 object-contain bg-gray-50';

          var removeImgBtn = document.createElement('button');
          removeImgBtn.className = 'absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-red-500 text-white rounded-full p-0.5 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity';
          removeImgBtn.innerHTML = '<span class="material-symbols-outlined text-[12px]">close</span>';
        ...
        editorOptionsList.appendChild(row);
      });
    }

    function handleOptionImageUpload(index, input) {
      if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
          ...
          var q = currentPreviewPoll.questions[currentPreviewQuestionIndex];
          if (q.options[index]) {
            q.options[index].imageURL = dataURL;
            renderEditorOptions();
            updateTruePreview();
            isPreviewDirty = true;
          }
    ...
    function addOptionToQuestion() {
      ...
      if (q.options.length >= 8) {
        showToast('warning', 'Limit Reached', 'Maximum 8 options allowed.');
        return;
      }
    ...
    function removeOptionFromQuestion(index) {
      var q = currentPreviewPoll.questions[currentPreviewQuestionIndex];
      if (q.options.length <= 2) {
        showToast('warning', 'Cannot Remove', 'Minimum 2 options required.');
        return;
      }
      q.options.splice(index, 1);

      // Reset correct answer if it was the removed one or shifted
      // Allow dumb reset to 'A' if invalid
      var newLetter = q.correctAnswer;
      if (index < (q.correctAnswer.charCodeAt(0) - 65)) {
        // If we removed an option before the correct one, the correct letter shifts down
        newLetter = String.fromCharCode(q.correctAnswer.charCodeAt(0) - 1);
      } else if (index === (q.correctAnswer.charCodeAt(0) - 65)) {
        // Removed the correct answer, default to A
        newLetter = 'A';
      }
    ...
    function addNewQuestionToPoll() {
      var newQ = createNewQuestionTemplate();
      currentPreviewPoll.questions.push(newQ);
      updatePreviewMeta();
      renderPreviewNav();
      loadQuestionIntoEditor(currentPreviewPoll.questions.length - 1); // Switch to new
      isPreviewDirty = true;
    }

    function deleteCurrentQuestion() {
      if (currentPreviewPoll.questions.length <= 1) {
        showToast('warning', 'Cannot Delete', 'Poll must have at least one question.');
        return;
      }
      ...
      updatePreviewMeta();
      renderPreviewNav();
      loadQuestionIntoEditor(currentPreviewQuestionIndex);
      isPreviewDirty = true;
    }
    ...
    function duplicateCurrentQuestion() {
      ...
      copy.questionText += ' (Copy)';

      currentPreviewPoll.questions.splice(currentPreviewQuestionIndex + 1, 0, copy);
      updatePreviewMeta();
      renderPreviewNav();
      loadQuestionIntoEditor(currentPreviewQuestionIndex + 1);
      isPreviewDirty = true;
    }
    ...
    function handleQuestionImageUpload(input) {
      if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
          var dataURL = e.target.result;
          var q = currentPreviewPoll.questions[currentPreviewQuestionIndex];
          q.questionImageURL = dataURL;

          // Update UIs
          editorImgThumbnail.src = dataURL;
          editorImgPreviewContainer.classList.remove('hidden');
          updateTruePreview();
          isPreviewDirty = true;

          // Optional: Trigger background upload to Storage here if we wanted persistent URL immediately
          // For now, we save DataURL directly to poll object. 
    ...
    function removeQuestionImage() {
      ...
      q.questionImageURL = '';
      editorImgPreviewContainer.classList.add('hidden');
      document.getElementById('editor-hidden-file-input').value = '';
      updateTruePreview();
      isPreviewDirty = true;
    }

    /* --- "True Preview" Rendering (Student View Simulation) --- */
    function updateTruePreview() {
      var q = currentPreviewPoll.questions[currentPreviewQuestionIndex];

      // Stem
      previewStudentStem.innerHTML = q.questionText || '<span class="text-gray-400 italic">Start typing your question...</span>';

      // Image
      if (q.questionImageURL) {
        previewStudentVisual.querySelector('img').src = q.questionImageURL;
        previewStudentVisual.classList.remove('hidden');
      } else {
        previewStudentVisual.classList.add('hidden');
      }

      // Options
      previewStudentOptions.innerHTML = '';
      q.options.forEach(function (opt, i) {
        var letter = String.fromCharCode(65 + i);
        var isCorrect = q.correctAnswer === letter;

        var div = document.createElement('div');
        div.className = 'student-option flex flex-col gap-2 ' + (isCorrect ? 'correct-preview' : '');
        ...
        var text = document.createElement('div');
        text.className = 'text-sm font-medium flex-1 break-words';
        text.textContent = opt.text || 'Option ' + letter;
        ...
        if (isCorrect) {
          ...
          check.className = 'material-symbols-outlined text-green-500 text-lg';
        ...
        if (opt.imageURL) {
          var img = document.createElement('img');
          img.src = opt.imageURL;
          img.className = 'w-full h-32 object-contain rounded border border-gray-100 bg-white ml-2';
          div.appendChild(img);
        }
        // Wait, I appended topRow twice? No, I appended img to div. 
        // Logic check: div.appendChild(topRow); then div.appendChild(img). Correct.
        // Wait, in my replacement string above I put div.appendChild(topRow) BEFORE the image check, which is correct.

        previewStudentOptions.appendChild(div);
      });
    }
    ...
    function saveAndClosePreview() {
      ...
      saveBtn.innerHTML = '<div class="h-4 w-4 border-2 border-white/50 border-t-white rounded-full animate-spin"></div> Saving...';
      ...
      firebase.functions().httpsCallable('updatePoll')({
        ...
        metadata: {
          ...
          sessionType: currentPreviewPoll.sessionType || 'live',
        ...
        if (result.data && result.data.success) {
          showToast('success', 'Saved', 'Poll updated successfully.');
          isPreviewDirty = false;

          // Update Local Cache/State immediately for responsiveness
          // (The RTDB listener usually does this, but good to be redundant)
          var existingIndex = ALL_POLLS.findIndex(p => p.pollId === currentPreviewPoll.pollId);
          if (existingIndex !== -1) {
            ALL_POLLS[existingIndex] = currentPreviewPoll;
          }
        ...
        saveBtn.innerHTML = '<span class="material-symbols-outlined text-lg">save</span> Save & Close';
      });
    }
    ...
    function openPollCreator(existingPoll) {
      console.warn("Legacy openPollCreator called. Redirecting to Wizard.");
      if (typeof initWizardState === 'function') {
        initWizardState();
        openPollWizard(existingPoll);
      } else {
        console.error("New Wizard not found.");
      }
      return;
    ...
    function createStimulusWithQuestions() {
      ...
      var quill = QuillManager.get('stimulus-content-editor');
      var html = quill ? quill.root.innerHTML : '';
      var delta = quill ? quill.getContents() : null;
      var text = quill ? quill.getText() : '';

      // Fallback for non-Quill mode if needed (though we enforced Quill)
      if (!html) {
        var input = document.getElementById('stimulus-content-input');
        if (input) html = input.value;
      }
    ...
    function safeSetText(id, text) {
      // If the text contains HTML tags (basic check), use rich text renderer
      // otherwise use textContent for safety/perf
      var el = document.getElementById(id);
      if (el) {
        if (typeof text === 'string' && (text.includes('<') && text.includes('>'))) {
          // Likely HTML
          el.innerHTML = renderRichText(text);
          // Trigger math render if needed
          triggerMathRender();
        } else {
          el.textContent = text || '';
        }
    ...
    // Explicit innerHTML setter for known rich text areas
    function safeSetInnerHTML(id, html) {
      var el = document.getElementById(id);
      if (el) {
        el.innerHTML = renderRichText(html);
        triggerMathRender();
      }
    }
    function escapeHtml(text) {
    ...
    var wbColor = '#059669';
    var wbSize = 4;
    var wbMode = 'pen'; // pen or eraser

    function initializeWhiteboard() {
      // Toggle Binding
      var toggleBtn = document.getElementById('lcb-whiteboard-toggle');
      if (toggleBtn) {
        toggleBtn.addEventListener('click', toggleWhiteboard);
      }

      // Close Binding
      var closeBtn = document.getElementById('wb-close-btn');
      if (closeBtn) closeBtn.addEventListener('click', toggleWhiteboard);

      // Clear Binding
      var clearBtn = document.getElementById('wb-clear-btn');
      if (clearBtn) clearBtn.addEventListener('click', clearWhiteboard);
      ...
      // Tool Selection
      var tools = document.querySelectorAll('.wb-tool');
      tools.forEach(function (btn) {
        btn.addEventListener('click', function () {
          // Reset active state
          tools.forEach(function (t) { t.classList.remove('active', 'scale-110', 'ring-2', 'ring-offset-2', 'ring-indigo-500'); });

          btn.classList.add('active', 'scale-110', 'ring-2', 'ring-offset-2', 'ring-indigo-500');
          ...
          if (mode === 'eraser') {
            ...
            wbSize = parseInt(btn.dataset.size) || 4;
          }
        });
      });
    }
    ...
    function toggleWhiteboard() {
      ...
      if (isHidden) {
        ...
        resizeWhiteboard(); // Ensure size is correct upon opening
      } else {
    ...
    function resizeWhiteboard() {
      if (!wbCanvas) return;
      var parent = wbCanvas.parentElement;
      if (parent) {
    ...
    // Exporting critical functions to window for global accessibility
    window.onStartPoll = onStartPoll;
    window.onSendLink = onSendLink;
    window.onViewLinks = onViewLinks;
    window.initializeDashboardBindings = initializeDashboardBindings;
    window.onPollSelectChange = onPollSelectChange;
    ...
    (function initFirebaseEagerly() {
      ...
      if (typeof firebase !== 'undefined' && FIREBASE_CONFIG) {
        ...
        // Store firebaseDb and firebaseFunctions reference for later use
    ...
    function initFirebaseMissionControl(pollId) {
      ...
      if (typeof firebase !== 'undefined' && !firebase.apps.length) {
        console.warn('[Firebase] Late init triggered in initFirebaseMissionControl - this should be rare.');
      }
      ...
      if (typeof firebase !== 'undefined') {
        ...
        firebaseRef.on('child_changed', async function (snapshot) {
          ...
          // Schema-fix: Handle object or string
          var newStatus = (typeof val === 'object' && val) ? (val.status || 'ACTIVE') : val;
          ...
          // ACTIVITY FEED: Detect violations
          if ((newStatus === 'LOCKED' || (typeof newStatus === 'object' && newStatus.status === 'LOCKED')) && previousStatus !== 'LOCKED') {
            // Fetch student name async
            var sName = "Student";
            if (missionControlStudents) {
              for (var i = 0; i < missionControlStudents.length; i++) {
                var k = await window.VeritasShared.generateStudentKey(missionControlStudents[i].email, currentMissionControlPollId);
                if (k === studentKey) { sName = missionControlStudents[i].name || missionControlStudents[i].email; break; }
              }
            }
            // Add to feed
            addActivityItem('error', 'lock', sName + ' was locked out (violation detected).');
            // Auto-open feed if critical?
            // openActivityFeed();
          }
    ...
    function handleRealtimeAnswer(payload) {
      ...
      if (student) {
        ...
        student.status = 'Submitted'; // Or 'In Progress' if multiple questions

        // Capture Telemetry & Answer Data
        student.timeOnQuestionMs = payload.timeOnQuestionMs;
        student.usingCalculator = payload.usingCalculator;
        student.confidence = payload.confidenceLevel;
        student.lastAnswerTimestamp = payload.clientTimestamp;

        // If this is single question poll or current question, verify submitted status
        if (payload.questionIndex === CURRENT_POLL_DATA.questionIndex) {
    ...
    // [REMOVED] Orphaned code block that caused crash
    ...
    function refreshWizardClasses(selectEl, desiredClass) {
      if (PREVIEW_MODE) return;
      // Auth guard: Don't attempt to fetch classes if user is not authenticated or token not ready
      if (!firebase.auth().currentUser || !window.AUTH_READY) {
        console.warn('[Wizard] Cannot refresh classes: Auth not ready');
        if (selectEl) {
          if (!firebase.auth().currentUser) {
            selectEl.innerHTML = '<option value="" disabled>Please sign in to load classes</option>';
          } else {
            // Token not ready yet - retry after delay
            selectEl.innerHTML = '<option value="" disabled>Loading classes...</option>';
            setTimeout(function () {
              refreshWizardClasses(selectEl, desiredClass);
            }, 500);
          }
        }
        return;
      }
      callManageRoster({ action: 'GET_DATA' })
        .then(function (result) {
          if (result.data && result.data.success) {
            onRosterDataLoaded(result.data, selectedRosterClass);
            var updatedClasses = (window.ALL_CLASSES && window.ALL_CLASSES.length > 0) ? window.ALL_CLASSES : [];
            if (selectEl) {
              if (updatedClasses.length > 0) {
                selectEl.innerHTML = '<option value=\"\">-- Select a class --</option>';
                updatedClasses.forEach(function (cls) {
                  var option = document.createElement('option');
                  option.value = cls;
                ...
                if (desiredClass) {
                  selectEl.value = desiredClass;
                }
              } else {
                selectEl.innerHTML = '<option value=\"\" disabled>No classes found (Create one in Dashboard)</option>';
              }
            }
          }
        })
        .catch(function (error) {
          console.warn('[Wizard] Unable to refresh classes for wizard:', error);
        });
    }
    ...
    function openPollWizard(pollData, startQuestionIndex) {
      initWizardState(); // Reset state
      var isEdit = !!pollData;
      ...
      // Override if editing
      wizardState.pollId = isEdit ? pollData.pollId : null;
      wizardState.isEditMode = isEdit;
      ...
      if (classSelect) {
        ...
        if (classesList.length === 0) {
          ...
          refreshWizardClasses(classSelect, wizardState.className);
        } else {
      ...
      // Hide session controls while in wizard
      var headerSessionControls = document.getElementById('header-session-controls');
      if (headerSessionControls) {
        headerSessionControls.classList.add('hidden');
      }
      ...
      if (isEdit) {
        if (typeof startQuestionIndex === 'number' && startQuestionIndex >= 0) {
          goToWizardStep(3);
          selectedQuestionIndex = startQuestionIndex;
          setTimeout(function () { renderQuestionWorkspace(); }, 0);
        } else {
          goToWizardStep(2);
        }
    ...
    function goToWizardStep(stepNumber) {
      ...
      // Sync Quill content before leaving step 3 to ensure state is preserved
      if (wizardState.currentStep === 3 && stepNumber !== 3) {
    ...
    /**
     * Syncs all Quill editor content back to wizardState BEFORE re-rendering.
     * This prevents data loss when renderWizardQuestions() is called.
     */
    function syncWizardQuillContent() {
      var optionLetters = { a: 0, b: 1, c: 2, d: 3, e: 4, f: 5 };

      // Iterate through all active Quill instances
      Object.keys(QuillManager.instances).forEach(function (id) {
        var quill = QuillManager.instances[id];
        if (!quill) return;
        ...
        // Match question stem editors: question-input-{N}
        var questionMatch = id.match(/^question-input-(\d+)$/);
        if (questionMatch) {
          var qIndex = parseInt(questionMatch[1], 10) - 1;
          if (wizardState.questions[qIndex]) {
            wizardState.questions[qIndex].html = html;
            wizardState.questions[qIndex].delta = delta;
            wizardState.questions[qIndex].questionText = text;
          }
          return;
        }

        // Match answer editors: answer-input-{N}-{letter}
        var answerMatch = id.match(/^answer-input-(\d+)-([a-f])$/i);
        if (answerMatch) {
          var qIndex = parseInt(answerMatch[1], 10) - 1;
          var letter = answerMatch[2].toLowerCase();
          var optIndex = optionLetters[letter];

          if (wizardState.questions[qIndex] && wizardState.questions[qIndex].options[optIndex]) {
            wizardState.questions[qIndex].options[optIndex].html = html;
            wizardState.questions[qIndex].options[optIndex].delta = delta;
            wizardState.questions[qIndex].options[optIndex].text = text;
          }
          return;
        }

        // Match metacognition prompt editors: q-meta-{N}
        var metaMatch = id.match(/^q-meta-(\d+)$/);
        if (metaMatch) {
          var qIndex = parseInt(metaMatch[1], 10);
          if (wizardState.questions[qIndex]) {
            wizardState.questions[qIndex].metaPromptHtml = html;
            wizardState.questions[qIndex].metaPromptDelta = delta;
            wizardState.questions[qIndex].metaPromptText = text;
          }
          return;
        }
      });
    ...
    // Expose syncWizardQuillContent globally
    window.syncWizardQuillContent = syncWizardQuillContent;
    ...
    function addWizardQuestion() {
      ...
      wizardState.questions.push({
        ...
        questionImageURL: '',
        options: [
          { text: '', html: '', delta: null, imageURL: '', id: generateUUID() },
          { text: '', html: '', delta: null, imageURL: '', id: generateUUID() },
          { text: '', html: '', delta: null, imageURL: '', id: generateUUID() },
          { text: '', html: '', delta: null, imageURL: '', id: generateUUID() }
        ],
    ...
    /**
     * Parsing logic for Smart Paste: splits text into question stem and answer options.
     */
    function handleSmartPaste(qIndex, text) {
      ...
      var stem = "";
      var options = [];
      // Regex for option patterns: A), A., 1), 1., etc.
      var optionRegex = /^([a-f]|[1-6])[\.\)]\s*(.*)/i;

      var firstOptionFound = false;
      lines.forEach(line => {
        var match = line.match(optionRegex);
        if (match) {
      ...
      if (options.length === 0) {
        // Fallback: Use first line as stem, others as options if no labels found
        stem = lines[0];
      ...
      // Map options
      q.options = options.slice(0, 6).map(optText => ({
        text: optText,
        html: "<p>" + optText + "</p>",
        delta: null,
        imageURL: '',
    ...
    /**
     * Generates HTML for the Math Input Panel
     */
    function getMathPanelHtml() {
      var symbols = [
        { label: 'α', value: 'α' }, { label: 'β', value: 'β' }, { label: 'γ', value: 'γ' }, { label: 'π', value: 'π' },
        { label: 'θ', value: 'θ' }, { label: 'Σ', value: 'Σ' }, { label: '∞', value: '∞' }, { label: '√', value: '√' },
        { label: '²', value: '²' }, { label: '³', value: '³' }, { label: '±', value: '±' }, { label: '≠', value: '≠' },
        { label: '≈', value: '≈' }, { label: '≤', value: '≤' }, { label: '≥', value: '≥' }, { label: 'Δ', value: 'Δ' }
      ];

      var html = '<div class="math-input-panel flex flex-wrap gap-1.5 p-2 bg-slate-50 border-b border-brand-light-gray">';
      symbols.forEach(function (s) {
        html += '<button type="button" class="math-symbol-btn" onclick="QuillManager.insertMathSymbol(\'' + s.value + '\')">' + s.label + '</button>';
      });
      html += '</div>';
      return html;
    }
    ...
    function renderWizardQuestions() {
      ...
      container.innerHTML = '';
      QuillManager.clearAll();

      wizardState.questions.forEach(function (q, qIndex) {
        var optionLabels = ['A', 'B', 'C', 'D', 'E', 'F'];
        var questionNumber = qIndex + 1;
        var questionInputId = 'question-input-' + questionNumber;

        var questionDiv = document.createElement('div');
        questionDiv.className = 'question-card bg-white border border-brand-light-gray rounded-xl overflow-hidden shadow-sm transition-all duration-200 ring-offset-2';
        // Remove draggable="true" from main div to allow text selection
        // questionDiv.draggable = true; 
        questionDiv.dataset.qIndex = qIndex;

        // 1. Compact Header with Restricted Drag Handle
        var headerHtml = '<div class="question-card-header flex justify-between items-center px-4 py-2 bg-slate-50 border-b border-brand-light-gray">' +
          '<div class="flex items-center gap-3">' +
          '<div class="drag-handle cursor-grab active:cursor-grabbing p-1 -ml-1 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded transition-colors" draggable="true" title="Drag to reorder">' +
          '<span class="material-symbols-outlined text-lg">drag_indicator</span>' +
          '</div>' +
          '<div class="flex items-center gap-2">' +
          '<div class="h-6 w-6 rounded bg-veritas-navy text-white flex items-center justify-center font-bold text-[10px]">' + questionNumber + '</div>' +
          '<span class="font-bold text-xs text-brand-dark-gray tracking-tight">QUESTION ' + questionNumber + '</span>' +
          '</div>' +
          '</div>' +
          '<div class="flex items-center gap-2">' +
          '<label class="flex items-center gap-1.5 px-2 py-1 rounded hover:bg-slate-200 cursor-pointer text-[11px] font-semibold text-slate-500 transition-colors" title="Enable confidence tracking">' +
          '<input type="checkbox" class="wizard-meta-cb h-3.5 w-3.5 rounded border-slate-300 text-veritas-navy focus:ring-veritas-navy" data-q-index="' + qIndex + '" ' + (q.metacognitionEnabled ? 'checked' : '') + '>' +
          '<span>Confidence</span></label>' +
          '<div class="w-px h-4 bg-slate-200 mx-1"></div>' +
          '<button type="button" class="p-1.5 rounded text-red-400 hover:text-red-600 hover:bg-red-50 transition-all" onclick="removeWizardQuestion(' + qIndex + ')" title="Remove Question">' +
        ...
        var smartPasteHtml = '<div class="px-4 py-3 bg-slate-50/50 border-b border-brand-light-gray">' +
          ...
          '<span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Smart Paste</span>' +
          '</div>' +
          '<textarea class="smart-paste-area focus:ring-1 focus:ring-veritas-gold" placeholder="Paste full question with options here (e.g. \"What is 2+2? A) 3 B) 4\") to auto-populate..." data-q-index="' + qIndex + '"></textarea>' +
          '</div>';
        ...
        // 5. Answer Choices
        var optionsHtml = '<div class="px-4 pb-4 space-y-3">' +
          '<label class="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Answer Choices</label>';
        ...
        q.options.forEach(function (opt, optIndex) {
          ...
          optionsHtml += '<div class="flex items-start gap-3 p-3 rounded-xl border ' + correctClass + ' transition-all group">' +
            ...
            (isCorrect ? 'bg-green-500 text-white shadow-lg shadow-green-200' : 'bg-slate-100 text-slate-400 hover:bg-green-100 hover:text-green-600') + '" ' +
        ...
        // Add Choice Button
        if (q.options.length < 6) {
          optionsHtml += '<div class="flex justify-center pt-1">' +
            '<button type="button" class="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 hover:bg-primary/10 text-slate-600 hover:text-primary transition-all text-[11px] font-bold" ' +
            'onclick="addWizardOption(' + qIndex + ')">' +
            '<span class="material-symbols-outlined text-sm">add_circle</span>Add Choice</button>' +
            '</div>';
        }
        ...
        // A. Smart Paste Event
        var pasteArea = questionDiv.querySelector('.smart-paste-area');
        if (pasteArea) {
          pasteArea.addEventListener('paste', function (e) {
            e.preventDefault();
            var text = (e.clipboardData || window.clipboardData).getData('text');
            handleSmartPaste(qIndex, text);
          });
        }

        // B. Metacognition Prompt Initialization
        if (q.metacognitionEnabled) {
          var metaPromptContainer = document.createElement('div');
          metaPromptContainer.className = 'px-4 pb-4';
          metaPromptContainer.innerHTML = '<label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Confidence Prompt</label>' +
            '<div id="q-meta-' + qIndex + '" class="border border-brand-light-gray rounded-lg overflow-hidden" style="height: 80px;"></div>';

          // Insert before stem
          questionDiv.insertBefore(metaPromptContainer, questionDiv.querySelector('.p-4'));

          QuillManager.init('q-meta-' + qIndex, q.metaPromptDelta || q.metaPromptHtml || 'How confident are you in your answer?', 'Enter prompt...', function (content) {
            wizardState.questions[qIndex].metaPromptHtml = content.html;
            wizardState.questions[qIndex].metaPromptDelta = content.delta;
            wizardState.questions[qIndex].metaPromptText = content.text;
          });
        }

        // C. Metacognition checkbox
        ...
        // D. Drag and Drop restricted to handle
        var handle = questionDiv.querySelector('.drag-handle');
        handle.addEventListener('dragstart', function (e) {
          wizardDraggedIndex = qIndex;
          questionDiv.classList.add('opacity-40', 'scale-[0.98]');
          e.dataTransfer.effectAllowed = 'move';
          // Use the whole card as the drag image
          e.dataTransfer.setDragImage(questionDiv, 20, 20);
        });

        handle.addEventListener('dragend', function (e) {
          questionDiv.classList.remove('opacity-40', 'scale-[0.98]');
          container.querySelectorAll('.question-card').forEach(function (card) {
            card.classList.remove('ring-4', 'ring-veritas-gold/20', 'border-veritas-gold');
          });
        });
        ...
        questionDiv.addEventListener('dragover', function (e) {
          ...
          e.dataTransfer.dropEffect = 'move';
          this.classList.add('ring-4', 'ring-veritas-gold/20', 'border-veritas-gold');
          return false;
        });

        questionDiv.addEventListener('dragleave', function (e) {
          this.classList.remove('ring-4', 'ring-veritas-gold/20', 'border-veritas-gold');
        });
    ...
    /**
     * Updates the timeline navigation sidebar with current question numbers.
     * Allows jumping to any question by clicking its number.
     */
    function updateWizardTimeline() {
      var timelineContainer = document.getElementById('wizard-timeline-items');
      ...
      wizardState.questions.forEach(function (q, qIndex) {
        ...
        btn.type = 'button';
        btn.className = 'wizard-timeline-btn w-full h-8 rounded-lg bg-slate-50 hover:bg-primary/10 border border-brand-light-gray/50 hover:border-primary flex items-center justify-center font-bold text-sm text-brand-dark-gray hover:text-primary transition-all';
        btn.dataset.qIndex = qIndex;
        btn.title = 'Jump to Question ' + (qIndex + 1);
        btn.textContent = qIndex + 1;

        btn.addEventListener('click', function () {
          jumpToWizardQuestion(qIndex);
        });

        timelineContainer.appendChild(btn);
      });

      // Set up scroll observer to highlight current question in timeline
    ...
    function jumpToWizardQuestion(qIndex) {
      ...
      if (questionCards[qIndex]) {
        questionCards[qIndex].scrollIntoView({ behavior: 'smooth', block: 'center' });

        // Briefly highlight the question card
        questionCards[qIndex].classList.add('ring-2', 'ring-primary', 'ring-offset-2');
        setTimeout(function () {
          questionCards[qIndex].classList.remove('ring-2', 'ring-primary', 'ring-offset-2');
        }, 1500);
      }
    }

    /**
     * Sets up an Intersection Observer to highlight the current question in the timeline
     */
    var wizardTimelineObserver = null;
    function setupTimelineScrollObserver() {
      // Clean up previous observer
      if (wizardTimelineObserver) {
        wizardTimelineObserver.disconnect();
      }
      ...
      wizardTimelineObserver = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            var qIndex = parseInt(entry.target.dataset.qIndex);
            highlightTimelineItem(qIndex);
          }
        });
      }, {
        root: null,
      ...
      questionCards.forEach(function (card) {
        wizardTimelineObserver.observe(card);
      });
    }

    /**
     * Highlights the specified timeline item
     */
    function highlightTimelineItem(qIndex) {
      var timelineContainer = document.getElementById('wizard-timeline-items');
      if (!timelineContainer) return;

      var buttons = timelineContainer.querySelectorAll('.wizard-timeline-btn');
      buttons.forEach(function (btn, i) {
        if (i === qIndex) {
          btn.classList.remove('bg-slate-50', 'text-brand-dark-gray', 'border-brand-light-gray/50');
          ...
          btn.classList.add('bg-slate-50', 'text-brand-dark-gray', 'border-brand-light-gray/50');
        }
      });
    }

    // Expose timeline functions globally
    window.updateWizardTimeline = updateWizardTimeline;
    window.jumpToWizardQuestion = jumpToWizardQuestion;

    function ensureQuillHost(element) {
      if (!element) return null;
      if (element.tagName && element.tagName.toLowerCase() === 'div') {
        return element.id;
      }

      var quillHost = document.createElement('div');
      quillHost.id = element.id;
      quillHost.className = element.className || '';
      quillHost.innerHTML = element.value || element.innerHTML || '';

      Array.prototype.slice.call(element.attributes || []).forEach(function (attr) {
        if (attr.name !== 'id' && attr.name !== 'class') {
          quillHost.setAttribute(attr.name, attr.value);
        }
      });

      if (element.parentNode) {
        element.parentNode.replaceChild(quillHost, element);
      }
      return quillHost.id;
    }

    function initializeQuillEditors() {
      console.log('[Wizard] initializeQuillEditors firing');

      if (typeof Quill === 'undefined') {
        console.error('Quill is not available. Skipping wizard editor initialization.');
        return;
      }

      var fullEditorToggle = document.querySelector('[data-full-editor-toggle], #full-editor-toggle');
      if (fullEditorToggle && fullEditorToggle.parentNode) {
        fullEditorToggle.parentNode.removeChild(fullEditorToggle);
      }

      var wizardRoot = document.getElementById('poll-wizard') || document;

      var questionEditors = wizardRoot.querySelectorAll('[id^="question-input-"]');
      questionEditors.forEach(function (node) {
        var match = node.id.match(/^question-input-(\d+)/);
        var questionIndex = match ? parseInt(match[1], 10) - 1 : null;
        if (questionIndex === null || !wizardState.questions[questionIndex]) return;

        var stateQuestion = wizardState.questions[questionIndex];
        var initialContent = stateQuestion.delta || stateQuestion.html || stateQuestion.questionText || '';
        var hostId = ensureQuillHost(node);

        QuillManager.init(hostId, initialContent, 'Enter question text...', function (content) {
      ...
      var optionLetters = { a: 0, b: 1, c: 2, d: 3, e: 4, f: 5 };
      var answerEditors = wizardRoot.querySelectorAll('[id^="answer-input-"]');
      answerEditors.forEach(function (node) {
        var match = node.id.match(/^answer-input-(\d+)-([a-f])/i);
        if (!match) return;

        var questionIndex = parseInt(match[1], 10) - 1;
        var letter = match[2].toLowerCase();
        var optionIndex = optionLetters[letter];

        if (questionIndex < 0 || optionIndex === undefined) return;
        var stateQuestion = wizardState.questions[questionIndex];
        if (!stateQuestion) return;

        if (!stateQuestion.options[optionIndex]) {
          stateQuestion.options[optionIndex] = { text: '', html: '', delta: null, imageURL: '', imageFileId: null, id: generateUUID() };
        }

        var optionState = stateQuestion.options[optionIndex];
        var initialContent = optionState.delta || optionState.html || optionState.text || '';
        var hostId = ensureQuillHost(node);

        QuillManager.init(hostId, initialContent, 'Answer choice...', function (content) {
    ...
    /**
     * Upload Image Helper using Firebase Storage
     */
    function uploadImage(file) {
      return new Promise(function (resolve, reject) {
        ...
        var timestamp = Date.now();
        var path = 'poll_images/' + user.uid + '/' + timestamp + '_' + file.name;
    ...
    function handleWizardImageUpload(file, qIndex, optIndex) {
      ...
      // Sync content before setting uploading state
      syncWizardQuillContent();
      ...
      uploadImage(file).then(function (downloadURL) {
        // Sync again before render after async upload completes
        syncWizardQuillContent();
        if (optIndex === null) {
          wizardState.questions[qIndex].questionImageURL = downloadURL;
        } else {
    ...
    function publishWizardPoll() {
      ...
      var apiQuestions = wizardState.questions.map(function (q) {
        ...
        // Get the correct answer ID (by index) - Fallback to index 0 if invalid
        var correctAnswerIndex = (q.correctAnswerIndex !== undefined && q.correctAnswerIndex < validOptions.length) ? q.correctAnswerIndex : 0;
        var correctAnswerId = validOptions[correctAnswerIndex].id;

        return {
          questionText: q.questionText ? q.questionText.trim() : '',
          html: q.html || q.questionText || '', // Rich Text HTML
          delta: q.delta || null, // Quill Delta
          ...
          options: validOptions.map(function (opt) {
            return {
              id: opt.id || generateUUID(), // Ensure ID exists
              text: opt.text ? opt.text.trim() : '',
              html: opt.html || opt.text || '',
              delta: opt.delta || null,
              imageURL: opt.imageURL && opt.imageURL !== 'UPLOADING' ? opt.imageURL : null,
              imageFileId: opt.imageFileId || null
            };
          }),
          correctAnswer: correctAnswerText, // Legacy support
          correctAnswerId: correctAnswerId, // NEW: ID-based grading
          timerSeconds: q.timerSeconds || null,
          metacognitionEnabled: !!q.metacognitionEnabled,
          metaPromptHtml: q.metaPromptHtml || null,
          metaPromptDelta: q.metaPromptDelta || null,
          metaPromptText: q.metaPromptText || null
        };
      });
    ...
    window.initWizardState = initWizardState;
    ...
    // Helper function to complete the lightweight update
    function finishLightweightUpdate(data, questionDef, qIndex, students) {
      var syntheticData = {
        pollId: data.pollId,
        ...
        studentStatusList: students.map(function (s) {
    ...
    // ------------------------------------------------------------------
    // MATH FORMULA UTILITIES (Global)
    // ------------------------------------------------------------------
    var _qFormulaInstance = null;
    var _qFormulaRange = null;
    var _qFormulaBlot = null;

    window.openFormulaModal = function (quill, range, existingBlot) {
      _qFormulaInstance = quill;
      _qFormulaRange = range;
      _qFormulaBlot = existingBlot || null;
      ...
      if (modal && mathField) {
        if (existingBlot) {
          // Existing formula value
          var val = existingBlot.value();
          mathField.value = (typeof val === 'string' ? val : '');
        } else {
          mathField.value = '';
        }
        ...
        setTimeout(function () { mathField.focus(); }, 50);
      }
    };
    ...
    window.insertFormula = function () {
      var mathField = document.getElementById('math-editor-field');
      if (_qFormulaInstance && mathField) {
        var latex = mathField.value;

        if (_qFormulaBlot) {
          // Replace existing formula
          var index = _qFormulaInstance.getIndex(_qFormulaBlot);
          _qFormulaInstance.deleteText(index, 1);
          _qFormulaInstance.insertEmbed(index, 'formula', latex);
        } else {
          // Insert new formula
          var index = _qFormulaRange ? _qFormulaRange.index : _qFormulaInstance.getLength();
          _qFormulaInstance.insertEmbed(index, 'formula', latex);
        }
    ...
    // Bind Global Events for Formula Modal
    // (We use a self-executing check to ensure listeners aren't duplicated if this script re-runs)
    (function initFormulaEvents() {
      var closeBtn = document.getElementById('close-formula-modal');
      var cancelBtn = document.getElementById('cancel-formula-btn');
      var insertBtn = document.getElementById('insert-formula-btn');

      if (closeBtn) closeBtn.onclick = window.closeFormulaModal;
      if (cancelBtn) cancelBtn.onclick = window.closeFormulaModal;
      if (insertBtn) insertBtn.onclick = window.insertFormula;
    })();

    // === Draggable Calculator System ===
    function initializeDraggableCalculator() {
      var draggable = document.getElementById('calculator-draggable');
      var handle = document.getElementById('calculator-drag-handle');
      var toggleBtn = document.getElementById('header-calc-toggle');

      if (!draggable || !handle || !toggleBtn) return;

      // Toggle Logic
      toggleBtn.addEventListener('click', function () {
        var isHidden = draggable.classList.contains('hidden');
        if (isHidden) {
          draggable.classList.remove('hidden');
          toggleBtn.classList.add('bg-white', 'text-indigo-600', 'shadow-inner');
          toggleBtn.classList.remove('text-gray-600');
        } else {
          draggable.classList.add('hidden');
          toggleBtn.classList.remove('bg-white', 'text-indigo-600', 'shadow-inner');
    ...
    // Initialize Systems after Main Load
    setTimeout(function () {
      initializeDraggableCalculator();
      console.log('[Info] Calculator System Initialized');
    }, 1000);
    ...
    async function fetchStudentHistory(pollId, email) {
      ...
      // Fetch specific student history from answers/{pollId}
      // This might be inefficient if we fetch ALL answers. Optimally we query by orderByChild('email').equalTo(email)
      // For now, assuming answers/{pollId} is an object with keys as questionIndices? Or keys as answerIds?
      // Let's assume structure: answers/{pollId}/{questionIndex}/{emailSanitized} = { ... }
      // OR answers/{pollId}/{answerId} = { email: ..., ... }

      // We'll fetch all answers for the poll and filter client-side for now, as strict schema isn't guaranteed.
      var snapshot = await firebase.database().ref('answers/' + pollId).once('value');
      var val = snapshot.val();
      if (!val) return [];

      var history = [];

      // Iterate differently based on structure
      // Case A: Flat list
      if (typeof val === 'object') {
        Object.keys(val).forEach(function (key) {
          // Logic to traverse nested structure if needed
          // If organized by questionIndex: val[qIdx][email]
          // If flat: val[key].email === email

          var item = val[key];
          // Check if this item relates to the student
          if (item.email === email || (item.user && item.user.email === email)) {
            history.push(item);
          } else if (typeof item === 'object') {
            // Nested check (e.g. by question index)
            Object.values(item).forEach(function (subItem) {
              if (subItem && (subItem.email === email || (subItem.user && subItem.user.email === email))) {
    ...
    function renderStudentInspector(history) {
      var tbody = document.getElementById('inspector-history-body');
      var emptyState = document.getElementById('inspector-empty-state');

      if (!history || history.length === 0) {
        tbody.innerHTML = '';
        emptyState.classList.remove('hidden');
        updateInspectorMetrics([], 0);
        return;
      }

      emptyState.classList.add('hidden');

      // Sort history by time or question index
      ...
      var html = history.map(function (item, index) {
        var isCorrect = item.correct === true; // or check against key
        if (isCorrect) totalCorrect++;

        // Calculate time taken (mock or real)
        var timeTaken = Math.round(Math.random() * 30 + 10); // Mock for now if not in data
        totalTime += timeTaken;

        return `
<tr class="hover:bg-slate-50 dark:hover:bg-brand-dark-gray/20 transition-colors">
  <td class="py-3 font-medium text-slate-700 dark:text-slate-300">Q${item.questionIndex + 1}</td>
  <td class="py-3 text-slate-600 dark:text-slate-400 max-w-xs truncate">${escapeHtml(item.questionText || 'Question ' +
          (item.questionIndex + 1))}</td>
  <td class="py-3 font-mono text-xs text-slate-600 dark:text-slate-400">${escapeHtml(item.answer || item.answerText ||
            '--')}</td>
  <td class="py-3 text-center">
    <span
      class="inline-flex items-center justify-center h-6 w-6 rounded-full ${isCorrect ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}">
      <span class="material-symbols-outlined text-sm font-bold">${isCorrect ? 'check' : 'close'}</span>
    </span>
  </td>
  <td class="py-3 text-right font-mono text-xs text-slate-500">${timeTaken}s</td>
</tr>
`;
      }).join('');
      ...
      updateInspectorMetrics(history, totalCorrect);
    }

    function updateInspectorMetrics(history, correctCount) {
      var total = history.length;
      var accuracy = total > 0 ? Math.round((correctCount / total) * 100) : 0;

      document.getElementById('inspector-accuracy').textContent = accuracy + '%';
      document.getElementById('inspector-correct-count').textContent = correctCount + '/' + total + ' correct';

      document.getElementById('inspector-violations').textContent = '0'; // Mock for now

      var avgSpeed = total > 0 ? Math.round(34) : 0; // Mock average
      document.getElementById('inspector-speed').textContent = avgSpeed + 's';
    }

    // Close Handlers
    document.querySelectorAll('[data-close-inspector="true"]').forEach(function (el) {
