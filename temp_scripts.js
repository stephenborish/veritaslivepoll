  (function () {
    ...
    */
    ...
    function publishWizardPoll() {
      ...
      // FIREBASE MIGRATION: Write definitions directly to RTDB (Source of Truth)
      // Remove dependency on Google Sheets for poll storage.
      var dbRef = firebase.database().ref('polls');

      if (wizardState.isEditMode && wizardState.pollId) {
        // UPDATE EXISTING POLL
        var pollId = wizardState.pollId;
        var existingPollRef = dbRef.child(pollId);
        ...
        existingPollRef.update(updatePayload)
          .then(async function () {
            console.log('Poll updated in Firebase:', pollId);
          ...
          .catch(async function (error) {
            ...
            await veritasAlert('Failed to update: ' + error.message, { title: 'Error', destructive: true });
            if (publishBtn) {
        ...
        // CREATE NEW POLL
        var newPollRef = dbRef.push(); // Generate ID
        var newPollId = newPollRef.key;
        ...
        var newPollPayload = {
          ...
          ownerId: 'TEACHER' // Phase 1: Single user assumption
        };

        newPollRef.set(newPollPayload)
          .then(async function () {
            console.log('Poll created in Firebase:', newPollId);
            await veritasAlert('Poll published successfully!', { title: 'Success' });

            var response = {
              pollId: newPollId,
              pollName: newPollPayload.pollName,
              className: newPollPayload.className,
              questions: newPollPayload.questions,
              metadata: newPollPayload.metadata,
              success: true
            };
            completeWizardPublish(response);
          })
          .catch(async function (error) {
            ...
            await veritasAlert('Failed to publish: ' + error.message, { title: 'Error', destructive: true });
            if (publishBtn) {
              publishBtn.disabled = false;
